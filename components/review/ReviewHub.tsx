...
import { ArrowRight, ChevronRight, Home, Settings, Sparkles } from "lucide-react";

import { BLUNDR_TEMPO_ASSETS } from "@/lib/blundr/assets/blundrAssetManifest";
import { DAILY_MINI_GAME_REGISTRY, getDailyMiniGameDefinition } from "@/lib/blundr/daily/miniGames/dailyMiniGameRegistry";
...
export function ReviewHub({ embedded = false, homeHref = "/", settingsHref = "/settings", className }: ReviewHubProps) {
  return (
    <section className={classNames("space-y-4", className)}>
        ...
        <header className="rounded-[1.75rem] border border-stone-200 bg-white p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <h1 className="mt-3 text-2xl font-black tracking-tight text-stone-950">Review and practice</h1>
              <p className="mt-2 text-sm leading-6 text-stone-600">
                Practice the patterns that make Daily Blundr easier. Tempo keeps the queue calm and the minigames focused.
              </p>
            </div>
      ...
      <section className="rounded-[1.75rem] border border-stone-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          <BlundrAssetImage asset={BLUNDR_TEMPO_ASSETS.coach} alt="Tempo coach" variant="tempoCard" className="mx-auto w-full max-w-[9rem] sm:mx-0 sm:w-32" />
          <div className="min-w-0 flex-1">
      ...
      <section className="grid gap-3 md:grid-cols-2">
        {DAILY_MINI_GAME_REGISTRY.map((definition) => {
          const resolved = getDailyMiniGameDefinition(definition.id) ?? definition;
          return (
            <Link
              key={definition.id}
            ...
            >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap gap-2">
                    <span className="rounded-full bg-green-50 px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-green-700">
                      {resolved.canAppearInDailyBlundr === false ? "Practice only" : "Daily Blundr"}
                    </span>
                    <span className="rounded-full bg-stone-100 px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-stone-500">
                      {resolved.canAppearInStandalonePractice === false ? "Queue only" : "Standalone"}
                    </span>
                  </div>
                  <h3 className="mt-3 text-lg font-black tracking-tight text-stone-950">{resolved.displayName ?? resolved.title}</h3>
                  <p className="mt-2 text-sm leading-6 text-stone-600">{resolved.shortDescription ?? resolved.summary}</p>
                </div>
                <ChevronRight size={18} className="mt-1 text-stone-400 transition group-hover:text-green-700" />
              </div>
              ...
              <div className="mt-4 grid gap-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="rounded-2xl bg-white px-3 py-3 text-xs font-black uppercase tracking-[0.18em] text-stone-500 ring-1 ring-stone-200">
                    Time
                    <div className="mt-1 text-sm font-black uppercase tracking-normal text-stone-950">
                      {resolved.estimatedSeconds ? `${resolved.estimatedSeconds}s` : "Quick"}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl bg-[#fbfcf7] px-3 py-3 ring-1 ring-stone-200">
                <div className="text-sm font-black text-stone-950">Start practice</div>
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-[1.5rem] border border-stone-200 bg-white p-4 shadow-sm">
          <div className="text-xs font-black uppercase tracking-[0.18em] text-stone-500">Tempo coach</div>
