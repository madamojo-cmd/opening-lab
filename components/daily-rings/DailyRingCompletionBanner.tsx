...
export function DailyRingCompletionBanner({
  result,
  className,
}: DailyRingCompletionBannerProps) {
  ...
  if (result.ok === false) {
    return (
      ...
      >
        ...
        {result.code === "free_tempo_daily_limit_reached" ? (
          <Link
            href="/billing/upgrade"
            className="mt-3 inline-flex rounded-2xl bg-green-700 px-4 py-2 text-sm font-black text-white"
          >
            Upgrade to Pro
          </Link>
        ) : null}
  ...
  return (
    <div
      className={classNames(
        "rounded-3xl border border-green-200 bg-green-50 p-4 shadow-sm",
        className,
      )}
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
          <BlundrAssetImage
            asset={
              result.allRingsClosedThisAction || result.streakMilestones?.length
                ? BLUNDR_TEMPO_ASSETS.celebrate
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-xs font-black uppercase tracking-[0.18em] text-green-700">
        <span className="rounded-full bg-white px-3 py-1 ring-1 ring-green-200">
          +{result.repertoirePointsAwarded} points
        </span>
        ...
        {result.streakMilestones?.length ? (
          <span className="rounded-full bg-white px-3 py-1 ring-1 ring-green-200">
            {result.streakMilestones[0].milestoneDays}-day streak
          </span>
        ) : null}
      </div>
    </div>
