...
export function testStage2PrematureBranchCompleteBrowserSnapshotRegression(): void {
  ...
  assert.equal((snapshot.frame as any)?.instructionKind === "branch_complete", false);
  assert.equal((snapshot.coach as any)?.visibleTitle, "Nf3 — Develop the knight");
  assert.equal((snapshot.coach as any)?.visibleButtons?.includes("continue_from_here"), false);
  ...
  assert.equal((snapshot.health.criticalIssues ?? []).includes("branch_complete_rendered_before_minimum_line_progress"), false);
}

testStage2PrematureBranchCompleteBrowserSnapshotRegression();
console.log("stage2PrematureBranchCompleteBrowserSnapshotRegression ok");
