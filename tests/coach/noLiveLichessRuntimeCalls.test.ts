...
export function testNoLiveLichessRuntimeCalls(): void {
  ...
  assert.equal(
    source.includes("async function loadExplorer"),
    true,
    "loadExplorer_function_not_found",
  );
  assert.equal(
    source.includes("/api/explorer"),
    false,
    "loadExplorer_must_not_fetch_api_explorer",
  );
  assert.equal(
    source.includes("explorer.lichess.org"),
    false,
    "loadExplorer_must_not_call_liv_lichess",
  );
  assert.equal(
    source.includes("local_crawled_package"),
    true,
    "loadExplorer_must_use_local_crawled_package",
  );
  assert.match(
    source,
    /runtimePackageId:\s*STAGE2_RUNTIME_PACKAGE_ID/,
    "runtime_package_id_debug_missing",
  );
