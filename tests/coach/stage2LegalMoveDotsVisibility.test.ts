...
function legalDestinations(game: Chess, square: string): string[] {
  return (
    game.moves({ square: square as any, verbose: true }) as Array<{
      to: string;
    }>
  )
...
export function testStage2LegalMoveDotsVisibility(): void {
  ...
  assert.equal(
    /if\(\s*!instructionTarget\?\.uci\s*\|\|\s*suppressPlainPreTargetHighlights\s*\)/.test(
      pageSource,
    ),
    false,
  ...
  assert.equal(
    whiteVisibleSquares.has(whiteSelectedSquare),
    true,
    "white_selected_square_must_remain_visible",
  );
  assert.equal(
    whiteVisibleSquares.has("f3"),
    true,
    "white_legal_destination_must_remain_visible",
  );
  assert.equal(
    whiteVisibleSquares.has("h3"),
    true,
    "white_legal_destination_must_remain_visible",
  );
  ...
  assert.equal(
    blackVisibleSquares.has(blackSelectedSquare),
    true,
    "black_selected_square_must_remain_visible",
  );
  assert.equal(
    blackVisibleSquares.has("f6"),
    true,
    "black_legal_destination_must_remain_visible",
  );
  assert.equal(
    blackVisibleSquares.has("h6"),
    true,
    "black_legal_destination_must_remain_visible",
  );
}
