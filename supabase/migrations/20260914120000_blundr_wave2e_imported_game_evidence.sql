-- Wave 2E: preserve provider outcomes and make imported-game evidence auditable.

alter table public.blundr_external_games
  add column if not exists termination_reason text,
  add column if not exists provider_metadata jsonb not null default '{}'::jsonb;

alter table public.blundr_learning_findings
  add column if not exists outcome text,
  add column if not exists user_move_uci text,
  add column if not exists canonical_fen text,
  add column if not exists move_order_key text,
  add column if not exists expected_move_uci text,
  add column if not exists expected_move_set text[] not null default '{}',
  add column if not exists import_weight numeric not null default 0;

alter table public.blundr_learning_findings
  drop constraint if exists blundr_learning_findings_wave2e_outcome_check;

alter table public.blundr_learning_findings
  add constraint blundr_learning_findings_wave2e_outcome_check check (
    outcome is null
    or outcome in (
...
alter table public.blundr_learning_findings
  drop constraint if exists blundr_learning_findings_wave2e_weight_check;

alter table public.blundr_learning_findings
  add constraint blundr_learning_findings_wave2e_weight_check check (
    import_weight >= -0.2 and import_weight <= 1.5
  );

create index if not exists blundr_learning_findings_wave2e_priority_idx
  on public.blundr_learning_findings (
    user_id,
    status,
    opening_id,
    position_key,
...
-- Imported observations must never become recall attempts during provider
-- deletion/replay reconstruction.
