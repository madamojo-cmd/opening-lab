...
begin
  ...
  where name = 'blundr_rewards_hmac_secret'
  limit 1;
...
create or replace function public.blundr_apply_reward_transaction_v2_core(
...
begin
  ...
  if v_randomness_available and v_all_closed_this_action then
    ...
    if v_current_streak>0 and v_current_streak%30=0 then v_reward_trigger:='monthly_cache'; v_reward_mode:='guaranteed_cache'; v_should_reward:=true;
    elsif v_current_streak>0 and v_current_streak%7=0 then v_reward_trigger:='weekly_cache'; v_reward_mode:='guaranteed_cache'; v_should_reward:=true;
    elsif v_pity_count>=14 then v_reward_trigger:='all_rings_closed'; v_reward_mode:='pity_bonus'; v_should_reward:=true;
    else v_reward_trigger:='all_rings_closed'; v_reward_mode:='random_bonus'; v_should_reward:=true; end if;
    ...
    insert into public.blundr_reward_rolls(id,user_id,trigger,rolled_at,did_reward,reward_json,seed) values(v_reward_roll_id,p_user_id,v_reward_trigger,now(),true,jsonb_build_object('id',v_reward_roll_id || ':' || v_reward_rarity,'rarity',v_reward_rarity,'rewardType',coalesce(v_bonus_type,'unlock_points'),'amount',v_reward_amount),v_reward_roll_id) on conflict(id) do nothing;
    update public.blundr_reward_history set random_bonus_pity_counter=0,all_rings_days_since_random_reward=0,last_random_bonus_at=case when v_reward_mode in ('random_bonus','pity_bonus') then now() else last_random_bonus_at end,last_random_reward_local_date=case when v_reward_mode in ('random_bonus','pity_bonus') then v_local_date::text else last_random_reward_local_date end,last_pity_guarantee_local_date=case when v_reward_mode='pity_bonus' then v_local_date::text else last_pity_guarantee_local_date end,applied_reward_ids=array_append(applied_reward_ids,v_reward_roll_id || ':' || v_reward_rarity),updated_at=now() where user_id=p_user_id;
  end if;
  ...
  if v_should_reward then
    v_envelope := jsonb_build_object('transactionId',v_tx.id,'grantId',v_bonus_grant_id,'grantType',case when v_bonus_type is not null then v_bonus_type when v_reward_rarity='epic' then 'epic_points' else 'reward_bonus_points' end,'quantity',case when v_bonus_type is null then v_reward_amount else 1 end,'randomBonusType',coalesce(v_bonus_type,'unlock_points'),'rewardGrants',v_reward_grants,'rarity',v_reward_rarity,'amount',v_reward_amount,
    ...
    values(v_tx.id,p_user_id,'reward:' || v_completion_id,'toast',70,v_envelope,p_policy_version);
  end if;
...
begin
  ...
  return jsonb_build_object('id',v_row.id,'presentation_kind',v_row.presentation_kind,'envelope',v_row.envelope,'leaseExpiresAt',v_row.lease_expires_at);
end;
$$;

revoke all on function public.blundr_rewards_v2_hmac_random(text),
  public.blundr_apply_reward_transaction_v2_core(uuid,text,text,text,text,text,text),
  public.blundr_claim_reward_presentation_v2(uuid,text,integer)
  from public, anon, authenticated, service_role;
grant execute on function public.blundr_claim_reward_presentation_v2(uuid,text,integer)
  to service_role;
