-- PR-04 follow-up: repair the Daily v3 call chain so normalized evidence is
-- actually consumed by the inner transaction and the learning event identity
-- is taken from the validated event object, not from the v2 projector return.
begin;
...
grant execute on function public.blundr_commit_daily_action_v3_inner(uuid, text, jsonb) to service_role;
