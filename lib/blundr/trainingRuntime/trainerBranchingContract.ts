...
export function validateTrainerBranchingContract(
  ...
  const byKey = new Map(nodes.map((node) => [`${node.openingId}:${node.playKey}`, node]));
  ...
  const terminals = new Set(nodes.filter((node) => node.ply === TRAINER_MAX_PLY).map((node) => node.openingId));
  const issues: string[] = [];
  const rootsByOpening = new Map<string, number>();
  let edges = 0;
  for (const node of nodes) {
    if (node.ply > TRAINER_MAX_PLY) issues.push(`depth:${node.openingId}:${node.playKey}`);
    if (node.profileId && node.profileId !== TRAINER_PROFILE) issues.push(`profile:${node.openingId}:${node.playKey}`);
    if (typeof node.totalGames === "number" && node.totalGames < TRAINER_MIN_TOTAL_GAMES) issues.push(`frequency:${node.openingId}:${node.playKey}`);
    const moves = node.playKey.split(",").filter(Boolean);
    if (moves.length <= 1) {
      rootsByOpening.set(node.openingId, (rootsByOpening.get(node.openingId) ?? 0) + 1);
      continue;
    }
    ...
    const evidence = candidateByKey.get(`${node.openingId}:${parentKey}:${move}`);
    if (!evidence || typeof evidence.rank !== "number") issues.push(`evidence:${node.openingId}:${node.playKey}`);
    else if (evidence.rank > trainerBranchingCap(node.ply)) issues.push(`rank:${node.openingId}:${node.playKey}`);
  ...
  if (rootsByOpening.size !== openings.size || [...rootsByOpening.values()].some((count) => count !== 1)) issues.push("opening_roots");
  if (terminals.size !== openings.size) issues.push("terminal_openings");
  return { roots: rootsByOpening.size, nodes: nodes.length, edges, terminalOpenings: terminals.size, issues: issues.sort() };
}
