...
export function testFeatureOpportunityMapper(): void {
  ...
  const opportunities = mapFeaturesToOpportunities({ features, plans, expectedMoveUci: "f1c4", expectedMoveSan: "Bc4", visualRecipeId: "recipe", conceptId: "develop_with_pressure", trainerView: "assisted" });
  assert.equal(opportunities.some((opportunity) => opportunity.layer === "visual_recipe"), true);
  assert.equal(opportunities.some((opportunity) => opportunity.layer === "expected_move"), true);
}
