...
export function buildTrainerDebugSnapshot(input: Record<string, any>): TrainerDebugSnapshot {
  ...
  const snapshot: TrainerDebugSnapshot = {
    ...
    frame: {
      ...
      continueFromHereClickHandled: Boolean(input.continueFromHereClickHandled),
      continueFromHereClickBlockedReason: input.continueFromHereClickBlockedReason ?? null,
      terminalProof,
    ...
    continuation: {
      ...
      continuationSessionId: normalizeString(input.continuationSessionId ?? null),
      runtimeBookQueried: Boolean(input.runtimeBookQueried),
      runtimeBookOpeningId: input.runtimeBookOpeningId ?? null,
      runtimeBookPlayKeyBefore: normalizeRuntimePlayKey(input.runtimeBookPlayKeyBefore ?? "") ?? null,
      runtimeBookStatus: input.runtimeBookStatus ?? null,
      runtimeBookCandidateCount: Number(input.runtimeBookCandidateCount ?? 0),
      runtimeBookTopCandidateUci: normalizeRuntimeCastlingUci(input.runtimeBookTopCandidateUci) ?? null,
      runtimeBookTopCandidateSan: input.runtimeBookTopCandidateSan ?? null,
      runtimeBookTopCandidateRank: input.runtimeBookTopCandidateRank ?? null,
      runtimeBookTopCandidateGames: input.runtimeBookTopCandidateGames ?? null,
      runtimeBookTopCandidatePlayPct: input.runtimeBookTopCandidatePlayPct ?? null,
      opponentReplyAuthoritySource: input.opponentReplyAuthoritySource ?? null,
      opponentReplyAuthorityCandidateUci: normalizeRuntimeCastlingUci(input.opponentReplyAuthorityCandidateUci) ?? null,
      opponentReplyAuthorityCandidateSan: input.opponentReplyAuthorityCandidateSan ?? null,
      opponentReplyAuthorityCandidateGames: input.opponentReplyAuthorityCandidateGames ?? null,
      opponentReplyAuthorityCandidatePlayPct: input.opponentReplyAuthorityCandidatePlayPct ?? null,
      opponentReplyAuthorityRejectedReason: input.opponentReplyAuthorityRejectedReason ?? null,
      runtimeBookBookExhausted: Boolean(input.runtimeBookBookExhausted),
      runtimeBookFallbackUsed: Boolean(input.runtimeBookFallbackUsed),
      runtimeBookFallbackAuthority: input.runtimeBookFallbackAuthority ?? null,
      stage2OpeningDepthTargetPly: Number(input.stage2OpeningDepthTargetPly ?? 0),
      stage2OpeningCurrentPly: Number(input.stage2OpeningCurrentPly ?? 0),
      stage2OpeningDepthReached: Boolean(input.stage2OpeningDepthReached),
      runtimeGraphAuthorityUsed: input.runtimeGraphAuthorityUsed ?? null,
      runtimeGraphCurrentPlayKey: normalizeRuntimePlayKey(input.runtimeGraphCurrentPlayKey ?? "") ?? null,
      runtimeGraphCandidateCount: Number(input.runtimeGraphCandidateCount ?? 0),
      runtimeGraphSelectedCandidateUci: normalizeRuntimeCastlingUci(input.runtimeGraphSelectedCandidateUci) ?? null,
      selectedRuntimeLineUsedFor: input.selectedRuntimeLineUsedFor ?? null,
      hardRailDetected: Boolean(input.hardRailDetected),
      hardRailBlockedReason: input.hardRailBlockedReason ?? null,
      stage2CoachingResolverEnabled: Boolean(input.stage2CoachingResolverEnabled),
      stage2ApprovedContentEnabled: Boolean(input.stage2ApprovedContentEnabled),
      stage2SafeFallbackEnabled: Boolean(input.stage2SafeFallbackEnabled),
      stage2CoachingPacketKind: input.stage2CoachingPacketKind ?? "none",
      stage2CoachingSafetyStatus: input.stage2CoachingSafetyStatus ?? null,
      ...
      approvedPacketSourceBundle: input.trainerFrameResolution?.approvedContent?.sourceBundle ?? input.stage2ApprovedPacketSourceBundle ?? null,
      approvedPacketSourceFile: input.trainerFrameResolution?.approvedContent?.sourceFile ?? input.stage2ApprovedPacketSourceFile ?? null,
      approvedPacketStatus: input.trainerFrameResolution?.approvedContent?.packetStatus ?? input.stage2ApprovedPacketStatus ?? null,
      approvedPacketApprovalReadiness: input.trainerFrameResolution?.approvedContent?.approvalReadiness ?? input.stage2ApprovedPacketApprovalReadiness ?? null,
      approvedPacketMissReason: input.trainerFrameResolution?.approvedContent?.missReason ?? input.stage2ApprovedPacketMissReason ?? null,
      approvedPacketFallbackReason: input.trainerFrameResolution?.approvedContent?.fallbackReason ?? input.stage2ApprovedPacketFallbackReason ?? null,
      approvedPacketVisualSource: input.trainerFrameResolution?.approvedContent?.visualSource ?? input.stage2ApprovedPacketVisualSource ?? null,
    ...
    runtime: {
      ...
      lineSelectionMode: input.lineSelectionMode ?? null,
      lineSelectionSource: input.lineSelectionSource ?? (input.runtimeBookQueried ? "local_runtime_package" : null),
      lineSelectionWeighted: input.lineSelectionWeighted ?? Boolean(input.runtimeBookQueried),
      lineSelectionContentGated: input.lineSelectionContentGated ?? false,
      lineSelectionRuntimeBacked: input.lineSelectionRuntimeBacked ?? Boolean(input.runtimeBookQueried),
      lineSelectionEligibleCount: Number(input.lineSelectionEligibleCount ?? 0),
      lineSelectionEligibleLineIds: Array.isArray(input.lineSelectionEligibleLineIds) ? input.lineSelectionEligibleLineIds.map(String) : [],
      lineSelectionEligibleLineKeys: Array.isArray(input.lineSelectionEligibleLineKeys) ? input.lineSelectionEligibleLineKeys.map(String) : [],
      ...
      selectedRuntimeLineId: input.selectedRuntimeLineId ?? null,
      selectedRuntimeLineKey: normalizeString(input.selectedRuntimeLineKey ?? null),
      selectedRuntimeLineIndex: input.selectedRuntimeLineIndex ?? null,
      selectedRuntimeLinePlayKey: normalizeRuntimePlayKey(input.selectedRuntimeLinePlayKey ?? null) ?? null,
      selectedRuntimeLinePlaySequenceUci: Array.isArray(input.selectedRuntimeLinePlaySequenceUci)
        ? normalizeRuntimePlaySequenceUci(input.selectedRuntimeLinePlaySequenceUci)
        : [],
    ...
    maia: {
      ...
      maiaContinuationEnabled: Boolean(input.maiaContinuationEnabled ?? (input.trainingMode === "continuation" && input.userExplicitlyEnteredContinuation)),
      maiaContinuationStatus: input.maiaContinuationStatus ?? maiaProviderStatus,
      maiaContinuationLoaded: Boolean(input.maiaContinuationLoaded ?? maiaProviderStatus === "ready"),
      maiaContinuationError: input.maiaContinuationError ?? maiaRuntimeErrorReason ?? null,
      maiaOpponentProviderUsed: Boolean(input.maiaOpponentProviderUsed ?? (input.trainingMode === "continuation" && input.userExplicitlyEnteredContinuation)),
      maiaOpponentRequestPending: Boolean(input.maiaOpponentRequestPending ?? input.pendingOpponentRequest),
      maiaOpponentLastMoveUci: input.maiaOpponentLastMoveUci ?? input.lastMoveUci ?? null,
