...
import type { MaiaRuntimeConfig, MaiaRuntimeHealth, MaiaRuntimeStatus } from "./maiaRuntimeTypes";
...
function parseIntBounded(value: string | undefined, fallback: number, min: number, max: number): number {
...
function parseSkill(value: string | undefined): MaiaSkillLevel {
  const candidate = String(value ?? "maia-1500").trim().toLowerCase() as MaiaSkillLevel;
...
export function readMaiaRuntimeConfig(): MaiaRuntimeConfig {
  ...
  const timeoutMs = parseIntBounded(process.env.MAIA_TIMEOUT_MS, 1500, 250, 5000);
  const nodes = parseIntBounded(process.env.MAIA_NODES, 1, 1, 128);
  const maxConcurrentRequests = parseIntBounded(process.env.MAIA_MAX_CONCURRENT_REQUESTS, 2, 1, 8);
...
export function evaluateMaiaRuntimeConfig(config: MaiaRuntimeConfig): { status: MaiaRuntimeStatus; errorReason: string | null } {
  ...
  if (!existsSync(config.weightsPath)) return { status: "weights_not_found", errorReason: "weights_not_found" };
