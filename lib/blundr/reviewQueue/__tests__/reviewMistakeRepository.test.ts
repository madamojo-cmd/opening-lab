...
test("review mistake snapshot filters by user_id and does not select expected move", async () => {
  ...
  const fake = new FakeAdminClient({
    ...
    blundr_learning_events: (q) => {
      ...
      assert.equal(q.selectColumns.includes("expected_move_uci"), false);
      return {
        position_key: mistakeId,
...
test("review mistake solution filters by user_id and selects expected move only on solution path", async () => {
  process.env.BLUNDR_FEATURE_LEARNING_CORE_V2_READ = "true";
  const userId = "user-a";
