import { Chess } from "chess.js";

const UCI_PATTERN = /^[a-h][1-8][a-h][1-8][qrbn]?$/;

export type VerifiedContinuationPath = {
  pathUci: string[];
  completedFen: string;
};

export function verifyContinuationPath(input: {
  terminalFen: string;
  userColor: "w" | "b";
  pathUci: readonly unknown[];
}): VerifiedContinuationPath {
  const pathUci = input.pathUci.map((move) =>
    String(move ?? "")
      .trim()
      .toLowerCase(),
  );
  if (
    pathUci.length < 1 ||
    pathUci.length > 2 ||
    pathUci.some((move) => !UCI_PATTERN.test(move))
  )
    throw new Error("continuation_path_invalid");

  let game: Chess;
  try {
    game = new Chess(input.terminalFen);
  } catch {
    throw new Error("continuation_terminal_position_invalid");
  }
  if (game.isGameOver() || game.moves().length === 0)
    throw new Error("continuation_terminal_position_unplayable");

  let userMoveCount = 0;
  let lastMover: "w" | "b" | null = null;
  for (const uci of pathUci) {
    const mover = game.turn();
    let applied = null;
    try {
      applied = game.move({
        from: uci.slice(0, 2),
        to: uci.slice(2, 4),
        promotion: uci[4] as "q" | "r" | "b" | "n" | undefined,
      });
    } catch {
      applied = null;
    }
    if (!applied) throw new Error("continuation_move_illegal");
    if (mover === input.userColor) userMoveCount += 1;
    lastMover = mover;
  }
  if (userMoveCount !== 1 || lastMover !== input.userColor)
    throw new Error("continuation_user_move_unverified");

  return { pathUci, completedFen: game.fen() };
}
