...
export async function commitContinuationCompletion(input: {
  ...
  if (!trainerSessionId || !Array.isArray(input.pathUci))
    throw new Error("continuation_completion_invalid");
  const admin = createBlundrSupabaseAdminClient();
  if (!admin)
    throw new Error("continuation_completion_persistence_unavailable");
  ...
  const verified = verifyContinuationPath({
  ...
  const completionId = `continuation-completion:${sha256(identityMaterial)}`;
  ...
  const result = await admin.rpc("blundr_commit_continuation_completion_v1", {
    ...
    p_completion: {
      ...
      completed_fen: verified.completedFen,
