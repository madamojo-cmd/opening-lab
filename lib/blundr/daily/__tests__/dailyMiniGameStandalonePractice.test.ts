...
void (async () => {
  ...
  const practiceBundle = await waitForPracticeBundle(
    "king_race",
    0,
    [],
    userId,
  ...
  assert.equal(
    practiceBundle?.card.miniGame.scenario?.source,
    "standalone_review",
  );
  assert.equal(practiceBundle?.card.miniGame.completed, false);
  assert.equal(
    practiceBundle?.card.miniGame.currentFen,
    practiceBundle?.card.miniGame.startFen,
  );

  const practiceScenario = buildLegacyTestScenario(practiceBundle!.card);
  assert.ok(practiceScenario);
  assert.equal(
    practiceScenario?.board.fen,
    practiceBundle?.card.miniGame.startFen,
  );
  ...
  assert.equal(
    buildLegacyPracticeBundle("unknown_mini_game", 0, [], userId),
    null,
  );
  ...
  assert.equal(
    after.currentSession?.deckFingerprint,
    before.currentSession?.deckFingerprint,
  );

  const summary = loadBlundrProgressSummary({ userId, now });
  assert.equal(summary.trainingVolume.minigamesToday >= 1, true);
  assert.equal(
    summary.trainingVolume.dailyBlundrToday,
    before.reviewAttempts.length,
  );
