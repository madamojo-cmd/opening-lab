...
import {
  DAILY_MINI_GAME_REGISTRY,
  getDailyMiniGameDefinition,
} from "../miniGames/dailyMiniGameRegistry";

export function testDailyMiniGameRegistry(): void {
  assert.deepEqual(
    DAILY_MINI_GAME_REGISTRY.map((definition) => definition.id),
    [
      "king_race",
      "knight_gymnasium",
      "pawn_wars",
  ...
  assert.equal(
    getDailyMiniGameDefinition("tactic_shots")?.title,
    "Tactic Shots",
  );
}
