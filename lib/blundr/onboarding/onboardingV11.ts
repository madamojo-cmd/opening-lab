...
import type { CurrentBlundrUser, RatingBandId, StarterPackId } from "@/lib/blundr/accounts/accountTypes";
...
import { buildInitialRepertoireFromStarterPack, getStarterPackById } from "./starterPacks";
import { ONBOARDING_V11_PACE_GOALS, ONBOARDING_V11_PRIORITIES, ONBOARDING_V11_STEPS, getEarliestIncompleteOnboardingV11Step, type OnboardingPriority, type OnboardingV11Pace, type OnboardingV11State, type OnboardingV11Step } from "./onboardingV11Contract";
export { ONBOARDING_V11_STEPS, getEarliestIncompleteOnboardingV11Step, getOnboardingV11PaceGoals, type OnboardingPriority, type OnboardingV11Pace, type OnboardingV11State, type OnboardingV11Step } from "./onboardingV11Contract";
...
function isRatingBand(value: unknown): value is RatingBandId {
  return ["new_to_openings", "u800", "800-1200", "1200-1600", "1600-2000", "2000-plus"].includes(text(value));
}
...
function priorities(value: unknown): OnboardingPriority[] {
  return Array.from(new Set(Array.isArray(value) ? value.filter((item): item is OnboardingPriority => ONBOARDING_V11_PRIORITIES.includes(item as OnboardingPriority)) : []));
}

function paceFromGoals(row: ProfileRow): OnboardingV11Pace | null {
  for (const [pace, goals] of Object.entries(ONBOARDING_V11_PACE_GOALS) as Array<[OnboardingV11Pace, (typeof ONBOARDING_V11_PACE_GOALS)[OnboardingV11Pace]]>) {
    if (Number(row.daily_tempo_goal) === goals.tempo && Number(row.daily_battery_goal) === goals.battery && Number(row.daily_blundr_goal) === goals.daily) return pace;
  }
  return null;
}

export function normalizeOnboardingV11ProfileRow(row: ProfileRow | null | undefined): OnboardingV11State {
  const completed = Boolean(row?.onboarding_completed);
  return {
    step: isStep(row?.onboarding_step) ? row.onboarding_step : completed ? "ready" : "welcome",
    completed,
    ...
    starterPackId: getStarterPackById(text(row?.selected_starter_pack_id) as StarterPackId)?.id ?? null,
    trainingMode: isMode(row?.preferred_training_mode) ? row.preferred_training_mode : null,
...
function nextStep(step: OnboardingV11Step): OnboardingV11Step {
  ...
  return ONBOARDING_V11_STEPS[Math.min(index + 1, ONBOARDING_V11_STEPS.length - 1)] ?? "ready";
}

function profileClient(user: CurrentBlundrUser) {
  return createBlundrSupabaseServerClient({ accessToken: user.accessToken, forUserQueries: true });
}

async function ensureProfile(user: CurrentBlundrUser): Promise<ProfileRow | null> {
  const client = profileClient(user);
  if (!client) return null;
  const { data: existing, error } = await client.from("blundr_user_profiles").select("*").eq("user_id", user.userId).maybeSingle();
  ...
  if (existing) {
    if (user.age13Confirmed && !(existing as ProfileRow).age_confirmed_at) {
      const { data, error: confirmationError } = await client
        ...
        .select("*")
        .single();
      if (confirmationError || !data) throw new Error("onboarding_persistence_unavailable");
  ...
  const { data, error: insertError } = await client
    ...
    .select("*")
    .single();
  if (insertError || !data) throw new Error("onboarding_persistence_unavailable");
  return data as ProfileRow;
}

export async function readOnboardingV11State(user: CurrentBlundrUser): Promise<OnboardingV11State> {
...
export async function saveOnboardingV11Step(
  ...
  const update: Record<string, unknown> = { onboarding_step: nextStep(input.step), updated_at: now };
  if (input.step === "welcome") update.onboarding_started_at = existing.startedAt ?? now;
  if (input.step === "level") {
  ...
  if (input.step === "starter-pack") {
    if (!getStarterPackById(text(input.value) as StarterPackId)) throw new Error("invalid_starter_pack");
  ...
  if (input.ageConfirmed === true) update.age_confirmed_at = existing.ageConfirmed ? existing.startedAt ?? now : now;
  const { data, error } = await client.from("blundr_user_profiles").update(update).eq("user_id", user.userId).select("*").single();
...
export async function completeOnboardingV11(user: CurrentBlundrUser): Promise<OnboardingV11State> {
  const state = await readOnboardingV11State(user);
  const earliest = getEarliestIncompleteOnboardingV11Step(state);
  if (earliest !== "plan" && earliest !== "ready") throw new Error(`onboarding_incomplete:${earliest}`);
  if (!state.starterPackId || !state.ratingBandId || !state.pace || !state.trainingMode || state.priorities.length === 0) throw new Error("onboarding_incomplete:plan");
  const adapter = getAccountPersistenceAdapter({ user, accessToken: user.accessToken, mode: user.mode, allowLocalFallback: false });
  const now = nowIso();
  const repertoire = buildInitialRepertoireFromStarterPack({ userId: user.userId, starterPackId: state.starterPackId, now });
  const repertoireResult = await adapter.upsertUserRepertoire(repertoire);
  if (!repertoireResult.ok) throw new Error("starter_repertoire_persistence_unavailable");
  const verifiedRepertoire = await adapter.getUserRepertoire(user.userId);
  if (!verifiedRepertoire.ok || !verifiedRepertoire.data || !repertoire.unlockedOpeningIds.every((openingId) => verifiedRepertoire.data?.unlockedOpeningIds.includes(openingId))) {
  ...
  const { data, error } = await client
    .from("blundr_user_profiles")
    .update({ onboarding_completed: true, onboarding_step: "ready", onboarding_completed_at: now, updated_at: now })
    .eq("user_id", user.userId)
    .select("*")
    .single();
  if (error || !data) throw new Error("onboarding_persistence_unavailable");
