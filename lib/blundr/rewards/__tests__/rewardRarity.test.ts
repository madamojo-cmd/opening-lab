...
assert.equal(pickRewardTypeForRarity("uncommon"), "opening_fragment");
...
const uncommonReward = buildRewardReward({
...
assert.equal(uncommonReward.rarity, "uncommon");
assert.equal(uncommonReward.rewardType, "opening_fragment");
assert.equal(uncommonReward.amount, 1);
