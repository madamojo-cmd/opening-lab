...
assert.match(service, /resolveCommercialAccess/);
assert.match(service, /FREE_DAILY_TEMPO_RUN_LIMIT/);
assert.match(service, /completion_time_zone_unavailable/);
assert.match(service, /Intl\.DateTimeFormat/);
assert.match(service, /if \(!timeZone\) throw new Error\("completion_time_zone_unavailable"\)/);
assert.match(service, /catch \{\n    throw new Error\("completion_time_zone_unavailable"\)/);
assert.match(service, /state.*completed/);
assert.match(service, /terminal_completion_id/);
assert.match(createRoute, /free_tempo_daily_limit_reached/);
assert.match(createRoute, /403/);
assert.match(actionRoute, /free_tempo_daily_limit_reached/);
assert.match(actionRoute, /403/);
assert.match(page, /error\.code === "free_tempo_daily_limit_reached"/);
assert.match(page, /router\.push\("\/billing\/upgrade"\)/);
