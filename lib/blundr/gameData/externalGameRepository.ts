    ...
    const result = await client.from("blundr_external_games").upsert(
      {
        ...
        provider_metadata: game.providerMetadata,
      ...
      {
        onConflict: game.providerFingerprint
          ? "user_id,provider_fingerprint"
          : "user_id,fallback_fingerprint",
      },
    );
    ...
    const result = await client.from("blundr_learning_findings").upsert(
      {
        ...
        canonical_fen: finding.position.canonicalFen,
        opening_id: finding.position.openingId,
        move_order_key: finding.position.moveOrderKey,
        expected_move_uci: finding.position.expectedMoveUci,
        ...
        user_move_uci: finding.userMoveUci,
        expected_move_set: finding.expectedMoveSet,
        import_weight: finding.importWeight,
