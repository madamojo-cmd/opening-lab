...
export async function recordTempoRunCompleted(args: {
  ...
  return completeDailyRingActivity({
    userId: args.userId,
    activity: { userId: args.userId ?? "", source: "opening_run_completed", openingId: normalizeOpeningId(args.openingId), dailySessionId: normalizeText(args.runSessionId) || undefined, completionId },
...
export async function recordBatteryLineCompleted(args: {
  ...
  return completeDailyRingActivity({
    userId: args.userId,
    activity: { userId: args.userId ?? "", source: "continuation_completed", openingId: normalizeOpeningId(args.openingId), dailySessionId: normalizeText(args.continuationRunId) || undefined, completionId },
    repertoireProgress: args.repertoireProgress ?? undefined,
    profile: args.profile ?? undefined,
    now: args.now,
  });
...
export async function recordBlundrTaskCompleted(args: {
  ...
  return completeDailyRingActivity({
    userId: args.userId,
    activity: { userId: args.userId ?? "", source: "daily_blundr_deck_completed", dailySessionId: normalizeText(args.reviewSessionId) || normalizeText(args.deckId) || undefined, completionId },
    repertoireProgress: args.repertoireProgress ?? undefined,
    profile: args.profile ?? undefined,
    now: args.now,
  });
