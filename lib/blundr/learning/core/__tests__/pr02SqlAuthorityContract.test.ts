...
const migration = readFileSync(
  resolve(root, "supabase/migrations/20260805140000_blundr_learning_daily_runtime_v2.sql"),
  "utf8",
);
...
test("PR-02 SQL authority is service-only and fail-closed at every mutation boundary", () => {
  for (const name of ["blundr_project_learning_evidence_v2", "blundr_reserve_daily_v2", "blundr_commit_daily_action_v2"])
    assert.match(migration, new RegExp(`public\\.${name}`));
  assert.match(migration, /grant execute on function public\.blundr_project_learning_evidence_v2[\s\S]*to service_role/);
...
test("PR-02 blocks spoofing, stale writes, duplicate actions, and reservation races", () => {
  for (const contract of ["p_event->>'user_id' <> p_user_id::text", "for update", "learning_review_state_conflict", "learning_mastery_state_conflict", "action_id", "daily_session_conflict", "on conflict (user_id,local_date) do nothing"])
...
test("Daily uses exactly one commit RPC and does not issue a second learning transport", () => {
  ...
  assert.match(migration, /blundr_project_learning_evidence_v2\(p_user_id, p_action->'learning_event'\)/);
});

test("Daily policy and projection contracts retain explicit no-fabrication boundaries", () => {
  assert.match(dailyWriter, /previousFsrs: \(review\.data\?\.srs_state as never\) \?\? null/);
