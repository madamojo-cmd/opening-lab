...
import { renderProceduralExplanation } from "../proceduralExplanationEngine";

export function testProceduralExplanationEngine(): void {
  ...
  const selected = rankTeachingOpportunities(mapFeaturesToOpportunities({ features, plans, expectedMoveUci: "f1c4", expectedMoveSan: "Bc4", trainerView: "assisted", visualRecipeId: "r", conceptId: "develop_with_pressure" }))!;
  const rendered = renderProceduralExplanation({ opportunity: selected, features, plans });
  assert.equal(rendered.safetyStatus, "passed");
  assert.equal(rendered.body.includes("bishop") || rendered.body.includes("development"), true);
}
