...
import { appendDeveloperAuditLogEntry, readUserRepertoire, saveOpeningUnlockProgress, saveUserRepertoire } from "@/lib/blundr/accounts/accountRepository";
import { createDefaultOpeningUnlockProgress } from "@/lib/blundr/accounts/accountDefaults";
...
export async function POST(request: NextRequest) {
  ...
  const nextUnlocked = repertoire.unlockedOpeningIds.filter((id) => id !== openingId);
  const nextLocked = Array.from(new Set([...repertoire.lockedOpeningIds, openingId]));
