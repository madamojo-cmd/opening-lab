...
export async function POST(request: Request) {
  const expected = String(process.env.BLUNDR_GAME_DATA_CRON_SECRET ?? "").trim();
  const supplied = String(request.headers.get("x-blundr-cron-secret") ?? "").trim();
  if (!expected || supplied !== expected) return NextResponse.json({ error: "unauthorized" }, { status: 401 });
  if (!isGameDataWorkerEnabled()) return NextResponse.json({ error: "worker_disabled" }, { status: 503 });
  ...
  for (const pending of await jobs.nextPending(3)) {
    ...
    if (!account) {
      await jobs.update(leased.id, { status: "permanent_error", errorCode: "invalid_username" });
      continue;
    }
    const access = await loadOpeningAccess({ userId: leased.userId, isAuthenticated: true, mode: "authenticated", isAdmin: false, accessToken: null, email: null, provider: null });
    const result = await processGameImportBatch(leased, account, {
      runtime,
      jobs,
      workerId: leased.leaseOwner ?? "cron",
      source: leased.provider === "chesscom" ? new ChessComClient() : new LichessClient(),
      access: (userId, openingId, side) => access.get({ userId, openingId, repertoireSide: side }),
    });
    results.push({ jobId: leased.id, status: result.status, counts: result.counts });
  }
