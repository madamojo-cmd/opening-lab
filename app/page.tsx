...
function BottomNav({activeTab,setActiveTab}:{activeTab:string;setActiveTab:(tab:Tab)=>void}){const tabs=[{id:"home",label:"Home",icon:Home},{id:"train",label:"Train",icon:Target},{id:"review",label:"Review",icon:CheckCircle2},{id:"progress",label:"Progress",icon:BarChart3},{id:"repertoire",label:"Repertoire",icon:BookOpen}] as const;return <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-stone-200 bg-white/95 px-2 pb-4 pt-2 backdrop-blur"><div className="mx-auto grid max-w-md grid-cols-5 gap-1">{tabs.map(tab=>{const Icon=tab.icon;const active=activeTab===tab.id;const baseClass=classNames("flex flex-col items-center gap-1 rounded-2xl px-2 py-2 text-xs font-semibold",active?"bg-green-50 text-green-700":"text-stone-500");return tab.id==="repertoire"?<Link key={tab.id} href="/repertoire" className={baseClass}><Icon size={19}/>{tab.label}</Link>:<button key={tab.id} onClick={()=>setActiveTab(tab.id)} className={baseClass}><Icon size={19}/>{tab.label}</button>})}</div></nav>}
