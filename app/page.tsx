...
type ActiveLine = { from: string; to: string; kind: LineKind; label?: string; pathStyle?: "auto" | "curve" | "knight-l" | "temporal-gate"; gateState?: "locked" | "unlocking" | "open"; timing?: "now" | "not-yet" | "prepare"; motif?: "fork" | "pin" | "xray" | "hanging" | "defense" | "attack" | "plan" | "opponent" };
type TemporalGateSettings = { motion: boolean; labels: boolean; gateText: boolean; arrows: boolean; autoMotifs: boolean };
...
export default function App(){
  ...
  async function runBrain(eventType:string,extra:Record<string,any>={}){if(activeTab!=="train")return null;setVisualReady(false);setThinkingStep("facts");setPipelineNote("Analyzing chess.js facts and restricted opening context");setThinkingStep("engine");setPipelineNote("Preparing browser Stockfish analysis");setBrain(p=>({...p,engine:"loading",gpt:"loading",source:"Browser Stockfish"}));setPipelineNote("Running lightweight browser Stockfish before GPT");
    ...
    const payload={fen,openingId:repertoire.id,openingName:repertoire.name,userColor,trainingMode,eventType,selectedView:activeBoardView,moveHistory,lastMoveSan,lastMoveUci:lastMove,expectedMoves:expectedUserOptions.map(m=>({san:m.san,uci:m.uci})),opponentBookMoves:opponentBookOptions.map(m=>({san:m.san,uci:m.uci})),ratingPool:rating.target,ratingLabel:rating.label,ratingFilter,speedFilter,skill:rating.skill,clientEngine,...extra};setThinkingStep("brain");setPipelineNote("Sending Stockfish lines + board facts to GPT through /api/brain");const start=performance.now();try{const res=await fetch("/api/brain",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});const data=await res.json() as BrainResponse;setThinkingStep("gpt-receive");setPipelineNote(data.annotation?.fallback?"Received fallback annotation":"Received GPT annotation");setBrainResponse(data);setAnnotation(data.annotation);setThinkingStep("visual-update");setTimeout(()=>setVisualReady(true),350);setPipelineNote("Updating board from validated Brain annotation");window.setTimeout(()=>{setThinkingStep("ready");setPipelineNote(data.annotation?.fallback?"Ready with fallback annotation":"Ready with GPT visual annotation")},250);setBrain(p=>({...p,engine:data.engine?.fallback?"fallback":"active",gpt:data.annotation?.fallback?"fallback":"active",latency:Math.round(performance.now()-start),source:data.annotation?.fallback?"Brain fallback":"GPT visual Brain",note:data.pipeline?.gpt||data.annotation?.reason||""}));return data}catch(e){setVisualReady(true);setThinkingStep("error");setPipelineNote(e instanceof Error?e.message:"Brain endpoint failed");setBrain(p=>({...p,engine:"error",gpt:"error",source:"Brain error",note:e instanceof Error?e.message:"Brain failed"}));return null}}
  function selectRepertoire(id:string){setSelectedRepertoireId(id);setFen(new Chess().fen());setSelectedSquare(null);setFeedback("Opening loaded. Play the restricted training move.");setLastMove(null);setLastMoveSan("");setMoveHistory([]);setShowAnswer(false);setTrainingMode("restricted");setBookComplete(false);setOpponentCue(null);setAnnotation(blankAnnotation());setActiveBoardView("plan");setActiveTab("train")}
  function resetBoard(){setFen(new Chess().fen());setSelectedSquare(null);setFeedback("Restarted. Find the first training move.");setLastMove(null);setLastMoveSan("");setMoveHistory([]);setShowAnswer(false);setTrainingMode("restricted");setBookComplete(false);setOpponentCue(null);setAnnotation(blankAnnotation());setActiveTab("train")}
  async function playOpponentMove(forceContinuation=false){const current=new Chess(fen);const mode=forceContinuation?"continuation":trainingMode;setBrain(p=>({...p,source:"opponent thinking",book:mode==="restricted"&&opponentBookOptions.length?"active":"complete",lichess:"loading"}));await new Promise(r=>setTimeout(r,700));let chosen:{san:string;uci:string;fen:string}|null=null;let source="";if(mode==="restricted"&&opponentBookOptions.length){const explorer=await loadExplorer(current.fen());const valid=opponentBookOptions.map(book=>{const match=explorer.find(m=>m.uci===book.uci);return{...book,weight:match?.total??1,pct:match?.pct??0}});const weighted=pickWeighted(valid);chosen={san:weighted.san,uci:weighted.uci,fen:weighted.resultingFen};source=weighted.pct?`Lichess-weighted opening branch (${weighted.pct}%)`:"Saved opening branch"}else{if(mode==="restricted"&&!opponentBookOptions.length){setTrainingMode("continuation");setBookComplete(false);setFeedback("Opening book ended. Blundr switched into continuation mode automatically.");}const explorer=await loadExplorer(current.fen());const playable=explorer.map(m=>{const a=applyUci(current.fen(),m.uci);return a?{...a,weight:m.total,pct:m.pct}:null}).filter(Boolean) as Array<{san:string;uci:string;fen:string;weight:number;pct:number}>;if(playable.length){const pick=pickWeighted(playable);chosen=pick;source=`Lichess continuation (${pick.pct}%)`}else{const data=await runBrain("bot_select",{skipGpt:true});const top=data?.engine?.pvs?.[0];const a=top?applyUci(current.fen(),top.uci):null;if(a){chosen=a;source=`Engine continuation (${rating.target})`}}}if(!chosen){const legal=current.moves({verbose:true}) as any[];if(!legal.length)return;const move=legal[0];current.move({from:move.from,to:move.to,promotion:move.promotion??"q"});chosen={san:move.san,uci:moveToUci(move),fen:current.fen()};source="Emergency legal fallback"}setFen(chosen.fen);setLastMove(chosen.uci);setLastMoveSan(chosen.san);setMoveHistory(prev=>[...prev,chosen.san]);setSelectedSquare(null);setShowAnswer(false);setOpponentCue({expiresAt:Date.now()+2500,title:`Opponent: ${chosen.san}`,message:"Brief opponent cue. Your selected user-side view stays visible after this fades.",lines:[{from:chosen.uci.slice(0,2),to:chosen.uci.slice(2,4),kind:"opponent",label:chosen.san,pathStyle:isKnightGeometry(chosen.uci.slice(0,2),chosen.uci.slice(2,4))?"knight-l":"auto"}],cues:[{square:chosen.uci.slice(2,4),kind:"opponent"}]});setFeedback(`Opponent played ${chosen.san}. Source: ${source}.`);setBrain(p=>({...p,source,book:mode==="restricted"&&opponentBookOptions.length?"active":"complete",lichess:source.includes("Lichess")?"active":p.lichess}))}
  ...
  async function attemptMove(from:string,to:string){const current=new Chess(fen);const beforeFen=fen;const currentKey=normalizeFen(current.fen());let legal:any=null;try{legal=current.move({from,to,promotion:"q"})}catch{}setSelectedSquare(null);if(!legal){setFeedback("Illegal move. Try another move.");return}const playedUci=moveToUci(legal);const noSavedUserMove=trainingMode==="restricted"&&expectedUserOptions.length===0;if(trainingMode==="restricted"&&!noSavedUserMove){const correct=expectedUserOptions.some(m=>m.uci===playedUci);if(!correct){const expected=expectedUserOptions[0]?.san??"No saved move";logMistake(beforeFen,expected,legal.san);setShowAnswer(true);setFeedback(`Not quite. ${legal.san} is legal, but this drill expects ${expected}. The position was saved for review.`);await runBrain("wrong_move",{attemptedMoveSan:legal.san,attemptedMoveUci:playedUci});return}}if(noSavedUserMove){setTrainingMode("continuation");setBookComplete(false);setFeedback(`Opening book ended. Accepted ${legal.san} and switched to continuation mode.`)}setFen(current.fen());setLastMove(playedUci);setLastMoveSan(legal.san);setMoveHistory(prev=>[...prev,legal.san]);setOpponentCue(null);setShowAnswer(false);if(!noSavedUserMove)setFeedback(trainingMode==="restricted"?`Correct: ${legal.san}.`:`Played ${legal.san}. Move will be evaluated.`);setProgress(prev=>{const next={...prev.mistakes};if(reviewingFen&&next[reviewingFen]){if(next[reviewingFen].count<=1)delete next[reviewingFen];else next[reviewingFen]={...next[reviewingFen],count:next[reviewingFen].count-1}}return{...prev,attempts:prev.attempts+1,correct:prev.correct+1,streak:prev.streak+1,trainedPositions:{...prev.trainedPositions,[currentKey]:true},mistakes:next}});setReviewingFen(null)}
  ...
  const squareStyles:Record<string,CSSProperties>={};if(lastMove&&lastMove.length>=4){squareStyles[lastMove.slice(0,2)]={boxShadow:"inset 0 0 0 999px rgba(255,255,255,.09), inset 0 0 12px rgba(255,255,255,.32)"};squareStyles[lastMove.slice(2,4)]={boxShadow:"inset 0 0 0 999px rgba(255,255,255,.12), inset 0 0 14px rgba(255,255,255,.38)"}}if(activeBoard){for(const cue of currentView.cues.slice(0,3)){const ring=cue.kind==="support"?"rgba(82,214,171,.55)":cue.kind==="danger"?"rgba(255,122,89,.62)":"rgba(94,126,255,.62)";squareStyles[cue.square]={...squareStyles[cue.square],boxShadow:`inset 0 0 0 2px ${ring}`}}if(opponentCue)for(const cue of opponentCue.cues){squareStyles[cue.square]={...squareStyles[cue.square],boxShadow:"inset 0 0 0 2px rgba(180,120,255,.55)"}}}if(selectedSquare)squareStyles[selectedSquare]={...squareStyles[selectedSquare],boxShadow:"inset 0 0 0 3px rgba(22,101,52,.85), inset 0 0 16px rgba(22,101,52,.28)"};
...
function TapChessboard({game,orientation,selectedSquare,squareStyles,lines,transientLines,visualSettings,onSquareTap,whitePct,evalText}:{game:Chess;orientation:RepertoireColor;selectedSquare:string|null;squareStyles:Record<string,CSSProperties>;lines:ActiveLine[];transientLines:ActiveLine[];visualSettings:TemporalGateSettings;onSquareTap:(s:string)=>void;whitePct:number;evalText:string}){
  ...
  return <div className="mx-auto flex w-full max-w-[450px] items-stretch gap-2">
    ...
    <div className="flex-1 rounded-[28px] bg-white p-3 shadow-xl shadow-stone-300/40 ring-1 ring-stone-200">
      <div className="relative aspect-square w-full overflow-hidden rounded-[18px] border border-stone-300 bg-stone-200">
        <BoardLines lines={lines} centerFor={centerFor} transient={false} settings={visualSettings}/>
        <BoardLines lines={transientLines} centerFor={centerFor} transient settings={visualSettings}/>
...
function TemporalGateControls({settings,setSettings}:{settings:TemporalGateSettings;setSettings:(settings:TemporalGateSettings)=>void}){
  const item=(key:keyof TemporalGateSettings,label:string)=><button type="button" onClick={()=>setSettings({...settings,[key]:!settings[key]})} className={classNames("rounded-full px-3 py-2 text-[11px] font-black",settings[key]?"bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200":"bg-stone-100 text-stone-500 ring-1 ring-stone-200")}>{label} {settings[key]?"ON":"OFF"}</button>;
  return <div className="mb-3 rounded-2xl bg-stone-50 p-2 ring-1 ring-stone-200"><div className="mb-2 px-1 text-[10px] font-black uppercase tracking-wide text-stone-500">Temporal Gate Display</div><div className="grid grid-cols-2 gap-2 sm:grid-cols-5">{item("motion","Motion")}{item("labels","Labels")}{item("gateText","Gate Text")}{item("arrows","Arrows")}{item("autoMotifs","Motifs")}</div></div>
}
...
function shouldUseKnightL(l:ActiveLine){return isKnightGeometry(l.from,l.to)||l.pathStyle==="knight-l"}
function temporalGateColor(line:ActiveLine,transient:boolean){
  if(transient||line.kind==="opponent")return {primary:"#b884ff",secondary:"#d2b0ff",danger:"#f0e5ff",soft:"rgba(184,132,255,.14)"};
  if(line.kind==="defense")return {primary:"#21b8a6",secondary:"#84e8dd",danger:"#d8faf4",soft:"rgba(33,184,166,.14)"};
  if(line.kind==="attack")return {primary:"#ff7a59",secondary:"#ffc26a",danger:"#ffe3b0",soft:"rgba(255,122,89,.14)"};
  return {primary:"#5e7eff",secondary:"#9cb7ff",danger:"#dce6ff",soft:"rgba(94,126,255,.14)"};
}
function temporalGateToken(line:ActiveLine,transient:boolean){if(transient||line.kind==="opponent")return "THEM";if(line.motif==="fork")return "FORK";if(line.motif==="pin")return "PIN";if(line.motif==="xray")return "X-RAY";if(line.motif==="hanging")return "LOOSE";if(line.timing==="not-yet"||line.gateState==="locked")return "WAIT";if(line.timing==="prepare"||line.gateState==="unlocking")return "PREP";if(line.kind==="attack")return "HIT";if(line.kind==="defense")return "HOLD";return "NOW"}
function temporalGateDash(line:ActiveLine){if(line.gateState==="locked"||line.timing==="not-yet")return "1.1 1.25";if(line.gateState==="unlocking"||line.timing==="prepare")return "2.2 1.15";return "3.6 .85"}
function lPath(f:{x:number;y:number},t:{x:number;y:number},rankFirst=false){const c=rankFirst?{x:f.x,y:t.y}:{x:t.x,y:f.y};return {d:`M ${f.x} ${f.y} L ${c.x} ${c.y} L ${t.x} ${t.y}`,points:`${f.x},${f.y} ${c.x},${c.y} ${t.x},${t.y}`,corner:c,mid:{x:(f.x+c.x+t.x)/3,y:(f.y+c.y+t.y)/3}}}

function BoardLines({lines,centerFor,transient,settings}:{lines:ActiveLine[];centerFor:(s:string)=>{x:number;y:number};transient:boolean;settings:TemporalGateSettings}){
  const visible=lines.filter(l=>isValidSquare(l.from)&&isValidSquare(l.to)).slice(0,transient?1:settings.autoMotifs?3:2);
  if(!visible.length)return null;
  const motionClass=settings.motion?"blundr-temporal-motion":"";
  return <svg className="pointer-events-none absolute inset-0 z-20 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none">
    <defs>{visible.map((l,i)=>{const c=temporalGateColor(l,transient);return <linearGradient key={i} id={`tg-${transient?"t":"p"}-${i}`} x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor={c.primary} stopOpacity=".92"/><stop offset="100%" stopColor={c.secondary} stopOpacity=".92"/></linearGradient>})}</defs>
    {visible.map((l,i)=>{
      const f=centerFor(l.from),t=centerFor(l.to);
      const c=temporalGateColor(l,transient);
      const knight=shouldUseKnightL(l);
      const rankFirst=/rank/i.test(String((l as any).knightCorner??""));
      let path="";
      let points="";
      let mid={x:(f.x+t.x)/2,y:(f.y+t.y)/2};
      if(knight){
        const built=lPath(f,t,rankFirst);
        path=built.d;
        points=built.points;
        mid=built.mid;
      }else{
        const curve=Math.max(-2.2,Math.min(2.2,(Math.abs(t.x-f.x)+Math.abs(t.y-f.y))*.016));
        const cx=(f.x+t.x)/2+curve,cy=(f.y+t.y)/2-curve;
        path=`M ${f.x} ${f.y} Q ${cx} ${cy} ${t.x} ${t.y}`;
        mid={x:cx,y:cy};
      }
      const token=temporalGateToken(l,transient);
      const dash=temporalGateDash(l);
      const key=`${l.from}-${l.to}-${i}`;
      const sourceOuter=3.15, sourceInner=1.32, sourceAura=4.0;
      const targetOuter=4.7, targetInner=2.15, targetAura=6.1;
      const railStroke=`url(#tg-${transient?"t":"p"}-${i})`;
      return <g key={key} className={classNames("blundr-boardlab-temporal-gate",motionClass,transient?"blundr-opponent-line":"")} style={{["--tg-primary" as any]:c.primary,["--tg-secondary" as any]:c.secondary,["--tg-danger" as any]:c.danger}}>
        {knight?<>
          <polyline points={points} fill="none" stroke={c.soft} strokeWidth={transient?"2.4":"2.15"} strokeLinecap="round" strokeLinejoin="round" opacity=".78"/>
          <polyline points={points} fill="none" stroke={railStroke} strokeWidth={transient?"1.2":"1.02"} strokeLinecap="round" strokeLinejoin="round" strokeDasharray={dash}/>
        </>:<>
          <path d={path} fill="none" stroke={c.soft} strokeWidth={transient?"2.4":"2.15"} strokeLinecap="round" strokeLinejoin="round" opacity=".78"/>
          <path d={path} fill="none" stroke={railStroke} strokeWidth={transient?"1.2":"1.02"} strokeLinecap="round" strokeLinejoin="round" strokeDasharray={dash}/>
        </>}
        <g className={classNames("blundr-boardlab-lock-step",settings.motion?"blundr-boardlab-lock-step-motion":"")}>
          <circle cx={f.x} cy={f.y} r={sourceAura} fill={c.primary} opacity=".06"/>
          <circle className={settings.motion?"blundr-boardlab-lock-gate":""} cx={f.x} cy={f.y} r={sourceOuter} fill="none" stroke={c.primary} strokeWidth=".56" strokeDasharray="1.9 .95" opacity=".92"/>
          <circle cx={f.x} cy={f.y} r={sourceInner} fill={c.primary} opacity=".2"/>
        </g>
        <g className={classNames("blundr-boardlab-lock-step",settings.motion?"blundr-boardlab-lock-step-motion":"")}>
          <circle cx={t.x} cy={t.y} r={targetAura} fill={c.primary} opacity=".08"/>
          <circle className={settings.motion?"blundr-boardlab-lock-gate":""} cx={t.x} cy={t.y} r={targetOuter} fill="none" stroke={c.primary} strokeWidth=".72" strokeDasharray="2.5 1.1" opacity=".98"/>
          <circle cx={t.x} cy={t.y} r={targetInner} fill={c.primary} opacity=".2"/>
          <circle cx={t.x} cy={t.y} r={targetOuter*.68} fill="none" stroke={c.secondary} strokeWidth=".34" opacity=".55"/>
          {settings.gateText?<text x={t.x} y={t.y-6.1} textAnchor="middle" className="blundr-temporal-token" fill={c.primary}>{token}</text>:null}
        </g>
        {settings.arrows?<path d={`M ${t.x-1.55} ${t.y} L ${t.x-3.2} ${t.y-1.1} M ${t.x-1.55} ${t.y} L ${t.x-3.2} ${t.y+1.1}`} fill="none" stroke={c.primary} strokeWidth=".52" strokeLinecap="round" strokeOpacity=".9"/>:null}
        {settings.labels&&l.label?<text x={mid.x} y={mid.y-4.8} textAnchor="middle" className="blundr-temporal-label" fill={c.primary}>{l.label}</text>:null}
      </g>
    })}
  </svg>
}
