...
function BlundrApp({ initialTab = "home", initialOpeningId = null }: { initialTab?: Tab; initialOpeningId?: string | null }){
  ...
  const initialSelectedRepertoireId=useMemo(()=>{
    const canonicalInitialOpeningId=resolveStage2CanonicalOpeningId(initialOpeningId);
    return canonicalInitialOpeningId??initialOpeningId??runtimeOpeningSelection.selectedOpeningId;
  ...
  const [selectedRuntimeTrainingLineSelection,setSelectedRuntimeTrainingLineSelection]=useState<RuntimeWeightedTrainingLineSelection | null>(()=>buildRuntimeTrainingLineSelection(initialSelectedRepertoireId,[],runtimeTrainingSessionId));
  ...
  useEffect(()=>{
    if(!PROJECTIVE_TACTICS_ENABLED)clearProjectiveTacticOverlay("feature_disabled");
    return()=>clearProjectiveTacticOverlay("unmount");
  },[]);
  useEffect(()=>{
    if(shouldClearProjectiveTacticsOnViewMode(trainerView))clearProjectiveTacticOverlay("view_mode_switch");
  },[trainerView]);
  useEffect(()=>{
  ...
  function showProjectiveTacticOverlay(visuals:ProjectiveTacticVisual[]){
    ...
    },Math.max(0,DEFAULT_PROJECTIVE_TACTIC_DURATION_MS-DEFAULT_PROJECTIVE_TACTIC_FADE_MS));
  ...
  function maybeShowProjectiveTacticsAfterMove(input:{nextFen:string;lastMoveUci:string;movedColor:ChessColor}){
  ...
  function handleTrainerViewChange(nextTrainerView:TrainerView){
    ...
    if(shouldClearProjectiveTacticsOnViewMode(nextTrainerView))clearProjectiveTacticOverlay("view_mode_switch");
  ...
  if(!instructionTarget?.uci && !selectedSquare && selectedLegalMoves.length===0){
    ...
    const allowedSquares=buildTrainingBoardVisibilitySquares({
      instructionTargetFrom: suppressPlainPreTargetHighlights ? null : instructionTarget?.from ?? null,
      instructionTargetTo: suppressPlainPreTargetHighlights ? null : instructionTarget?.to ?? null,
      selectedSquare,
  ...
  if (isActiveTab(activeTab, "repertoire")) {
    return <RepertoireProgressPanel embedded onTrainOpening={(openingId) => selectRepertoire(openingId, { allowLocked: false })} />;
  }
  return <main className="blundr-page-bg min-h-screen text-stone-950"><div className="mx-auto flex min-h-screen max-w-md flex-col px-4 pb-24 pt-5">
    {isActiveTab(activeTab, "home")&&<section className="space-y-5"><header className="rounded-[2rem] border border-stone-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-xs font-black uppercase tracking-[0.18em] text-green-700">Blundr</div><h1 className="mt-2 text-3xl font-black tracking-tight text-stone-950">What should I do today?</h1><p className="mt-2 text-sm leading-6 text-stone-600">Learn it. Play it. Keep it.</p></div><ProfileSettingsIcon /></div><div className="mt-4 grid gap-2"><button onClick={()=>router.push("/train")} className="inline-flex w-full items-center justify-between rounded-2xl bg-green-700 px-4 py-3 text-sm font-black text-white shadow-sm"><span>Continue training</span><ChevronRight size={18}/></button><div className="grid grid-cols-3 gap-2"><Link href="/daily" className="rounded-2xl bg-stone-100 px-3 py-3 text-center text-xs font-black text-stone-700">Daily</Link><Link href="/profile" className="rounded-2xl bg-stone-100 px-3 py-3 text-center text-xs font-black text-stone-700">Profile</Link><Link href="/profile" className="rounded-2xl bg-stone-100 px-3 py-3 text-center text-xs font-black text-stone-700">Account</Link></div></div></header>{shouldShowOnboarding(onboardingProfile)&&<Link href="/onboarding" className="flex items-center justify-between rounded-3xl border border-green-200 bg-green-50 p-4 text-left shadow-sm"><div><div className="text-sm font-black uppercase tracking-[0.18em] text-green-700">Finish setup</div><p className="mt-2 text-sm leading-6 text-stone-700">Choose a starter pack, daily goals, and your first training destination.</p></div><ChevronRight className="text-green-700" size={20}/></Link>}<TempoDailyBlundrCard/><DailyRingsCard repertoireProgress={repertoireProgress} refreshKey={repertoireProgress.updatedAt} completionResult={latestCompletionResult} onStartTraining={()=>router.push("/train")}/><section className="rounded-3xl border border-stone-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-[0.18em] text-green-700">Repertoire</div><h2 className="mt-1 text-lg font-black tracking-tight text-stone-950">Build a repertoire you actually remember.</h2><p className="mt-2 text-sm leading-6 text-stone-600">Unlock openings with points earned from consistent training.</p></div><Link href="/repertoire" className="rounded-2xl bg-white px-4 py-3 text-sm font-black text-green-700 ring-1 ring-green-200">View</Link></div><div className="mt-4 grid gap-3"><RepertoirePointsSummary progress={repertoireProgress} compact/><RepertoireUnlockProgress progress={repertoireProgress}/></div></section><div className="grid grid-cols-3 gap-3"><MetricCard label="Accuracy" value={`${accuracy}%`} sub="all time" icon={<Trophy size={19}/>}/><MetricCard label="Streak" value={String(progress.streak)} sub="correct" icon={<Flame size={19}/>}/><MetricCard label="Review" value={String(mistakes.length)} sub="saved" icon={<XCircle size={19}/>} warning/></div><section className="space-y-3"><div><div className="text-xs font-black uppercase tracking-[0.18em] text-green-700">Openings</div><h2 className="mt-1 text-lg font-black tracking-tight text-stone-950">Quick start</h2></div>{repertoires.slice(0,3).map(r=>{const positions=getRepertoirePositionCount(r);const isLocked=repertoireProgress.lockedOpeningIds.includes(resolveStage2CanonicalOpeningId(r.id)??r.id);return <button key={r.id} onClick={()=>selectRepertoire(r.id)} disabled={isLocked} className={classNames("flex w-full items-center gap-3 rounded-3xl border bg-white p-3 text-left shadow-sm",isLocked?"cursor-not-allowed border-stone-200 opacity-70":"border-stone-200")}><div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-stone-100 text-2xl">{r.color==="white"?"♙":"♟"}</div><div className="min-w-0 flex-1"><div className="font-bold">{r.name}</div><div className="text-sm text-stone-500">{r.lines.length} lines • {positions} positions</div>{isLocked?<div className="mt-2 inline-flex rounded-full bg-stone-100 px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-stone-500">Locked</div>:null}</div><ChevronRight className="text-stone-400" size={20}/></button>})}</section></section>}
    ...
    {isActiveTab(activeTab, "review")&&<section className="space-y-5"><ReviewHub embedded homeHref="/" settingsHref="/settings"/></section>}
...
function BottomNav({ activeTab }: { activeTab: string; setActiveTab: (tab: Tab) => void }) {
  return <BlundrBottomNav activeTab={activeTab as "home" | "train" | "review" | "progress" | "repertoire"} />;
}
