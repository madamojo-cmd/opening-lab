...
import {
  mapEngineLinesToStockfishTopMoves,
  rateContinuationUserMove,
  validateContinuationSuggestionAgainstStockfish,
} from "@/lib/blundr/engine/stockfishContinuationValidation";
import type {
  ContinuationUserMoveRatingResult,
  MoveStrengthRating,
} from "@/lib/blundr/engine/stockfishEvaluationTypes";
import {
...
import { Figma5303HomeScreen } from "@/components/figma-source/5303-dashboard-daily-review/Figma5303DashboardDailyReview";
...
} from "@/lib/blundr/opponent/opponentVariationMemory";
...
import {
  buildCoachUtteranceRecordKey,
  loadCoachUtteranceMemory,
  readCoachUtteranceMemoryMeta,
  recordCoachUtterance,
} from "@/lib/blundr/coach/coachUtteranceMemory";
...
import {
  ...
  isDebugLeakText,
} from "@/lib/blundr/coachBrain/coachExplanationPipeline";
...
} from "@/lib/blundr/presentation/phaseActionGating";
...
import {
  adaptVisibleSurfaceToBoardVisuals,
  adaptVisibleSurfaceToCoachUi,
} from "@/lib/blundr/presentation/uiSurfaceAdapter";
...
import {
  getPendingPromotionFromAttempt,
  resolvePromotionAuthority,
  type PendingPromotion,
  type PromotionPiece,
} from "@/lib/blundr/runtime/promotionAuthority";
import {
  classifyContinuationRuntimeState,
  type ContinuationRuntimeStatus,
} from "@/lib/blundr/runtime/continuationRuntimeState";
...
import type {
  ...
  MaiaProviderStatus,
  MaiaSkillLevel,
} from "@/lib/blundr/maia/maiaTypes";
import {
...
import {
  ...
  type MaiaTimelineEvent,
} from "@/lib/blundr/debug/maiaTimeline";
...
} from "@/lib/blundr/daily-rings/trainingCompletionEligibility";
import {
...
const BlundrDiagnosticsPanel = dynamic(
  () =>
    import("@/components/debug/BlundrDiagnosticsPanel").then(
      (mod) => mod.BlundrDiagnosticsPanel,
    ),
...
function isActiveTab(activeTab: Tab, expected: Tab): boolean {
  return activeTab === expected;
}
...
type BrainAnnotation = {
  source: string;
  fallback: boolean;
  selectedView: ActiveBoardView;
  headline: string;
...
type BrainResponse = {
  ...
  engine: { source: string; fallback: boolean; pvs: EngineLine[] };
  annotation: BrainAnnotation;
  candidates?: any;
  debug?: any;
};
type VisualArrowRole =
  | "move"
  | "pressure"
  | "defense"
  | "future"
...
type VisualSquareRole =
  ...
  | "king_safety"
  | string;
type VisualModelArrow = {
  from: string;
  to: string;
  role?: VisualArrowRole;
  kind?: LineKind;
  label?: string;
  reason?: string;
};
type VisualModelSquare = {
  square: string;
  role?: VisualSquareRole;
  kind?: SquareCue["kind"];
  reason?: string;
};
type VisualModelContext = {
  headline: string;
  body: string;
  next: string;
...
type VisualModelOutput = Partial<BrainAnnotation> & {
  arrows?: VisualModelArrow[];
  squares?: VisualModelSquare[];
  animation?: string;
  animationPackage?: { name: string; intensity?: number };
  context?: VisualModelContext;
  suppress?: string[];
  debug?: any;
};
...
type OpponentVariationDebug = {
  ...
  candidateOpponentBranches: Array<{
    branchKey: string;
    uci: string;
    san?: string;
    baseWeight: number;
...
function toAppRuntimeRepertoire(
  repertoire: RuntimeTrainableRepertoire,
): Repertoire {
  return {
    ...repertoire,
...
function buildDiagnosticsPlaceholder(
  input: DiagnosticsPlaceholderInput,
): TrainerDebugSnapshot {
  return {
    ...
    frame: {
      ...
      debugSnapshotStatus: input.debugEnabled
        ? "loading_debug_collector"
        : "debug_disabled",
    },
    board: {},
...
type CapturedSummary = {
  ...
  materialAdvantage: { side: ChessColor | null; value: number };
};
...
const INITIAL_COUNTS: Record<ChessColor, Record<string, number>> = {
...
const STAGE2_RUNTIME_TRAINING_LINE_MEMORY_KEY =
  "blundr-stage2-runtime-training-line-memory-v1";
...
const OPENINGS: Repertoire[] = [
  {
    ...
    name: "Italian Game",
    color: "white",
    description: "Develop fast, pressure f7, castle, and prepare c3–d4.",
    lines: [
      [
        "e4",
        "e5",
  ...
  },
  {
    ...
    name: "Ruy Lopez",
    color: "white",
    description: "Pressure e5, build with c3/Re1, and prepare d4.",
    lines: [
      [
        "e4",
        "e5",
  ...
  },
  {
    ...
    name: "Queen's Gambit",
    color: "white",
    description: "Use central tension to pressure d5 and develop cleanly.",
    lines: [
      [
        "d4",
        "d5",
  ...
  },
  {
    ...
    name: "London System",
    color: "white",
    description: "Build a stable setup, support d4, and look for e4/Ne5 ideas.",
    lines: [
      [
        "d4",
        "Nf6",
  ...
  },
  {
    ...
    color: "black",
    description:
      "Build a resilient center and challenge e4/d4 without weakening the king.",
    lines: [
      [
        "e4",
        "c6",
      ...
      ["e4", "c6", "d4", "d5", "exd5", "cxd5", "Nf3", "Nf6", "Bd3", "Nc6"],
    ],
  },
  {
    id: "sicilian-black",
    name: "Sicilian as Black",
    color: "black",
    description: "Use asymmetry to fight for d4 and create active counterplay.",
    lines: [
      [
        "e4",
        "c5",
  ...
  },
  {
    ...
    color: "black",
    description:
      "Challenge the white center with ...d5 and pressure the pawn chain.",
    lines: [
      [
        "e4",
        "e6",
  ...
  },
  {
    ...
    name: "King's Indian as Black",
    color: "black",
    description:
      "Allow White's center, then counter with ...e5 or ...c5 and kingside activity.",
    lines: [
      [
        "d4",
        "Nf6",
...
function buildRuntimeFrameKey(input: {
  fen: string;
  trainerPhase: string;
  trainerView: TrainerView;
  trainingMode: TrainingMode;
  ...
  return `${normalizeFen(input.fen)}|${input.trainerPhase}|${input.trainerView}|${input.trainingMode}|${input.isUserTurn ? "user" : "opp"}|${input.instructionTargetUci ?? "none"}`;
}
...
function buildRuntimePlayKeyBeforeFromSanHistory(
  historySan: string[],
): string | null {
...
function countMatchedRuntimeLinePlies(
  reference: string[],
  current: string[],
): number {
...
function preflightRuntimeTrainingLineSelection(
  selection: RuntimeWeightedTrainingLineSelection | null,
  ...
  for (const summary of selection.lineWeightsSummary) {
    const sequence = summary.playKey
      .split(",")
...
function buildRuntimeTrainingLineSelection(
  openingId: string,
  recentLineKeys: string[],
  seed: string,
  repertoire?: Repertoire,
  ratingFilterValue: string = getStage2RatingBandByFilterValue(
    DEFAULT_STAGE2_RATING_BAND_ID,
  ).value,
): RuntimeWeightedTrainingLineSelection | null {
  ...
  const preflightedSelection = preflightRuntimeTrainingLineSelection(
    selection,
    repertoire,
    seed,
  );
...
function isValidSquare(square: string) {
...
function isImmediateReverseOf(
  prevUci: string | null | undefined,
  candidateUci: string,
): boolean {
  if (!prevUci || prevUci.length < 4 || candidateUci.length < 4) return false;
  const pFrom = prevUci.slice(0, 2),
    pTo = prevUci.slice(2, 4);
  const cFrom = candidateUci.slice(0, 2),
    cTo = candidateUci.slice(2, 4);
  return cFrom === pTo && cTo === pFrom;
}
function visualLineKind(role?: string, kind?: LineKind): LineKind {
  if (kind && ["attack", "defense", "plan", "opponent"].includes(kind))
    return kind;
  if (role === "defense" || role === "retreat" || role === "castle")
    return "defense";
  if (
    role === "pressure" ||
    role === "threat" ||
...
function visualCueKind(
  role?: string,
  kind?: SquareCue["kind"],
): SquareCue["kind"] {
  if (
    kind &&
    ["origin", "target", "support", "danger", "opponent"].includes(kind)
  )
    return kind;
  if (role === "source") return "origin";
  if (role === "defense" || role === "king_safety") return "support";
  if (role === "weakness" || role === "danger" || role === "soft_target")
    return "danger";
  return "target";
}
function visualAnimationClass(name?: string) {
  const known = new Set([
  ...
  return known.has(name ?? "")
    ? `blundr-anim-${name}`
    : "blundr-anim-quiet-development-glow";
}
function getPiece(game: Chess, square: string) {
  return game.get(square as any);
}
function isOwnPiece(game: Chess, square: string, color: ChessColor) {
  const p = getPiece(game, square);
  return Boolean(p && p.color === color);
}
...
function ratingPreset(value: string) {
  return getStage2RatingBandByFilterValue(value);
}
function buildTree(rep: Repertoire) {
  ...
  for (const line of rep.lines) {
    ...
    for (const san of line) {
      ...
      try {
        ...
        const ex = tree[key] ?? [];
        tree[key] = ex.some((x) => x.uci === cont.uci) ? ex : [...ex, cont];
      } catch {
        break;
      }
    }
...
function repertoireLineInputs(rep: Repertoire): RepertoireLineInput[] {
  return rep.lines.map((line, index) => ({
    openingId: rep.id,
    lineId: `${rep.id}:${index}`,
    openingName: rep.name,
    sideToTrain: rep.color,
    movesSan: line,
  }));
}
function getRepertoirePositionCount(rep: Repertoire) {
  const availability = getStage2OpeningAvailability(rep.id);
  if (availability) return availability.runtimeNodeCount;
  return rep.lines.reduce((sum, line) => sum + Math.max(1, line.length), 0);
}
...
function parseExplorerMoves(payload: any): ExplorerMove[] {
  const moves = Array.isArray(payload?.moves) ? payload.moves : [];
  const denom =
    moves.reduce(
      (s: number, m: any) =>
        s + (m.white ?? 0) + (m.draws ?? 0) + (m.black ?? 0),
      0,
    ) || 1;
  return moves
    .map((m: any) => {
      ...
      return {
        ...
        san: m.san,
        total,
        pct: Math.round((total / denom) * 100),
        averageRating: m.averageRating,
      };
    })
    .filter((m: ExplorerMove) => m.uci && m.total > 0);
}
...
function blankAnnotation(): BrainAnnotation {
  return {
    ...
    selectedView: "plan",
    headline: "Ready",
    mainExplanation: "Make a move or tap Reveal Next Move.",
    visualExplanation:
      "The board can now show a fast local cue immediately while Brain refines the coaching text.",
    planExplanation: "Restricted mode keeps you inside the selected opening.",
    nextPlan: "Play the highlighted training move when available.",
    keySquares: [],
    planArrows: [],
    attack: {
      title: "Your attack",
      message: "Fast local visuals will appear as soon as training starts.",
      lines: [],
      cues: [],
    },
    defense: {
...
function isKnightGeometry(from: string, to: string) {
  if (!isValidSquare(from) || !isValidSquare(to)) return false;
  const df = Math.abs(FILE_TO_INDEX[from[0]] - FILE_TO_INDEX[to[0]]);
  const dr = Math.abs(Number(from[1]) - Number(to[1]));
  return (df === 1 && dr === 2) || (df === 2 && dr === 1);
}
function lineFromContinuation(
  move: Continuation,
  kind: LineKind = "plan",
): ActiveLine {
  return {
...
function lineFromVerboseMove(move: any, kind: LineKind = "plan"): ActiveLine {
  return {
    from: move.from,
    to: move.to,
    kind,
...
function lineFromEngine(
  fen: string,
  line: EngineLine,
  kind: LineKind = "plan",
): ActiveLine | null {
  const applied = applyUci(fen, line.uci);
  if (!applied) return null;
  return {
    from: line.uci.slice(0, 2),
    to: line.uci.slice(2, 4),
    kind,
    label: applied.san || line.san || line.uci,
  };
}
function engineAnnotationFromLine({
  fen,
  line,
  openingName,
}: {
  ...
  if (!visual)
    return {
      ...base,
      source: "engine pending",
      fallback: true,
      headline: "Checking continuation",
      mainExplanation:
        "Blundr Brain is checking the continuation before highlighting a move.",
      visualExplanation:
        "No random legal fallback is shown as a recommendation.",
      planExplanation: "Wait for the teaching cue.",
      nextPlan: "Teaching cue pending.",
      plan: {
        title: "Checking continuation",
        message: "Blundr Brain is checking the continuation.",
        lines: [],
        cues: [],
      },
      confidence: "engine-pending",
    };
  const cues: SquareCue[] = [
    { square: visual.from, kind: "origin" },
    { square: visual.to, kind: "target" },
  ];
  ...
  return {
    ...
    fallback: false,
    selectedView: "plan",
    headline: `Suggested continuation: ${move}`,
    mainExplanation: `Manual analysis currently prefers ${move}.`,
    visualExplanation:
      "The highlighted move is analysis-backed, not a placeholder legal move.",
    planExplanation: `Use ${move} as the current continuation while explanation text stays concise.`,
    nextPlan: `Play ${move}.`,
    keySquares: [visual.to],
    planArrows: [visual],
    attack: {
      title: "Active continuation",
      message: `${move} is the current continuation.`,
      lines: [{ ...visual, kind: "attack" }],
      cues,
    },
    defense: {
      title: "Safety check",
      message: "The recommendation is validated by local analysis.",
      lines: [],
      cues: [{ square: visual.from, kind: "support" }],
    },
    plan: {
      title: `${openingName}: continuation plan`,
      message: `Analysis-backed continuation: ${move}.`,
      lines: [visual],
      cues,
    },
    confidence: "analysis",
...
function deriveFastAnnotation({
  ...
  opponentBookOptions: Continuation[];
}): BrainAnnotation {
  ...
  if (local.isGameOver())
    return {
      ...
      fallback: true,
      headline: "Game over",
      mainExplanation: "This line has reached a terminal position.",
      visualExplanation: "No training cue is shown because the game is over.",
      planExplanation: "Restart the opening to train again.",
      nextPlan: "Restart or choose another repertoire.",
      plan: {
        title: "Game over",
        message: "Restart the opening to continue training.",
        lines: [],
        cues: [],
      },
      confidence: "local",
  ...
  if (userTurn && trainingMode === "restricted" && expectedUserOptions.length) {
    const lines = expectedUserOptions
      .slice(0, 2)
      .map((m) => lineFromContinuation(m, "plan"));
    const cues: SquareCue[] = lines.flatMap((l) => [
      { square: l.from, kind: "origin" as const },
      { square: l.to, kind: "target" as const },
    ]);
    const moveText = expectedUserOptions.map((m) => m.san).join(" / ");
    return {
      ...base,
      source: "fast local repertoire",
      fallback: true,
      selectedView: "plan",
      headline: `${openingName}: play ${moveText}`,
      mainExplanation: `The saved repertoire expects ${moveText}.`,
      visualExplanation:
        "The board is showing the source and destination for the current saved training move immediately.",
      planExplanation:
        "Stay inside the restricted opening line by playing the highlighted move.",
      nextPlan: `Play ${moveText}.`,
      keySquares: lines.map((l) => l.to),
      planArrows: lines,
      attack: {
        title: "Current attacking/development idea",
        message: `The immediate training move is ${moveText}. Use this view as a fast cue, not a tactical claim.`,
        lines: lines.map((l) => ({ ...l, kind: "attack" as LineKind })),
        cues,
      },
      defense: {
        title: "Current responsibility",
        message:
          "Restricted mode first checks whether the move belongs to your saved repertoire. Deeper defensive motifs are refined after the local cue appears.",
        lines: [],
        cues: lines.map((l) => ({ square: l.from, kind: "support" as const })),
      },
      plan: {
        title: "Fast local plan",
        message: `Play ${moveText} to continue the saved ${openingName} branch.`,
        lines,
        cues,
      },
      confidence: "local",
  ...
  if (
    ...
    return {
      ...base,
      source: "fast local book complete",
      fallback: true,
      headline: "Book complete",
      mainExplanation: "The saved repertoire branch has ended on your turn.",
      visualExplanation:
        "No move is highlighted because there is no saved restricted-mode continuation.",
      planExplanation: "Choose Train Again or Continue vs Bot.",
      nextPlan:
        "Continue vs Bot to accept normal legal moves from this position.",
      plan: {
        title: "Book complete",
        message: "No saved move remains for this branch.",
        lines: [],
        cues: [],
      },
      confidence: "local",
  ...
  if (
    !userTurn &&
    trainingMode === "restricted" &&
    opponentBookOptions.length
  ) {
    return {
      ...base,
      source: "fast local opponent book",
      fallback: true,
      selectedView: "plan",
      headline: "Opponent selecting move",
      mainExplanation: "The opponent is selecting a supported opening reply.",
      visualExplanation:
        "Opponent candidate branches are not shown on the main board before commit.",
      planExplanation: "After commit, the last opponent move is shown briefly.",
      nextPlan: "Wait for the opponent move to be committed.",
      keySquares: [],
      planArrows: [],
      attack: {
        title: "Opponent selecting",
        message: "Candidate branches stay in debug only until commit.",
        lines: [],
        cues: [],
      },
      defense: {
        title: "Prepare for reply",
        message:
          "Your next teaching cue appears after the opponent move is committed.",
        lines: [],
        cues: [],
      },
      plan: {
        title: "Opponent selecting",
        message: "Waiting for committed opponent move.",
        lines: [],
        cues: [],
      },
      confidence: "local",
  ...
  return {
    ...
    selectedView: "plan",
    headline:
      trainingMode === "continuation" && userTurn
        ? "Checking continuation"
        : "Waiting for opponent",
    mainExplanation:
      trainingMode === "continuation" && userTurn
        ? "Blundr Brain is checking the continuation before showing a cue."
        : "The opponent is selecting a continuation.",
    visualExplanation:
      "No provisional legal-move recommendation is drawn. The next plan visual appears only when it is validated.",
    planExplanation:
      trainingMode === "continuation" && userTurn
        ? "Wait for the teaching cue."
        : "Wait for the opponent move.",
    nextPlan:
      trainingMode === "continuation" && userTurn
        ? "Teaching cue pending."
        : "Wait for the opponent move.",
    attack: {
      title: "Checking",
      message: "Attack cues are withheld until the move is validated.",
      lines: [],
      cues: [],
    },
    defense: {
      title: "Checking",
      message: "Defense cues are withheld until the move is validated.",
      lines: [],
      cues: [],
    },
    plan: {
...
function impactFromEngine(line?: EngineLine) {
  ...
  if (typeof cp !== "number")
    return {
      ...
      pct: 64,
      tone: "bg-green-700",
      note: "Move impact will use Brain endpoint engine output when available.",
    };
  if (cp > 180)
    return {
      label: "Strong",
      pct: 92,
      tone: "bg-green-700",
      note: "Blundr Brain likes this continuation.",
    };
  if (cp > 80)
    return {
      label: "Stable",
  ...
  if (cp > 20)
    return {
      ...
      pct: 58,
      tone: "bg-yellow-500",
      note: "Playable continuation.",
    };
  return {
    label: "Needs care",
    pct: 36,
    tone: "bg-orange-600",
    note: "Look for a more forcing or developing move.",
  };
}
...
function compactText(value: unknown, fallback: string, max = 140) {
  ...
  return text.length > max
    ? `${text.slice(0, Math.max(0, max - 1)).trim()}…`
    : text;
}
...
function computeRenderedCoachQuality(input: {
  ...
  const repeatedStemCount = input.recentRenderedBodies.filter(
    (entry) => normalizeCoachBody(String(entry)) === renderedBodyStem,
  ).length;
  ...
  if (input.runtimeSafeFallbackUsed) {
    ...
    reasons.push(
      input.runtimeSafeFallbackReason === "claim_validation_failed"
  ...
  const pipelineLooksBetter = Boolean(
    renderedVsPipelineMismatch &&
      ...
      (looksGenericCoachTitle(renderedTitle) ||
        looksGenericCoachBody(renderedBody)),
  );
  ...
  const pipelineQualityScore = Number.isFinite(
    Number(input.pipelineQualityScore),
  )
    ? Number(input.pipelineQualityScore)
    : null;
  return {
...
function getRecentInstructionalCoachRecords(
  records: LastCoachRecord[],
  limit = 5,
) {
  return records
    .filter(
      (entry) =>
        entry.trainerPhase === "ready_for_user" &&
        Boolean(entry.instructionTargetUci),
    )
    .slice(-limit);
}

function normalizeCoachEntryKind(input: {
  trainerPhase: string;
  isUserTurn: boolean;
  instructionTargetUci: string | null;
  runtimeCriticalIssues?: string[];
}): CoachSessionLogEntry["entryKind"] {
  if (
    (input.runtimeCriticalIssues ?? []).some((issue) =>
      String(issue).toLowerCase().includes("error"),
    )
  )
    return "error";
  ...
  if (
    input.trainerPhase === "opponent_selecting" ||
    input.trainerPhase === "opponent_replying"
  )
...
function applyThemeProvenance(debug: any) {
  const theme =
    String(
      debug?.selectedTheme ?? debug?.coachQuality?.selectedTheme ?? "",
    ).trim() || null;
  ...
  > = {
    minor_piece_development: {
      layer: "development",
      minScore: 320,
      planTypes: ["development", "opportunity:minor_piece_development"],
      templateHints: ["minor_piece_development"],
    },
    central_pawn_advance: {
      layer: "center",
      minScore: 330,
      planTypes: ["center", "opportunity:central_pawn_advance"],
      templateHints: ["central_pawn_advance"],
    },
    capture_or_recapture: {
      layer: "tactical",
      minScore: 450,
      planTypes: ["tactical", "opportunity:capture_or_recapture"],
      templateHints: ["capture_or_recapture"],
    },
    bishop_activation: {
      layer: "development",
      minScore: 300,
      planTypes: ["development", "opportunity:bishop_activation"],
      templateHints: ["bishop_activation"],
    },
    checkmate: {
      layer: "tactical",
      minScore: 1000,
      planTypes: ["tactical", "opportunity:checkmate"],
      templateHints: ["checkmate"],
    },
    castle_king_safety: {
      layer: "king_safety",
      minScore: 350,
      planTypes: ["king_safety", "opportunity:castle_king_safety"],
      templateHints: ["castle", "king_safety"],
    },
    center_support: {
      layer: "center",
      minScore: 280,
      planTypes: ["center", "opportunity:center_support"],
      templateHints: ["center_support"],
    },
    stable_continuation: {
      layer: "fallback",
      minScore: 100,
      planTypes: ["stable_continuation"],
      templateHints: ["stable_continuation"],
    },
  };
  const rule = map[theme];
  if (!rule) return debug;
  const currentTemplate = String(debug?.selectedTemplateId ?? "");
  const templateMatches = rule.templateHints.some((hint) =>
    currentTemplate.includes(hint),
  );
  const selectedTemplateId = templateMatches
    ? currentTemplate
    : debug?.verifiedFallbackUsed ||
        debug?.candidateCoachFallbackUsed ||
        debug?.coachQuality?.usedFallback
      ? `fallback:${theme}:verified_safe`
      : `live:${theme}:${rule.layer === "tactical" ? "explain_tactic" : rule.layer === "center" ? "explain_center" : rule.layer === "development" ? "explain_development" : "explain_plan"}`;
  const existingPlanTypes = Array.isArray(debug?.recognizedPlanTypes)
    ? debug.recognizedPlanTypes
    : [];
  const selectedPlanType = String(
    debug?.selectedPlanType ?? existingPlanTypes[0] ?? "",
  );
  const selectedOpportunityScore = Math.max(
    Number(debug?.selectedOpportunityScore ?? 0) || 0,
    rule.minScore,
  );
  return {
    ...debug,
    ...
    selectedOpportunityId:
      theme === "stable_continuation" &&
      debug?.selectedOpportunityId &&
      !String(debug.selectedOpportunityId).includes("supported_continuation")
        ? debug.selectedOpportunityId
        : theme,
    selectedOpportunityLayer: rule.layer,
    selectedOpportunityScore,
    selectedPlanType: selectedPlanType || rule.planTypes[0],
    recognizedPlanTypes: existingPlanTypes.length
      ? existingPlanTypes
      : [rule.planTypes[0]],
    selectedTemplateId,
  };
...
function normalizeCoachDebugMetadata(debug: any) {
  ...
  const fallbackByReason = Boolean(
    withProvenance?.fallbackReason || quality?.fallbackReason,
  );
  const fallbackByFlag = Boolean(
    withProvenance?.verifiedFallbackUsed ||
      withProvenance?.candidateCoachFallbackUsed ||
      quality?.usedFallback,
  );
  const source = String(
    withProvenance?.coachDecisionSource ?? quality?.source ?? "",
  ).trim();
  const fallbackBySource = source === "verified_safe_fallback";
  const runtimeSafeFallbackUsed =
    fallbackByFlag || fallbackBySource || fallbackByReason;
  const runtimeSafeFallbackReason =
    String(
      withProvenance?.fallbackReason ?? quality?.fallbackReason ?? "",
    ).trim() || null;
  const normalizedSource = runtimeSafeFallbackUsed
    ? "verified_safe_fallback"
    : source || "live_coach";
  const hasTarget = Boolean(withProvenance?.coachMoveUci);
  const numericScore = Number(withProvenance?.selectedOpportunityScore);
  const selectedOpportunityScore =
  ...
  return {
    ...
    fallbackReason: runtimeSafeFallbackUsed
      ? (runtimeSafeFallbackReason ?? "verified_safe_fallback")
      : null,
    selectedOpportunityScore,
    coachQuality: {
      ...quality,
      ...
      fallbackReason: runtimeSafeFallbackUsed
        ...
        : null,
      selectedTheme:
        withProvenance?.selectedTheme ?? quality?.selectedTheme ?? null,
      qualityScore:
        quality?.qualityScore ?? withProvenance?.qualityScore ?? null,
    },
  };
}
...
function buildUserFacingTargetFallback(input: {
  fen: string;
  target: NonNullable<
    ReturnType<typeof buildCurrentInstructionFrame>["target"]
  >;
  trainingMode: TrainingMode;
  trainerPhase: string;
  openingId?: string | null;
...
function isEngineBestContinuationSource(source?: string | null) {
  ...
  return (
    value === "engine_best" ||
    value === "stockfish_best_move_fallback" ||
    value === "engine_best_move_fallback"
  );
}

function buildEngineBestContinuationCopy(
  target: NonNullable<
    ReturnType<typeof buildCurrentInstructionFrame>["target"]
  >,
) {
...
function detectUnverifiedCoachClaims(input: {
  ...
  const features = new Set<string>(
    [...input.featureClaims, ...input.planClaims].map((x) => String(x)),
  );
  const target = input.target;
  const pieceChecks: [string, string][] = [
    ["bishop", "b"],
    ["knight", "n"],
    ["rook", "r"],
    ["queen", "q"],
  ...
  if (/\bdevelops?\b|\bdevelopment\b/.test(text) && !target?.isDevelopment)
    claims.push("unverified_development_claim");
  if (
    /\bdiagonal\b/.test(text) &&
    !(
      target &&
      (target.pieceType === "b" || target.pieceType === "q") &&
      target.isDiagonalMove
    )
  )
    claims.push("unverified_diagonal_claim");
  if (
    /\bcenter\b|\bcentral\b|center tension/.test(text) &&
    !(
      target?.isCentralPawnAdvance ||
      features.has("center_tension") ||
      features.has("maintain_center_tension") ||
      features.has("central_break_preparation")
    )
  )
    claims.push("unverified_center_tension_claim");
  if (
    /\bpressure\b|\battack\b/.test(text) &&
    !(
      features.has("attacks_square") ||
      features.has("bishop_diagonal_pressure") ||
      features.has("rook_on_open_file")
    )
  )
    claims.push("unverified_pressure_claim");
  if (
    /king safety|castle|shelter/.test(text) &&
    !(
      target?.isKingSafetyMove ||
      features.has("king_safety_urgent") ||
      features.has("castle_and_connect_rooks")
    )
  )
    claims.push("unverified_king_safety_claim");
  if (/\bcaptures?\b|\btakes\b/.test(text) && !target?.isCapture)
    claims.push("unverified_capture_claim");
  if (/\bcheckmate\b|\bmate\b/.test(text) && !target?.isMate)
    claims.push("unverified_mate_claim");
  if (
    /\bcheck\b/.test(text) &&
    !/\bcheckmate\b|\bmate\b/.test(text) &&
    !target?.isCheck
  )
    claims.push("unverified_check_claim");
  if (/\bpromotion\b|=[qrbn]/.test(text) && !target?.isPromotion)
    claims.push("unverified_promotion_claim");
  if (
    /verified move:|pawn from|knight from|bishop from|rook from|queen from|king from|not_exposed_from_module|pipeline|fallback|runtime|candidate source/.test(
      text,
    )
  )
    claims.push("debug_copy_leaked_to_user");
  return {
    verifiedClaims: verified,
    unverifiedClaims: Array.from(new Set(claims)),
  };
}
...
function getMoveQualityUserStatus(
  moveQuality: MoveQualityResult | null,
  pending: boolean,
): "idle" | "checking" | "verified" | "needs_review" | "not_verified" {
...
function getMoveQualityBadgeLabel(input: {
  ...
  if (input.trainerView === "plain" && !input.showAnswer)
    return "Plain View • No hints";
  if (input.shouldValidateTrainingMove && !input.showAnswer) {
    const status = getMoveQualityUserStatus(
      input.moveQuality,
      input.moveQualityPending,
    );
...
function buildPatternCue(input: {
  ...
  if (input.trainerView === "plain" && !input.showAnswer) {
    return {
      ...
      snippet: "Solve the position without hints.",
      status: "plain",
      source: "plain",
    };
  }
  if (
    input.shouldValidateTrainingMove &&
    !input.showAnswer &&
    input.moveQualityPending
  ) {
    return {
      ...
      snippet:
        "Blundr Brain is checking this move before showing a teaching cue.",
      status: "pending",
      source: "pending",
    };
  }
  if (
    input.shouldValidateTrainingMove &&
    !input.showAnswer &&
    input.moveQuality?.status === "rejected"
  ) {
    return {
      title: "Line needs review",
      snippet:
        "Blundr Brain did not validate this saved line, so no teaching cue will be shown.",
      next: undefined,
      source: "suppressed",
      status: "suppressed",
    };
  ...
  if (
    input.shouldValidateTrainingMove &&
    !input.showAnswer &&
    input.moveQuality?.status === "unavailable"
  ) {
    return {
      title: "Move not verified",
      snippet:
        "Blundr could not verify this move, so it will not invent a teaching plan.",
  ...
  if (input.visualModelPending) {
    return {
      title: "Preparing visual cue",
      snippet:
        "Blundr is checking the deterministic visual pattern for this position.",
      status: "pending",
      source: "pending",
    };
  }
  if (input.visualSuppressed) {
    return {
      title: "No verified cue yet",
      snippet: "A recommendation is pending, so Blundr will not invent a plan.",
      next: "Use Reveal only if you want the answer.",
      status: "suppressed",
      source: "suppressed",
    };
  }
  if (input.visualModelOutput?.context) {
    return {
      title: compactText(
        input.visualModelOutput.context.headline,
        "Pattern cue",
        80,
      ),
      snippet: compactText(
        input.visualModelOutput.context.body,
        "Use the highlighted move pattern.",
        140,
      ),
      next:
        compactText(input.visualModelOutput.context.next, "", 80) || undefined,
      status: "ready",
      source: "rule_visual",
  ...
  if (input.visualModelOutput) {
    return {
      ...
      snippet: compactText(
        input.visualModelOutput.mainExplanation ??
          input.visualModelOutput.planExplanation,
        "Use the highlighted move pattern.",
        140,
      ),
  ...
  if (input.showAnswer && input.expectedUserOptions.length) {
    return {
      ...
      snippet: compactText(
        `Play ${input.expectedUserOptions.map((m) => m.san).join(" / ")}.`,
        "Play the saved line move.",
        140,
      ),
  ...
  return {
    ...
    snippet: compactText(
      input.annotation.mainExplanation ?? input.annotation.planExplanation,
      "Use the local visual cue.",
      140,
    ),
...
function pieceGlyph(color: ChessColor, type: string, style: PieceStyle) {
  return renderBoardPieceGlyph(
    color === "w" ? "w" : "b",
    type,
    style === "letters" ? "letters" : style === "neo" ? "neo" : "unicode",
  );
}
...
function capturedSummary(game: Chess): CapturedSummary {
  ...
  for (const row of game.board()) {
    for (const piece of row) {
      if (piece)
        counts[piece.color as ChessColor][piece.type] =
          (counts[piece.color as ChessColor][piece.type] ?? 0) + 1;
    }
  }
  const missing = (color: ChessColor) =>
    ["q", "r", "b", "n", "p"].flatMap((type) =>
      Array(
        Math.max(
          0,
          (INITIAL_COUNTS[color][type] ?? 0) - (counts[color][type] ?? 0),
        ),
      ).fill(type),
    );
  const whiteCaptured = missing("w");
  const blackCaptured = missing("b");
  const whiteCapturedValue = blackCaptured.reduce(
    (sum, t) => sum + (PIECE_VALUES[t] ?? 0),
    0,
  );
  const blackCapturedValue = whiteCaptured.reduce(
    (sum, t) => sum + (PIECE_VALUES[t] ?? 0),
    0,
  );
...
function gameEndingInfo(game: Chess) {
  ...
  if (game.isThreefoldRepetition())
    return { title: "Draw", message: "The game is drawn by repetition." };
  if (game.isInsufficientMaterial())
    return {
      title: "Draw",
      message: "The game is drawn by insufficient material.",
    };
  if (game.isDraw())
    return { title: "Draw", message: "The game has ended in a draw." };
...
function parseStockfishInfo(line: string, fen: string) {
  ...
  return {
    ...
    multipv: multiMatch ? Number(multiMatch[1]) : 1,
  };
}
...
async function runBrowserStockfish(
  fen: string,
  skill: number,
  movetime = 750,
  multiPv = 3,
  ...
  return new Promise((resolve) => {
    ...
    const finish = () => {
      ...
      const pvs = Array.from(bestByPv.entries())
        .sort((a, b) => a[0] - b[0])
        .map(([, v]) => v)
        .filter(Boolean)
        .slice(0, Math.max(1, Math.min(10, multiPv)));
      if (!pvs.length) {
        resolve(null);
        return;
      }
    ...
    try {
      ...
      worker.onmessage = (event) => {
        ...
        if (line === "uciok") {
          sawUciOk = true;
          send(
            `setoption name MultiPV value ${Math.max(1, Math.min(10, multiPv))}`,
          );
          send("setoption name UCI_LimitStrength value true");
          send(
            `setoption name UCI_Elo value ${Math.max(1320, Math.min(3190, skill))}`,
          );
          send("isready");
          return;
        }
...
function BlundrApp({
  ...
  const initialSelectedRepertoireId = useMemo(() => {
    const canonicalInitialOpeningId =
      resolveStage2CanonicalOpeningId(initialOpeningId);
    return (
      canonicalInitialOpeningId ??
      initialOpeningId ??
      runtimeOpeningSelection.selectedOpeningId
    );
  }, [initialOpeningId, runtimeOpeningSelection.selectedOpeningId]);
  const [activeTab, setActiveTab] = useState<Tab>(initialTab);
  const [onboardingProfile, setOnboardingProfile] =
    useState<UserTrainingProfile | null>(null);
  const [repertoireProgress, setRepertoireProgress] =
  ...
  const [runtimeRepertoires, setRuntimeRepertoires] = useState<Repertoire[]>(
    () => buildRuntimePlaceholderRepertoires(),
  );
  const [runtimeTrainingSessionId, setRuntimeTrainingSessionId] =
    useState<string>(() => createLearningSessionId());
  const [authoritativeTrainerSession, setAuthoritativeTrainerSession] =
    useState<AuthoritativeTrainerSession | null>(null);
  ...
  const [continuationSessionId, setContinuationSessionId] = useState<
    string | null
  >(null);
  const [recentRuntimeTrainingLineKeys, setRecentRuntimeTrainingLineKeys] =
    useState<string[]>([]);
  const [
    selectedRuntimeTrainingLineSelection,
    setSelectedRuntimeTrainingLineSelection,
  ...
  const [feedback, setFeedback] = useState(
    "Choose an opening and begin training.",
  );
  ...
  const [
    ...
    setUserExplicitlyEnteredContinuation,
  ] = useState(false);
  const [continueFromHereClicked, setContinueFromHereClicked] = useState(false);
  const [continuationPauseClicked, setContinuationPauseClicked] =
    useState(false);
  const [
    continuationHardStopAcknowledged,
    setContinuationHardStopAcknowledged,
  ] = useState(false);
  const [moveHistory, setMoveHistory] = useState<string[]>([]);
  const [moveHistoryUci, setMoveHistoryUci] = useState<string[]>([]);
  const [progress, setProgress] = useState<Progress>(DEFAULT_PROGRESS);
  const [showAnswer, setShowAnswer] = useState(false);
  const [trainerPersistencePending, setTrainerPersistencePending] =
    useState(false);
  const [reviewingFen, setReviewingFen] = useState<string | null>(null);
  const [activeBoard, setActiveBoard] = useState(true);
  const [activeBoardView, setActiveBoardView] =
    useState<ActiveBoardView>("plan");
  ...
  const [projectiveTacticVisuals, setProjectiveTacticVisuals] = useState<
    ProjectiveTacticVisual[]
  >([]);
  const [projectiveTacticsFading, setProjectiveTacticsFading] = useState(false);
  const [
    projectiveTacticsAllowNextVisuals,
    setProjectiveTacticsAllowNextVisuals,
  ] = useState(true);
  const [bookComplete, setBookComplete] = useState(false);
  const [opponentCue, setOpponentCue] = useState<OpponentCue | null>(null);
  const [opponentVariationDebug, setOpponentVariationDebug] =
    useState<OpponentVariationDebug | null>(null);
  const [explorerMoves, setExplorerMoves] = useState<ExplorerMove[]>([]);
  const [brainResponse, setBrainResponse] = useState<BrainResponse | null>(
    null,
  );
  ...
  const [annotation, setAnnotation] =
    useState<BrainAnnotation>(blankAnnotation());
  const [visualModelOutput, setVisualModelOutput] =
    useState<VisualModelOutput | null>(null);
  ...
  const [visualDebugSnapshot, setVisualDebugSnapshot] =
    useState<VisualDebugSnapshot>({
      requestKey: null,
      requestPayload: null,
  ...
  const [brain, setBrain] = useState<LiveBrain>({
    ratingLabel: "Club",
    ratingPool: "1200–1600",
    book: "ready",
    lichess: "ready",
    engine: "ready",
    gpt: "ready",
    source: "rule visual",
    note: "Manual reveal/debug only",
  });
  const [moveQuality, setMoveQuality] = useState<MoveQualityResult | null>(
    null,
  );
  ...
  const [trainerPhase, setTrainerPhase] =
    useState<OverlayPhase>("ready_for_user");
  ...
  const [overlayClearedOnPhaseChange, setOverlayClearedOnPhaseChange] =
    useState(false);
  const [pendingOpponentRequest, setPendingOpponentRequest] =
    useState<PendingOpponentRequest | null>(null);
  const [maiaOpponentProviderStatus, setMaiaOpponentProviderStatus] =
    useState<MaiaProviderStatus>("disabled");
  const [maiaOpponentRequestId, setMaiaOpponentRequestId] = useState<
    number | null
  >(null);
  const [maiaOpponentRequestFen4, setMaiaOpponentRequestFen4] = useState<
    string | null
  >(null);
  const [maiaOpponentSkillLevel, setMaiaOpponentSkillLevel] =
    useState<MaiaSkillLevel | null>(null);
  const [maiaOpponentCandidateCount, setMaiaOpponentCandidateCount] =
    useState(0);
  const [maiaOpponentSelectedUci, setMaiaOpponentSelectedUci] = useState<
    string | null
  >(null);
  const [maiaOpponentSelectedSan, setMaiaOpponentSelectedSan] = useState<
    string | null
  >(null);
  const [maiaOpponentHumanLikelihood, setMaiaOpponentHumanLikelihood] =
    useState<number | null>(null);
  const [maiaOpponentDecisionReason, setMaiaOpponentDecisionReason] =
    useState<string>("not_requested");
  const [maiaOpponentFallbackUsed, setMaiaOpponentFallbackUsed] =
    useState(false);
  const [maiaOpponentFallbackReason, setMaiaOpponentFallbackReason] = useState<
    string | null
  >(null);
  const [maiaOpponentStaleResultIgnored, setMaiaOpponentStaleResultIgnored] =
    useState(false);
  const [
    maiaOpponentIllegalCandidateRejected,
    setMaiaOpponentIllegalCandidateRejected,
  ] = useState(false);
  const [maiaOpponentSelectedLegal, setMaiaOpponentSelectedLegal] = useState<
    boolean | null
  >(null);
  const [
    maiaOpponentRuntimeCandidateLegal,
    setMaiaOpponentRuntimeCandidateLegal,
  ] = useState<boolean | null>(null);
  const [maiaOpponentAppliedMoveUci, setMaiaOpponentAppliedMoveUci] = useState<
    string | null
  >(null);
  const [maiaOpponentAppliedMoveSan, setMaiaOpponentAppliedMoveSan] = useState<
    string | null
  >(null);
  const [maiaOpponentAppliedFromFen4, setMaiaOpponentAppliedFromFen4] =
    useState<string | null>(null);
  const [maiaOpponentAppliedToFen4, setMaiaOpponentAppliedToFen4] = useState<
    string | null
  >(null);
  const [maiaOpponentSanityGuardResult, setMaiaOpponentSanityGuardResult] =
    useState<string>("not_run");
  const [
    maiaOpponentSanityGuardBlockedReason,
    setMaiaOpponentSanityGuardBlockedReason,
  ] = useState<string | null>(null);
  const [maiaRuntimeMs, setMaiaRuntimeMs] = useState<number | null>(null);
  const [maiaRuntimeErrorReason, setMaiaRuntimeErrorReason] = useState<
    string | null
  >(null);
  const [maiaApiRouteStatus, setMaiaApiRouteStatus] =
  ...
  const [branchCompleteLatch, setBranchCompleteLatch] = useState<{
    ...
    lineId: null,
    ply: null,
    latchedAtFrameId: null,
  });
  const [runtimeCriticalIssues, setRuntimeCriticalIssues] = useState<string[]>(
    [],
  );
  const [coachInteraction, setCoachInteraction] = useState<
    | "none"
    | "hint"
    | "answer"
    | "why"
  ...
  const [coachHiddenFrameId, setCoachHiddenFrameId] = useState<string | null>(
    null,
  );
  ...
  const [coachMemoryMigration, setCoachMemoryMigration] = useState({
  ...
  const [continuationAnalysisStatus, setContinuationAnalysisStatus] =
    useState<ContinuationRuntimeStatus>("idle");
  const [runtimeBookFrameQuery, setRuntimeBookFrameQuery] =
    useState<RuntimeBookFrameQueryState>({
      openingId: null,
      playKeyBefore: null,
      candidates: [],
  ...
  const [lastContinuationUserMoveRating, setLastContinuationUserMoveRating] =
    useState<
      | (ContinuationUserMoveRatingResult & {
          moveUci: string;
          moveSan: string;
  ...
  const [debugSnapshotCollector, setDebugSnapshotCollector] =
    useState<DebugSnapshotCollector | null>(null);
  const [debugEventLog, setDebugEventLog] = useState<DebugEvent[]>([]);
  const [lastActionDebug, setLastActionDebug] = useState<Record<
    string,
    unknown
  > | null>(null);
  const [coachCardRenderTimeline, setCoachCardRenderTimeline] = useState<
    CoachCardRenderTimelineEntry[]
  >([]);
  const [surfaceModeTransitionTimeline, setSurfaceModeTransitionTimeline] =
    useState<Record<string, unknown>[]>([]);
  const [actionTimeline, setActionTimeline] = useState<
    Record<string, unknown>[]
  >([]);
  const [visualRenderTimeline, setVisualRenderTimeline] = useState<
    Record<string, unknown>[]
  >([]);
  const [plainLeakTimeline, setPlainLeakTimeline] = useState<
    Record<string, unknown>[]
  >([]);
  ...
  const trainerLearningPersistenceGateRef = useRef(
    new AuthoritativeLearningEventGate(),
  );
  const authoritativeTrainerSessionRef =
    useRef<AuthoritativeTrainerSession | null>(null);
  ...
  const previousSurfaceTransitionRef = useRef<{
    frameId: number;
    mode: string;
    hintShown: boolean;
  ...
  const lockedContinuationRef = useRef<
    Record<string, { uci: string; san?: string; source?: string } | null>
  >({});
  ...
  const continuationCandidateLockRef =
    useRef<typeof continuationCandidateLock>(null);
  const continuationEngineCacheRef = useRef<
    Record<string, { fen: string; pvs: EngineLine[]; source: string }>
  >({});
  ...
  useEffect(() => {
    if (
      ...
      debugCollectorLoadStartedRef.current
    )
      return;
    let cancelled = false;
  ...
  useEffect(() => {
    clearProjectiveTacticOverlay("rating_band_switch");
  }, [ratingFilter]);
  const repertoires = useMemo(() => {
    ...
    for (const repertoire of runtimeRepertoires)
      merged.set(repertoire.id, repertoire);
    for (const repertoire of customRepertoires)
      merged.set(repertoire.id, repertoire);
  ...
  function upsertRuntimeRepertoire(nextRepertoire: Repertoire) {
    setRuntimeRepertoires((current) => {
      const existingIndex = current.findIndex(
        (candidate) => candidate.id === nextRepertoire.id,
      );
  ...
  function commitRuntimeTrainingLineSelection(
    nextOpeningId: string,
    recentLineKeys: string[],
    sessionId: string,
    nextRepertoire: Repertoire,
    recordRecentLine: boolean,
  ) {
    const nextSelection = buildRuntimeTrainingLineSelection(
      nextOpeningId,
      recentLineKeys,
      sessionId,
      nextRepertoire,
    ...
    if (recordRecentLine && nextSelection?.selectedLineKey) {
      setRecentRuntimeTrainingLineKeys((current) =>
        updateRuntimeTrainingLineKeys(current, nextSelection.selectedLineKey),
      );
    }
    return nextSelection;
  }
  function loadRuntimeRepertoireForSelection(
    nextOpeningId: string,
    recentLineKeys: string[],
    sessionId: string,
    recordRecentLine: boolean,
    ...
    const canonicalId =
      resolveStage2CanonicalOpeningId(nextOpeningId) ?? nextOpeningId;
    ...
    return loadStage2RuntimeTrainableRepertoire(canonicalId).then((loaded) => {
      ...
      commitRuntimeTrainingLineSelection(
        canonicalId,
        recentLineKeys,
        sessionId,
        nextRepertoire,
  ...
  function refreshRuntimeTrainingLineSelection(
    nextOpeningId: string,
    recentLineKeys: string[] = recentRuntimeTrainingLineKeys,
    sessionId: string = runtimeTrainingSessionId,
    recordRecentLine = true,
  ) {
    const canonicalId =
      resolveStage2CanonicalOpeningId(nextOpeningId) ?? nextOpeningId;
    const nextRepertoire = repertoires.find(
    ...
    if (!nextRepertoire || nextRepertoire.runtimeLoading) {
      ...
      void loadRuntimeRepertoireForSelection(
        canonicalId,
        recentLineKeys,
        sessionId,
        recordRecentLine,
    ...
    return commitRuntimeTrainingLineSelection(
      canonicalId,
      recentLineKeys,
      sessionId,
      nextRepertoire,
  ...
  useEffect(() => {
    const nextRepertoire = repertoires.find(
      (candidate) => candidate.id === selectedRepertoireId,
    );
    if (!nextRepertoire) return;
    refreshRuntimeTrainingLineSelection(
      nextRepertoire.id,
      recentRuntimeTrainingLineKeys,
      runtimeTrainingSessionId,
      false,
    );
  }, [
  ...
  const canonicalSelectedRepertoireId = useMemo(
    () =>
      resolveStage2CanonicalOpeningId(selectedRepertoireId) ??
      selectedRepertoireId,
    [selectedRepertoireId],
  );
  const selectedRuntimeLineId =
    selectedRuntimeTrainingLineSelection?.selectedLineId ??
    canonicalSelectedRepertoireId;
  const selectedRuntimeLineKey =
    selectedRuntimeTrainingLineSelection?.selectedLineKey ??
    selectedRuntimeLineId;
  const selectedRuntimeLinePlaySequenceUci =
    selectedRuntimeTrainingLineSelection?.selectedPlaySequenceUci ?? [];
  useEffect(() => {
    const selection = selectedRuntimeTrainingLineSelection;
    const localDemo =
      process.env.NODE_ENV !== "production" &&
      getBlundrStorageModeSetting() === "local_demo" &&
      getLocalAccountCurrentUserId() === BLUNDR_LOCAL_DEMO_USER_ID;
    if (localDemo) {
  ...
  const selectedRuntimeLinePlyLength =
    selectedRuntimeLinePlaySequenceUci.length;
  ...
  const selectedRuntimeLineExhausted =
    ...
    selectedRuntimeLineCurrentPly >= selectedRuntimeLinePlyLength;
  const stage2OpeningDepthTargetPly = 12;
  const stage2OpeningCurrentPly = selectedRuntimeLineCurrentPly;
  const stage2OpeningDepthReached =
    stage2OpeningCurrentPly >= stage2OpeningDepthTargetPly;
  const selectedRuntimeLineUsedFor =
    trainingMode === "continuation" ? "memory_only" : "initial_seed";
  const hardRailDetected =
    selectedRuntimeLinePlyLength > 0 &&
    selectedRuntimeLinePlyLength < stage2OpeningDepthTargetPly;
  const hardRailBlockedReason = hardRailDetected
    ? "runtime_line_shorter_than_opening_depth_target"
    : null;
  const lineSelectionPreviousTwoSame =
    recentRuntimeTrainingLineKeys.length >= 2 &&
    recentRuntimeTrainingLineKeys[0] === recentRuntimeTrainingLineKeys[1];
  const repertoire =
    repertoires.find((r) => r.id === canonicalSelectedRepertoireId) ??
    repertoires[0];
  const openingTree = useMemo(
    () => buildOpeningTree(repertoireLineInputs(repertoire)),
    [repertoire],
  );
  ...
  const restrictedRuntimeLineAuthority = useMemo(
    () =>
      resolveRestrictedRuntimeLineAuthority({
        trainingMode: runtimeBackedRestrictedLineActive
          ? "restricted"
          : "continuation",
        selectedRuntimeLineId,
        selectedRuntimeLineKey,
  ...
  const selectedRuntimeLineAuthorityComplete =
    restrictedRuntimeLineAuthority.kind === "complete";
  const selectedRuntimeLineLearnerProgress =
    restrictedRuntimeLineAuthority.progress;
  const key = normalizeFen(fen);
  const exactOpeningNodes = openingTree.nodesByFen4[key] ?? [];
  const expectedMoveResolution = useMemo(
    () =>
      resolveExpectedMoveForFrame({
        openingTree,
        fen,
        trainerPhase,
        trainingMode,
        ...
        lastOpponentMoveSan:
          lastMoveColor === opponentColor ? lastMoveSan : null,
  ...
  const expectedMoveResolverDebug = useMemo(
    () => buildOpeningResolverDebug(expectedMoveResolution),
    [expectedMoveResolution],
  );
  const restrictedRuntimeLineUserContinuation =
    useMemo<Continuation | null>(() => {
      if (
        !runtimeBackedRestrictedLineActive ||
        restrictedRuntimeLineAuthority.kind !== "user_target"
      )
        return null;
      return {
      ...
      runtimeBackedRestrictedLineActive,
      restrictedRuntimeLineAuthority,
      canonicalSelectedRepertoireId,
    ]);
  ...
  const expectedMovesForValidation = useMemo(() => {
    ...
    expectedMoveResolution?.candidateMoves,
    userColor,
  ]);
  ...
  const selectedLineCompleteConfirmed = useMemo(() => {
    ...
    runtimeBackedRestrictedLineActive,
    selectedRuntimeLineAuthorityComplete,
    selectedRuntimeLineExhausted,
    selectedRuntimeLinePlyLength,
    expectedMoveResolution?.lineCursor,
  ...
  const hasNextOpponentMoveInSelectedLine = useMemo(() => {
    if (!exactSelectedLineNodes.length) return "unknown" as const;
    return exactSelectedLineNodes.some((node) =>
      node.continuations.some((move) => move.color === opponentColor),
    );
  }, [exactSelectedLineNodes, opponentColor]);
  const hasNextUserMoveInSelectedLine = useMemo(() => {
    if (!exactSelectedLineNodes.length) return "unknown" as const;
    return exactSelectedLineNodes.some((node) =>
      node.continuations.some((move) => move.color === userColor),
    );
  }, [exactSelectedLineNodes, userColor]);
  const explicitCuratedTerminalNode = useMemo(() => {
    const sideToMove = game.turn();
    return exactSelectedLineNodes.some(
      (node) => node.terminal && node.sideToMove === sideToMove,
    );
      ...
      resolveStage2TerminalProof({
        ...
        validBranchCompleteLatch: Boolean(
          branchCompleteLatch.active &&
            branchCompleteLatch.lineId === selectedRuntimeLineId,
        ),
        bookCompleteAllowed: Boolean(
          selectedLineCompleteConfirmed || explicitCuratedTerminalNode,
        ),
  ...
  const restrictedLineExhaustedOnOpponentTurnAfterUserMove =
    restrictedRuntimeBookHandoff.restrictedRuntimeBookExhaustedOnOpponentTurnAfterUserMove;
  ...
  const lichessStatsStatus =
    brain.lichess === "loading"
      ? "loading"
  ...
  const currentPlyCount =
    trainingMode === "restricted" ? moveHistoryUci.length : moveHistory.length;
  ...
  const forceContinuationPause =
    isUserTurn &&
    continuationPauseDecision.pauseRequired &&
    !userExplicitlyEnteredContinuation;
  const continuationPauseAlreadyConsumed = Boolean(
    userExplicitlyEnteredContinuation || continueFromHereClicked,
  );
  ...
  const hardStopBackupBlockedReason = hardStopBackupEligible
    ? null
    : trainingMode !== "restricted"
      ? "not_restricted_mode"
      : continuationPauseAlreadyConsumed
  ...
  const opponentBookOptions = exactOpeningNodes
    .flatMap((node) => node.continuations)
    .filter(
      (move, index, all) =>
        move.color === opponentColor &&
        all.findIndex((candidate) => candidate.uci === move.uci) === index,
    )
    .map((move) => ({
      san: move.san,
      uci: move.uci,
  ...
  const guidedCoveragePolicy = useMemo(
    () =>
      decideGuidedCoveragePolicy({
        currentPly: moveHistory.length,
        fullMoveNumber: Number(
          fen.split(" ")[5] ?? Math.floor(moveHistory.length / 2) + 1,
        ),
        ...
        nodeContinuationCount: exactOpeningNodes.reduce(
          (sum, node) => sum + node.continuations.length,
          0,
        ),
        userContinuationCount: expectedMoveResolution.candidateMoves.filter(
          (move) => move.color === userColor,
        ).length,
        opponentContinuationCount: opponentBookOptions.length,
        legalMoveCount: game.moves().length,
        knownBranchAvailable: expectedMoveResolution.candidateMoves.some(
          (move) => move.color === userColor,
        ),
        adaptiveBranchAvailable: false,
        continuationCandidateExists: Boolean(
          enginePreview &&
            normalizeFen(enginePreview.fen) === key &&
            enginePreview.pvs[0],
        ),
        explicitCuratedTerminalNode,
      }),
  ...
  const enabledViews: ActiveBoardView[] = ([] as ActiveBoardView[]).concat(
    ...
    boardSettings.showPlan ? ["plan"] : [],
  );
  const safeBoardView: ActiveBoardView = enabledViews.includes(activeBoardView)
    ? activeBoardView
    : (enabledViews[0] ?? "plan");
  const currentView = annotation[safeBoardView] ?? annotation.plan;
  const engineLines =
    enginePreview && normalizeFen(enginePreview.fen) === normalizeFen(fen)
      ? enginePreview.pvs
      : [];
  const continuationPolicyCandidate = useMemo(() => {
    ...
    const isHardEndOfBookForCandidate =
      stage2TerminalProof.proven && !userExplicitlyEnteredContinuation;
  ...
  const continuationLockCandidate = useMemo(() => {
    ...
    return {
      ...
      rejectionReasons: [],
      debug: {
        lockId: lock.continuationCandidateLockId,
        requestId: lock.continuationCandidateLockRequestId,
      },
    };
  }, [
    trainingMode,
    isUserTurn,
    userExplicitlyEnteredContinuation,
    continuationCandidateLock,
    fen,
    game,
  ]);
  const runtimeBookFrameShouldQuery = useMemo(() => {
    if (activeTab !== "train") return false;
    if (!runtimeOpeningIdForFrame || !runtimePlayKeyBeforeForFrame)
      return false;
  ...
  useLayoutEffect(() => {
    ...
    setRuntimeBookFrameQuery((prev) => ({
      ...
      candidates:
        prev.openingId === runtimeOpeningIdForFrame &&
        prev.playKeyBefore === runtimePlayKeyBeforeForFrame
          ? prev.candidates
          : [],
      hasRuntimeBookCandidates:
        prev.openingId === runtimeOpeningIdForFrame &&
        prev.playKeyBefore === runtimePlayKeyBeforeForFrame
          ? prev.hasRuntimeBookCandidates
          : false,
      bookExhausted: false,
      status: "loading",
      error: null,
    ...
    void fetch(`/api/runtime-book/candidates?${params.toString()}`, {
      ...
      .then(async (res) => {
        ...
        setRuntimeBookFrameQuery({
          ...
          candidates,
          hasRuntimeBookCandidates: Boolean(
            payload?.hasRuntimeBookCandidates && candidates.length > 0,
          ),
          bookExhausted: Boolean(
            payload?.bookExhausted ?? candidates.length === 0,
          ),
          status: "ready",
          error: null,
        });
  ...
  const runtimeBookPreferredCandidate = useMemo(
    () =>
      runtimeBookFrameQuery.hasRuntimeBookCandidates
        ? (runtimeBookFrameQuery.candidates[0] ?? null)
        : null,
    [
      runtimeBookFrameQuery.hasRuntimeBookCandidates,
  ...
  const runtimeGraphAuthorityUsed = Boolean(
    selectedRuntimeTrainingLineSelection?.source ??
      selectedOpeningAvailability?.runtimeAvailable,
  );
  ...
  const runtimeGraphSelectedCandidateUci =
    runtimeBookPreferredCandidate?.uci ?? null;
  const stockfishTopMovesForContinuation = useMemo(
    () =>
      mapEngineLinesToStockfishTopMoves({
        fen,
        pvs: engineLines.map((line) => ({
          uci: line.uci,
          san: line.san,
          cp: line.cp,
        })),
        ...
        errorReason: engineLines.length
          ? undefined
          : "stockfish_provider_unavailable",
      }),
    [fen, engineLines],
  );
  const continuationResolvedTargetUci =
    runtimeBookPreferredCandidate?.uci ??
    stockfishTopMovesForContinuation.bestMoveUci ??
    null;
  const continuationResolvedTargetSan =
    runtimeBookPreferredCandidate?.san ??
    stockfishTopMovesForContinuation.bestMoveSan ??
    null;
  ...
  const continuationResolvedTargetLabel = runtimeBookPreferredCandidate?.uci
    ...
    : continuationResolvedTargetUci
      ? "Best"
      : null;
  const continuationTargetResolverStatus =
    trainingMode !== "continuation"
  ...
  const restrictedOpponentReplyAuthorityPreview = useMemo(() => {
    const runtimeBookMatchesFrame =
      runtimeBookFrameQuery.status === "ready" &&
      runtimeBookFrameQuery.openingId === runtimeOpeningIdForFrame &&
      runtimeBookFrameQuery.playKeyBefore === runtimePlayKeyBeforeForFrame;
    const runtimeBookSelectedCandidate = runtimeBookMatchesFrame
      ? (runtimeBookFrameQuery.candidates.find((candidate) =>
          continuationLegalMoveUcis.includes(candidate.uci),
        ) ?? null)
      : null;
  ...
  const continuationCandidateResolution = useMemo(
    () =>
      resolveEffectiveContinuationCandidate({
        trainingMode,
        isUserTurn,
        trainerPhase,
        boardFen: fen,
  ...
  const effectiveContinuationCandidate =
    continuationCandidateResolution.candidate
      ? {
          ...
          pieceTypeCode:
            continuationCandidateResolution.candidate.pieceTypeCode,
          pieceTypeCanonical:
            continuationCandidateResolution.candidate.pieceTypeCanonical,
          ...
          debug: {
            ...
            candidateReplacedByStockfish: false,
            replacementReason: null,
            topMoveUcis: stockfishTopMovesForContinuation.topMoves.map(
              (move) => move.uci,
            ),
          },
        }
  ...
  const continuationSuggestionValidation = useMemo(() => {
    ...
    effectiveContinuationCandidate?.uci,
    effectiveContinuationCandidate?.san,
    stockfishTopMovesForContinuation,
  ]);
  const validatedContinuationCandidate = effectiveContinuationCandidate;
  const branchCompleteContract = useMemo(
    () =>
      resolveBranchCompleteContract({
        trainingMode,
        trainerPhase,
        isUserTurn,
        userExplicitlyEnteredContinuation,
  ...
  const continueFromHereClickHandled = Boolean(
    continueFromHereClicked &&
      userExplicitlyEnteredContinuation &&
      trainingMode === "continuation" &&
      !branchCompleteEligibleNow,
  );
  const continueFromHereClickBlockedReason =
    continueFromHereClicked && !continueFromHereClickHandled
      ? branchCompleteEligibleNow
        ? "branch_complete_still_active"
        : "continuation_not_entered"
      : null;
  const branchCompleteReasonNow = stage2TerminalProof.proven
    ? (stage2TerminalProof.reason ??
      branchCompleteContract.reason ??
      "line_complete")
    : (stage2TerminalProof.blockedReasons[0] ??
      branchCompleteContract.blockedReason ??
      "terminal_proof_required");
  const branchCompleteShouldCancelPending =
    branchCompleteEligibleNow && Boolean(pendingOpponentRequest);
  const continuationFlowContract = useMemo(
    () =>
      resolveContinuationFlowContract({
        trainingMode,
  ...
  const currentInstructionFrame = useMemo(() => {
    ...
    if (
      ...
      return buildCurrentInstructionFrame({
        ...
        branchComplete: {
          isComplete: true,
          reason: selectedLineCompleteConfirmed
    ...
    const thisFrameKey = computeInstructionFrameKey({
      ...
      trainerPhase,
      source:
        trainingMode === "continuation" ? "continuation_candidate" : "guided",
    });
    ...
    const guidedMoveAuthorityEligible =
      Boolean(
        restrictedRuntimeLineUserContinuation?.uci ||
          expectedMoveResolution.expectedMoveUci,
      ) &&
    ...
    return buildCurrentInstructionFrame({
      ...
      continuationCandidate:
        ...
        !forceContinuationPause &&
        validatedContinuationCandidate?.uci &&
        validatedContinuationCandidate.source !== "no_reliable_continuation"
          ? {
              uci:
                useLocked && lockedForThisFrame
                  ? lockedForThisFrame.uci
                  : validatedContinuationCandidate.uci,
              san:
                useLocked && lockedForThisFrame
              ...
              source:
                useLocked && lockedForThisFrame
                  ? (lockedForThisFrame.source ??
                    "locked_continuation_candidate")
                  : validatedContinuationCandidate.source,
  ...
  if (
    instructionTarget?.kind === "continuation_candidate" &&
    instructionTarget.uci
  ) {
    const lockKey =
      currentInstructionFrame.instructionFrameKey ||
  ...
  const continuationRuntimeState = useMemo(
    () =>
      classifyContinuationRuntimeState({
        fen,
        trainingMode,
  ...
  const expectedUserOptions = useMemo<Continuation[]>(
    () =>
      isBookLikeInstructionTarget(instructionTarget)
        ? [
            {
              san: instructionTarget.san,
              uci: instructionTarget.uci,
  ...
  const currentSelectedCandidateUci =
    instructionTarget?.kind === "continuation_candidate"
      ? instructionTarget.uci
      : null;
  const currentSelectedCandidateSan =
    instructionTarget?.kind === "continuation_candidate"
      ? instructionTarget.san
      : null;
  const currentSelectedCandidateSource = instructionTarget?.source ?? "none";
  const currentSelectedCandidateLegal = Boolean(
    currentSelectedCandidateUci &&
      (game.moves({ verbose: true }) as any[]).some(
        (move) => moveToUci(move) === currentSelectedCandidateUci,
      ),
  );
  const expectedUserOptionsSignature = expectedUserOptions
    .map((move) => `${move.uci ?? ""}:${move.san ?? ""}`)
    .join("|");
  ...
  const expectedUserSans = expectedMovesForValidation
    .map((move) => move.san)
    .filter(Boolean) as string[];
  const frameKey = useMemo(
    () =>
      buildRuntimeFrameKey({
        fen,
        trainerPhase,
        trainerView,
        trainingMode,
  ...
  useEffect(() => {
    ...
    const staleDetected = Boolean(
      previous &&
        currentSelectedCandidateUci &&
        previous !== currentSelectedCandidateUci,
    );
    ...
    previousSelectedCandidateUciRef.current =
      currentSelectedCandidateUci ?? null;
  }, [currentSelectedCandidateUci]);
  const shouldValidateTrainingMove =
    activeTab === "train" &&
    trainingMode === "restricted" &&
    isUserTurn &&
    !bookComplete &&
    historyIndex >= positionHistory.length - 1 &&
    expectedMovesForValidation.length > 0;
  const moveQualityUserStatus = getMoveQualityUserStatus(
    moveQuality,
    moveQualityPending,
  );
  const moveQualityVerified = isMoveQualityVerified(moveQuality);
  const hideUnverifiedTrainingHints =
    trainingMode === "restricted" &&
    isUserTurn &&
    !showAnswer &&
    shouldValidateTrainingMove &&
    !moveQualityVerified;
  const visualSuppressed = Boolean(
    visualModelOutput?.suppress?.includes("recommendation_pending"),
  );
  const activeVisualModelOutput =
    visualModelOutput && !visualSuppressed ? visualModelOutput : null;
  const teachingOrchestration = useMemo(() => {
    if (activeTab !== "train" || trainingMode !== "restricted" || !isUserTurn)
      return null;
    const expectedMove = expectedMovesForValidation[0];
    if (!expectedMove) return null;
    const expectedMoveBook = explorerMoves.find(
      (m) => m.uci === expectedMove.uci,
    );
    const totalBookGames = explorerMoves.reduce(
      (sum, m) => sum + (m.total || 0),
      0,
    );
      ...
      return orchestrateTeaching({
        teachingInput: {
          ...
          move: {
            ...
            promotion:
              expectedMove.uci.length > 4
                ? expectedMove.uci.slice(4, 5)
                : undefined,
          },
          ...
          userMemory: {
            patternSeenCount: progress.trainedPositions[normalizeFen(fen)]
              ? 1
              : 0,
            patternMissedCount:
              progress.mistakes[normalizeFen(fen)]?.count ?? 0,
            patternSuccessCount: progress.trainedPositions[normalizeFen(fen)]
              ? 1
              : 0,
          },
        },
        expectedMove: { uci: expectedMove.uci, san: expectedMove.san },
        engineTopMoves: engineLines.map((line, index) => ({
          uci: line.uci,
          san: line.san,
          rank: index + 1,
          scoreCp: line.cp,
        })),
        bookTopMoves: explorerMoves.slice(0, 5).map((move) => ({
          uci: move.uci,
          san: move.san,
          moveShare: move.pct / 100,
          moveGames: move.total,
          totalGames: totalBookGames,
        })),
        repertoireMoves: expectedMovesForValidation.map((move) => ({
          uci: move.uci,
          san: move.san,
        })),
        bookSupportInput: {
    ...
    explorerMoves,
    engineLines,
    progress.trainedPositions,
    progress.mistakes,
  ]);
  const boardFen = normalizeFen(fen);
  const effectiveViewModeForVisual =
    trainerView === "plain" && showMoreShown ? "assisted" : trainerView;
  const visualRecipe = useMemo(
    () =>
    ...
    [
      ...
      isUserTurn,
      showMoreShown,
      effectiveViewModeForVisual,
    ],
  );
  const visualRecipeMoveUci = visualRecipe?.moveUci?.toLowerCase() ?? null;
  const visualRecipeTargetMatchesInstructionTarget = instructionTarget?.uci
    ? !visualRecipeMoveUci || visualRecipeMoveUci === instructionTarget.uci
    : "unknown";
  const visualRecipeBlockedByTargetMismatch = Boolean(
    instructionTarget?.uci &&
      visualRecipeMoveUci &&
      visualRecipeMoveUci !== instructionTarget.uci,
  );
  const visualRecipeForRender =
    !instructionTarget?.uci || visualRecipeBlockedByTargetMismatch
      ? null
      : visualRecipe;
  const visualRecipeOverlay = useMemo(
    () =>
      adaptVisualRecipe({
  ...
  const visualRecipePlayback = useVisualRecipePlayback({
    ...
    enabled:
      trainerPhase === "ready_for_user" &&
      isUserTurn &&
      visualRecipeOverlay.adapterAllowed &&
      effectiveViewModeForVisual === "assisted",
  });
  const overlayFen = teachingOrchestration
    ? normalizeFen(teachingOrchestration.cue.metadata.fenBefore)
    : undefined;
  const overlaySource = teachingOrchestration
    ? visualRecipeOverlay.overlaySource
    : activeVisualModelOutput
      ? "visual_model"
      : "annotation";
  const overlaySuppressedReason = teachingOrchestration
    ? visualRecipeOverlay.suppressedReason
    : trainerView === "plain" && !showAnswer
      ? "plain_view"
      : trainerPhase !== "ready_for_user"
        ? "phase_not_ready_for_user"
        : !isUserTurn
          ? "not_user_turn"
          : undefined;
  const staleOverlayFlag =
    staleOverlayIgnored || visualRecipeOverlay.staleOverlayIgnored;
  const visualRecipeMainLines: ActiveLine[] = useMemo(() => {
    if (overlaySuppressedReason || !instructionTarget?.uci) return [];
    const legalLines = visualRecipePlayback.lines.filter(
      (line) => isValidSquare(line.from) && isValidSquare(line.to),
    );
    if (!legalLines.length) return [];
    const expectedFrom = instructionTarget.from;
    const expectedTo = instructionTarget.to;
    const matching = legalLines.find(
      (line) => line.from === expectedFrom && line.to === expectedTo,
    );
    const primary = matching ?? {
      from: expectedFrom,
      to: expectedTo,
      kind: "plan" as const,
      label: instructionTarget.san,
    };
    return [primary];
  }, [
    overlaySuppressedReason,
    visualRecipePlayback.lines,
    instructionTarget?.uci,
    instructionTarget?.from,
    instructionTarget?.to,
    instructionTarget?.san,
  ]);
  const legacyVisualLines: ActiveLine[] = visualModelOutput
    ? activeVisualModelOutput &&
      ...
      ? filterLegacyMainUiLines(
          (activeVisualModelOutput.arrows ?? [])
            .filter((a) => isValidSquare(a.from) && isValidSquare(a.to))
            .slice(0, 2)
            .map((a) => ({
              from: a.from,
              to: a.to,
              kind: visualLineKind(a.role, a.kind),
              label: a.label,
            })),
        )
      : []
    : filterLegacyMainUiLines(currentView.lines);
  const visualContext = activeVisualModelOutput?.context;
  const visualAnimationName =
    activeVisualModelOutput?.animationPackage?.name ??
    activeVisualModelOutput?.animation;
  const safeMoveArrowVisual = useMemo(() => {
    ...
    const moveSan =
      ...
      isBookLikeInstructionTarget(instructionTarget)
        ? instructionTarget.san
        : null;
    const moveLegal = Boolean(
      moveUci &&
        (game.moves({ verbose: true }) as any[]).some(
          (move) => moveToUci(move) === moveUci,
        ),
    );
    ...
    if (!shouldRender || !moveUci) {
      return {
        ...
        lines: [],
        highlights: [],
        blockedReason: moveUci
          ? "not_authorized_visual_surface"
          : "missing_candidate_uci",
      };
    }
    return {
      source: "guided_target_fallback" as const,
  ...
  const basePatternCue = buildPatternCue({
    trainerView,
    visualModelOutput,
    visualModelPending,
    visualModelError,
  ...
  const patternCue =
    teachingOrchestration &&
    (teachingOrchestration.permission.canShowPatternCue ||
      teachingOrchestration.permission.canShowContextCue)
      ? {
          ...basePatternCue,
          status: "ready" as PatternCueStatus,
          source: teachingOrchestration.permission.canShowPatternCue
            ? ("rule_visual" as const)
            : ("local_fast" as const),
          title:
            teachingOrchestration.cue.userFacing.title || "Move not verified",
          snippet:
            teachingOrchestration.cue.userFacing.snippet ||
            "Blundr will not invent a plan here.",
          next: teachingOrchestration.nextPlay.allowed
            ? teachingOrchestration.cue.userFacing.next
            : undefined,
          concept: teachingOrchestration.cue.conceptId,
        }
      : basePatternCue;
  const patternCueBadgeLabel =
    trainerView === "assisted" && !showAnswer && teachingOrchestration
      ? `Assisted View • ${teachingOrchestration.userLabel}`
      : getMoveQualityBadgeLabel({
          trainerView,
          showAnswer,
          shouldValidateTrainingMove,
          moveQuality,
          moveQualityPending,
          patternCueStatus: patternCue.status,
        });
  const showValidatedBadge =
    trainerView === "assisted" &&
    !showAnswer &&
    Boolean(teachingOrchestration?.cue.userFacing.badge);
  const moveImpact =
    teachingOrchestration?.moveImpact ?? impactFromEngine(engineLines[0]);
  const coachMemoryForDecision = coachUtteranceMemoryRef.current;
  const coachContextResult = useMemo(
    () =>
      buildCoachContext({
        frameId: trainerFrameId,
        boardFen: fen,
        ...
        visualRecipe: visualRecipe
          ? {
              ...
              primitiveTypes: visualRecipe.beats.flatMap((beat) =>
                beat.primitives.map((primitive) => primitive.type),
              ),
        ...
        wrongAttempts:
          reviewingFen && progress.mistakes[reviewingFen]
            ? progress.mistakes[reviewingFen].count
            : 0,
        ...
        priorPatternSuccesses: progress.trainedPositions[normalizeFen(fen)]
          ? 1
          : 0,
        recentUtteranceIds: coachMemoryForDecision
          .slice(-5)
          .map((entry: any) => entry.utteranceId),
        recentUtteranceFamilies: coachMemoryForDecision
          .slice(-5)
          .map((entry: any) => entry.utteranceFamily),
      }),
    [
      trainerFrameId,
      fen,
      ...
      selectedRepertoireId,
      coachMemoryForDecision,
    ],
  );
  const adaptiveCoachDecision = useMemo(
    () =>
      decideCoachOutput({
        context: coachContextResult.context,
        interaction: coachInteraction,
        outcome:
          reviewingFen && progress.mistakes[reviewingFen] ? "wrong" : "none",
        ...
        brainInput: {
          ...
          trainingMode:
            bookComplete || trainingMode === "continuation"
              ? "continuation"
              : "restricted",
          viewMode:
            trainingMode === "continuation"
          ...
          stale: coachContextResult.context
            ? !coachContextResult.context.recipeFrameMatchesBoard ||
              !coachContextResult.context.recipeFenMatchesBoard
            : true,
          expectedMoveSource: expectedMoveResolution.source,
    ...
    [
      ...
      trainingMode,
      trainerView,
      expectedUserOptionsSignature,
      currentSelectedCandidateUci,
      currentSelectedCandidateSan,
      enginePreview,
      visualRecipe,
      teachingOrchestration,
      explorerMoves,
  ...
  const liveCoachState = useMemo(() => {
    if (trainerPhase === "ready_for_user" && isUserTurn && instructionTarget) {
      ...
      const recentInstructional = getRecentInstructionalCoachRecords(
        lastCoachRecordsRef.current,
        5,
      );
      ...
      const brainFrameKey = computeInstructionFrameKey({
        fen,
        trainingMode,
        isUserTurn,
        trainerPhase,
        source: instructionTarget?.kind || trainingMode,
      });
      const brainAnalysisForCoach = instructionTarget
        ? analyzeBlundrPosition({
            fen,
            currentInstructionFrame: currentInstructionFrame, // production shape
            frameKey: brainFrameKey,
            trainingMode:
              trainingMode === "restricted" ? "guided" : trainingMode,
            isUserTurn,
            debugEnabled: blundrDebugEnabled,
          } as any)
        : null; // temporary 'as any' during full type migration
      const coachPipeline = buildCoachExplanationPipeline({
        ...
        recentCoachThemes: recentInstructional.map((entry) =>
          String(entry.selectedOpportunityId ?? ""),
        ),
      ...
      const silence = shouldLiveCoachStaySilent({
        ...
        selected,
        userRequestedHelp:
          coachInteraction === "hint" || coachInteraction === "why",
        repeatedConcept: false,
      });
      const text = silence.silent
        ? ""
        : coachPipeline.coachExplanation.body ||
          pickLiveCoachCopy(
            selected?.opportunity ?? "silence",
            `${canonicalSelectedRepertoireId}:${normalizeFen(fen)}`,
          );
      const lintedText =
        validateLiveCoachCopy(text).allowed && !isDebugLeakText(text)
          ? text
          : buildVerifiedUserFacingFallback(coachPipeline.moveFactPacket).body;
      const safeText = lintedText;
      const exactMoveAllowed = Boolean(
        selected?.exactMoveAllowed &&
          selected?.candidateMoveUci &&
          selected?.candidateMoveSan,
      );
      const mode = exactMoveAllowed
        ? "supported_continuation"
        : "freeplay_principle";
      const selectedCandidate =
        candidates.find(
          (candidate) => candidate.moveUci === instructionTarget.uci,
        ) ?? null;
      const selectedTemplateId = coachPipeline.coachExplanation.selectedTheme
        ? `live:${coachPipeline.coachExplanation.selectedTheme}:${coachPipeline.coachExplanation.selectedTheme === "capture_or_recapture" || coachPipeline.coachExplanation.selectedTheme === "checkmate" ? "explain_tactic" : coachPipeline.coachExplanation.selectedTheme === "central_pawn_advance" ? "explain_center" : "explain_development"}`
        : selected
          ? `live:${selected.opportunity}:${selected.intent}`
          : null;
      const normalizedLiveDebug = normalizeCoachDebugMetadata({
        ...
        recognizedPlanTypes: selected
          ? [`opportunity:${selected.opportunity}`]
          : [],
        selectedOpportunityId:
          coachPipeline.coachExplanation.selectedTheme ??
          selected?.opportunity ??
          null,
        selectedOpportunityMoveUci:
          selected?.candidateMoveUci ?? instructionTarget.uci,
        selectedTemplateId: selectedTemplateId ?? undefined,
        templateCandidatesTop5: selected
          ? [
              {
                id: selectedTemplateId ?? "live:none",
        ...
        renderableOpportunityCount: opportunities.filter(
          (opportunity) => opportunity.totalScore !== undefined,
        ).length,
        explanationRenderMs: 0,
        coachDecisionSource: coachPipeline.coachExplanation.usedFallback
          ? "verified_safe_fallback"
          : "live_coach",
        selectedTheme: coachPipeline.coachExplanation.selectedTheme,
        selectedOpportunityLayer:
          coachPipeline.coachExplanation.selectedOpportunityLayer ?? null,
        selectedOpportunityScore:
          coachPipeline.coachExplanation.selectedOpportunityScore ??
          coachPipeline.opportunityPacket.selected?.score ??
          null,
        selectedPlanType:
          coachPipeline.coachExplanation.selectedPlanType ??
          coachPipeline.planPacket.plans[0]?.planType ??
          null,
        ...
        featurePacket: {
          status: coachPipeline.featurePacket.status,
          ...coachPipeline.featurePacket,
        },
        planPacket: {
          status: coachPipeline.planPacket.status,
          plans: coachPipeline.planPacket.plans,
        },
        opportunityPacket: {
          status: coachPipeline.opportunityPacket.opportunities.length
            ? "ran"
            : "ran_empty",
          opportunitiesTop5:
            coachPipeline.opportunityPacket.opportunities.slice(0, 5),
      ...
      return {
        ...
        buttons: exactMoveAllowed
          ...
          : (["hint", "show_plan", "analyze_idea"] as CoachButton[]),
  ...
  const selectedContinuationCandidateForCoach =
    trainingMode === "continuation" && currentSelectedCandidateUci
      ? {
          uci: currentSelectedCandidateUci,
          san: currentSelectedCandidateSan ?? currentSelectedCandidateUci,
        }
      : null;
  const rawCoachDecision = useMemo<any>(() => {
    if (coachHiddenFrameId === String(trainerFrameId))
      return {
        ...adaptiveCoachDecision,
        shouldShowCoachCard: false,
        suppressedReason: "hidden_for_frame",
      };
    if (
      trainingMode === "continuation" &&
      ...
      return {
        ...
        title: "Line complete",
        body:
          continuationRuntimeState.reason === "checkmate"
            ? "This line has reached checkmate. Restart the line or review the pattern."
            : "This position has no legal continuation. Restart the line or choose another branch.",
        buttons: ["hide"],
        ...
        debug: {
          coachIntent: "continuation_terminal",
          continuationRuntimeStatus: "terminal",
          continuationTerminalReason: continuationRuntimeState.reason,
          coachMoveUci: null,
          coachPieceType: null,
          coachVerifiedFactsUsed: false,
          verifiedFallbackUsed: true,
          fallbackReason: "terminal_position",
        },
      };
    }
    if (
      ...
      return {
        ...
        debug: {
          coachIntent: "opponent_replying",
          continuationRuntimeStatus: "opponent_replying",
          coachMoveUci: null,
          coachPieceType: null,
          coachVerifiedFactsUsed: false,
          verifiedFallbackUsed: true,
          fallbackReason: "opponent_replying",
        },
      };
    }
    if (
      ...
      return {
        ...
        debug: {
          coachIntent: "analyzing_continuation",
          coachMoveUci: null,
          coachPieceType: null,
          coachVerifiedFactsUsed: false,
          advancedFeatureClaimTypes: [],
          recognizedPlanTypes: [],
    ...
    if (
      ...
      return {
        ...
        claimTypes: ["plan_principle"],
        debug: {
          coachIntent: "no_reliable_continuation",
          coachMoveUci: null,
          coachPieceType: null,
          coachVerifiedFactsUsed: false,
          verifiedFallbackUsed: true,
          fallbackReason: "no_reliable_continuation",
        },
      };
    }
    if (liveCoachState) {
      return {
        ...
        normalizedFen: normalizeFen(fen),
        title:
          liveCoachState.title ??
          (liveCoachState.mode === "supported_continuation"
        ...
        debug: {
          ...
          selectedIntent:
            liveCoachState.selected?.intent ??
            (liveCoachState.selected
              ? selectIntentForOpportunity(liveCoachState.selected.opportunity)
              : undefined),
          ...
          advancedFeaturePacketExists: Array.isArray(
            (liveCoachState.debug as any)?.advancedFeatureClaimTypes,
          ),
          strategicPlanPacketExists: Array.isArray(
            (liveCoachState.debug as any)?.recognizedPlanTypes,
          ),
          selectedOpportunityId:
            (liveCoachState.debug as any)?.selectedOpportunityId ??
            liveCoachState.selected?.opportunity ??
            undefined,
          selectedOpportunityMoveUci:
            (liveCoachState.debug as any)?.selectedOpportunityMoveUci ??
            instructionTarget?.uci ??
            null,
          selectedTemplateId: (liveCoachState.debug as any)?.selectedTemplateId,
          templateCandidatesTop5:
            (liveCoachState.debug as any)?.templateCandidatesTop5 ?? [],
          blockedTemplatesTop10:
            (liveCoachState.debug as any)?.blockedTemplatesTop10 ?? [],
          whySelectedOpportunityWon:
            (liveCoachState.debug as any)?.whySelectedOpportunityWon ?? null,
          opportunityCount:
            (liveCoachState.debug as any)?.opportunityCount ??
            liveCoachState.opportunities?.length ??
            0,
          renderableOpportunityCount:
            (liveCoachState.debug as any)?.renderableOpportunityCount ??
            liveCoachState.opportunities?.length ??
            0,
        },
      };
    ...
    () =>
      decideTrainerPhaseActionGate({
        ...
        trustedContinuationCandidateAvailable: Boolean(
          selectedContinuationCandidateForCoach,
        ),
  ...
  const coachDecision = useMemo<any>(() => {
    ...
    const candidateCoachAllowed =
      trainingMode === "continuation" &&
      instructionTarget?.kind === "continuation_candidate" &&
      currentSelectedCandidateLegal;
    const transitionCoachAllowed =
      trainingMode === "continuation" &&
      [
        "continuation_terminal",
        "opponent_replying",
        "analyzing_continuation",
      ].includes(String(rawCoachDecision?.debug?.coachIntent ?? ""));
    const rawIntent =
      rawCoachDecision?.debug?.coachIntent ??
      rawCoachDecision?.debug?.selectedIntent;
    const missingDeepCoach =
      !(
        rawCoachDecision?.debug?.selectedTemplateId ||
        rawCoachDecision?.debug?.mappingTemplateId
      ) && !rawCoachDecision?.debug?.selectedOpportunityId;
    const genericCandidateCoach =
      rawCoachDecision?.title === "Position context" ||
      rawIntent === "silent" ||
      missingDeepCoach;
    if (!phaseActionGate.shouldRenderCoach && !transitionCoachAllowed) {
      return {
        ...
        buttons: [],
        suppressedReason:
          phaseActionGate.blockedReason ?? rawCoachDecision?.suppressedReason,
        debug: normalizeCoachDebugMetadata({
          ...(rawCoachDecision?.debug ?? {}),
          ...coachDebugBase,
          phaseActionGate,
        }),
      };
    ...
    if (
      candidateCoachAllowed &&
      instructionTarget &&
      isEngineBestContinuationSource(instructionTarget.source)
    ) {
      ...
      return {
        ...
        buttons: phaseActionGate.filteredButtons.length
          ? phaseActionGate.filteredButtons
          : ["show_plan", "analyze_idea", "show_move", "hide"],
        shouldShowCoachCard: true,
        ...
        debug: normalizeCoachDebugMetadata({
          ...
          ...coachDebugBase,
          phaseActionGate,
          coachIntent: "show_continued_plan",
          coachDecisionSource: "verified_safe_fallback",
          candidateCoachFallbackUsed: true,
          candidateCoachFallbackReason: "engine_best_move_fallback",
          selectedCandidateSource: "engine_best",
          verifiedFallbackUsed: true,
          fallbackReason: "engine_best_move_fallback",
        }),
      };
    }
    if (candidateCoachAllowed && genericCandidateCoach) {
      const safeFallback = instructionTarget
        ? buildUserFacingTargetFallback({
            fen,
            target: instructionTarget,
            trainingMode,
            trainerPhase,
      ...
      return {
        ...
        debug: normalizeCoachDebugMetadata({
          ...
          coachIntent: "show_continued_plan",
          candidateCoachFallbackUsed: true,
          candidateCoachFallbackReason:
            "missing_template_or_silent_generic_candidate",
          coachSelectedCandidateMove: currentSelectedCandidateUci,
          coachDecisionSource: "verified_safe_fallback",
          selectedTheme: safeFallback?.theme,
          coachQuality: safeFallback?.quality,
          fallbackReason: safeFallback?.reason ?? "candidate_safe_fallback",
        }),
      };
    }
    if (rawIntent === "silent") {
      return {
        ...
        debug: normalizeCoachDebugMetadata({
          ...
          ...coachDebugBase,
          phaseActionGate,
          coachRenderedDespiteSilentIntent: false,
        }),
      };
    }
    let out = {
    ...
    const sameNormalizedCount = recent.filter(
      (record) => record.normalizedBody === normalizedBody,
    ).length;
    const sameFamilyCount = recent.filter(
      (record) =>
        record.utteranceFamily &&
        record.utteranceFamily === out.utteranceFamily,
    ).length;
    const repeatedGenericPattern =
      sameNormalizedCount >= 2 || sameFamilyCount >= 2;
    if (repeatedGenericPattern && instructionTarget) {
      ...
      out = {
        ...
        debug: {
          ...
          repetitionGuardReason:
            sameNormalizedCount >= 2
              ? "same_normalized_body_3_in_5"
              : "same_family_3_in_5",
    ...
    const featureClaims = Array.isArray(
      (out.debug as any)?.advancedFeatureClaimTypes,
    )
      ? (out.debug as any).advancedFeatureClaimTypes
      : [];
    const planClaims = Array.isArray((out.debug as any)?.recognizedPlanTypes)
      ? (out.debug as any).recognizedPlanTypes
      : [];
    ...
    if (instructionTarget && claimValidation.unverifiedClaims.length) {
      ...
      out = {
        ...out,
        title:
          out.title === "Analyzing continuation"
            ? out.title
            : safeFallback.title,
        body: safeFallback.body,
        debug: {
  ...
  const coachSurfacePolicy = useMemo(
    () =>
      decideCoachSurfacePolicy({
        ...
        engineValidationStatus:
          (coachDecision?.debug as any)?.coachEngineStatus ??
          (enginePreview?.pvs?.length ? "ready" : "idle"),
  ...
  const branchTransitionSurface = useMemo(() => {
    ...
    const transitionBody =
      "You finished this training line. Continue from this position or train the line again.";
    const transitionButtons = ["continue_from_here", "restart_line"] as const;
    if (
      stage2TerminalProof.proven &&
      branchCompleteContract.branchCompleteEligible
    ) {
      return {
        render: true,
    ...
    if (
      ...
      const reason =
        continuationPauseDecision.pauseReason === "move_11_hard_stop"
          ? "hard_stop_backup"
          : (continuationPauseDecision.pauseReason ?? "line_complete");
      return {
        render: true,
        title: transitionTitle,
        body: transitionBody,
  ...
  const moveImpactPresentation = useMemo(
    () =>
      presentMoveImpact({
        exactMoveAllowed: Boolean(coachDecision?.exactMoveAllowed),
        engineStatus:
          ...
          (enginePreview?.pvs?.length ? "ready" : "idle"),
        isSafeMove: Boolean(
          (coachDecision?.debug as any)?.coachEngineSafeMoves?.length,
        ),
        isPlayableMove: Boolean(
          (coachDecision?.debug as any)?.coachSelectedCandidateMove,
        ),
        isStudyLineMove: Boolean(
          teachingOrchestration?.moveTrust === "book_supported" ||
            teachingOrchestration?.moveTrust === "repertoire_supported",
        ),
        reviewWorthy: Boolean(coachDecision?.shouldMarkReviewWorthy),
      }),
    [coachDecision, enginePreview, teachingOrchestration],
  );
  const continuationCandidateVisual = useMemo(
    () =>
  ...
  const brainAnalysisForPresentation =
    instructionTarget && currentInstructionFrame
      ? analyzeBlundrPosition({
          fen,
          currentInstructionFrame: currentInstructionFrame,
          frameKey: computeInstructionFrameKey({
            fen,
            trainingMode,
            isUserTurn,
            trainerPhase,
  ...
  const presentationFrame = useMemo(
    () =>
      computeTrainerPresentationFrame({
        frameId: trainerFrameId,
        fen,
        activeBoard,
        trainerView,
        ...
        overlayFrameId,
        playbackReady:
          visualRecipePlayback.animationState === "playing" ||
          visualRecipePlayback.animationState === "held_end_state" ||
          visualRecipePlayback.animationState === "skipped_to_end",
        coachShouldShow: Boolean(coachDecision?.shouldShowCoachCard),
        coachHiddenForFrame,
    ...
    [
      ...
      isUserTurn,
      visualRecipeForRender,
      visualRecipeMainLines,
      safeMoveArrowVisual.lines,
      continuationCandidateVisual.lines,
      legacyVisualLines,
      visualRecipePlayback.activePrimitiveIds,
      visualRecipePlayback.animationState,
      visualRecipeOverlay,
      overlayFrameId,
      coachDecision,
      coachHiddenForFrame,
      coachSurfacePolicy,
      branchTransitionSurface,
      brainAnalysisForPresentation,
      showMoreShown,
    ],
  );
  const displayedCoachDecision = useMemo(() => {
    ...
    const staleCoachFrame = Boolean(
      (coachDecision as any)?.shouldShowCoachCard &&
        ((decisionFrameId && decisionFrameId !== String(trainerFrameId)) ||
          (decisionFen && decisionFen !== normalizedCurrentFen)),
    );
    if (staleCoachFrame) {
      return {
        ...coachDecision,
    ...
    if (presentationFrame.coach.owner !== "branch_transition_surface")
    ...
    return {
      ...
      body:
        ...
        "You finished this training line. Continue from this position or train the line again.",
      buttons:
        (presentationFrame.coach.buttons as CoachButton[]) ??
        (["continue_from_here", "restart_line"] as CoachButton[]),
  ...
  const coachFrameStale = Boolean(
    (displayedCoachDecision?.debug as any)?.staleCoachFrame,
  );
  const visualFrameStale = Boolean(
    trainerPhase === "ready_for_user" &&
      presentationFrame.visual.shouldRender &&
      overlayFrameId !== trainerFrameId,
  );
  const revealTargetStale = Boolean(
    (lastActionDebug as any)?.revealTargetUci &&
      instructionTarget?.uci &&
      (lastActionDebug as any)?.revealTargetUci !== instructionTarget.uci,
  );
  ...
  useEffect(() => {
    ...
    const frameId =
      (displayedCoachDecision as any)?.frameId ??
      coachContextResult.context?.frameId ??
      String(trainerFrameId);
    const normalizedDecisionFen =
      (displayedCoachDecision as any)?.normalizedFen ??
      coachContextResult.context?.normalizedFen ??
      normalizeFen(fen);
    const viewMode =
      coachContextResult.context?.viewMode ??
      (trainerView === "assisted" ? "assisted" : "plain");
    ...
    const blockedUnsafeTemplateIds =
      (displayedCoachDecision.debug as any)?.mappingBlockedReasons ?? [];
    const record: LastCoachRecord = {
      ...
      coachMoveUci:
        (displayedCoachDecision.debug as any)?.coachMoveUci ??
        instructionTarget?.uci ??
        null,
      coachPieceType:
        (displayedCoachDecision.debug as any)?.coachPieceType ??
        instructionTarget?.pieceType ??
        null,
      visualMoveUci:
        presentationFrame.visual.shouldRender &&
        Array.isArray(presentationFrame.visual.lines) &&
        presentationFrame.visual.lines[0]
          ? String((presentationFrame.visual.lines[0] as any).from) +
            String((presentationFrame.visual.lines[0] as any).to)
          : null,
      revealTargetUci:
        (lastActionDebug as any)?.revealTargetUci ??
        instructionTarget?.uci ??
        null,
      selectedTemplateId:
        (displayedCoachDecision.debug as any)?.selectedTemplateId ?? null,
      utteranceFamily: displayedCoachDecision.utteranceFamily ?? null,
      selectedOpportunityId:
        (displayedCoachDecision.debug as any)?.selectedOpportunityId ?? null,
      selectedPlanId:
        (displayedCoachDecision.debug as any)?.selectedPlanId ?? null,
      body: bodyText,
      normalizedBody: normalizeCoachBody(bodyText),
      verifiedClaims:
        (displayedCoachDecision.debug as any)?.verifiedClaims ?? [],
      unverifiedClaims:
        (displayedCoachDecision.debug as any)?.unverifiedClaims ?? [],
      blockedUnsafeTemplateIds: Array.isArray(blockedUnsafeTemplateIds)
        ? blockedUnsafeTemplateIds
        : [],
    };
    const isInstructionalRecord =
      trainerPhase === "ready_for_user" &&
      isUserTurn &&
      Boolean(instructionTarget?.uci);
    if (isInstructionalRecord)
      setLastCoachRecords((prev) => [...prev.slice(-19), record]);
  }, [
  ...
  useEffect(() => {
    const visibleTitle = presentationFrame.coach.shouldRender
      ? String(presentationFrame.coach.title ?? "").trim()
      : displayedCoachDecision?.shouldShowCoachCard
        ? String(displayedCoachDecision?.title ?? "").trim()
        : "";
    const visibleBody = presentationFrame.coach.shouldRender
      ? String(presentationFrame.coach.body ?? "").trim()
      : displayedCoachDecision?.shouldShowCoachCard
    ...
    const entryKind = normalizeCoachEntryKind({
      trainerPhase,
      isUserTurn,
    ...
    const shouldLog = Boolean(
      visibleTitle ||
        ...
        ["opponent_status", "terminal", "line_complete", "error"].includes(
          entryKind,
        ),
    );
    if (!shouldLog) return;
    ...
    const renderedCoachQualityAtFrame = computeRenderedCoachQuality({
      ...
      pipelineTitle: displayedCoachDecision?.shouldShowCoachCard
        ? String(displayedCoachDecision?.title ?? "").trim()
        : null,
      pipelineBody: displayedCoachDecision?.shouldShowCoachCard
        ? String(displayedCoachDecision?.body ?? "").trim()
        : null,
      pipelineSource:
        String(
      ...
      pipelineQualityScore: Number.isFinite(Number(baseQuality?.qualityScore))
        ? Number(baseQuality.qualityScore)
        : null,
      moveSan: instructionTarget?.san ?? null,
      moveUci: instructionTargetUci,
      runtimeSafeFallbackUsed: Boolean(
        (displayedCoachDecision?.debug as any)?.verifiedFallbackUsed ??
          baseQuality?.usedFallback,
      ),
      runtimeSafeFallbackReason:
        String(
          (displayedCoachDecision?.debug as any)?.fallbackReason ??
            baseQuality?.fallbackReason ??
            "",
        ).trim() || null,
      recentRenderedBodies: coachTimeline
        .slice(-8)
        .map((entry: any) => String(entry?.visibleBody ?? "")),
    });
    const normalizedDebug = normalizeCoachDebugMetadata({
      ...
      coachQuality: {
        ...
        selectedTheme:
          baseQuality?.selectedTheme ??
          (displayedCoachDecision?.debug as any)?.selectedTheme ??
          null,
        evidenceTags: Array.isArray(baseQuality?.evidenceTags)
          ? baseQuality.evidenceTags
          : [],
      },
    });
    ...
    const nextEntry: CoachSessionLogEntry = {
      ...
      selectedOpportunityLayer:
        normalizedDebug.selectedOpportunityLayer ?? null,
      selectedOpportunityScore:
        typeof normalizedDebug.selectedOpportunityScore === "number"
          ? normalizedDebug.selectedOpportunityScore
          : null,
      ...
      containsDebugLeak: Boolean(
        normalizedDebug.coachQuality?.containsDebugLeak ||
          isDebugLeakText(visibleBody || ""),
      ),
      qualityScore:
        typeof normalizedDebug.coachQuality?.qualityScore === "number"
          ? normalizedDebug.coachQuality.qualityScore
          : null,
      hasPedagogicalReason: Boolean(
        normalizedDebug.coachQuality?.hasPedagogicalReason,
      ),
      repeatedGeneric: Boolean(normalizedDebug.coachQuality?.repeatedGeneric),
      targetAligned: instructionTargetUci
        ? Boolean(normalizedDebug.coachQuality?.targetAligned)
        : "not_applicable",
      pieceAligned: instructionTarget?.pieceType
        ? Boolean(normalizedDebug.coachQuality?.pieceAligned)
        : "not_applicable",
    ...
    setCoachTimeline((prev) => {
      ...
      if (
        last &&
        last.trainerFrameId === nextEntry.trainerFrameId &&
        last.visibleTitle === nextEntry.visibleTitle &&
        last.visibleBody === nextEntry.visibleBody
      )
        return prev;
    ...
    (displayedCoachDecision?.debug as any)?.fallbackReason,
    coachTimeline
      .map((entry) => String((entry as any)?.visibleBody ?? ""))
      .join("|"),
  ...
  function recordDebugAction(input: {
    action: string;
    ...
    const renderedFromSurface = visibleTeachingSurface?.coach?.shouldRender
      ? (visibleTeachingSurface.actions as any)
      : null;
    const effectiveRenderedButtons =
      renderedFromSurface || displayedCoachButtons;
    const details = {
      ...
      revealExpectedMoveTriggered:
        input.normalizedAction === "answer" ||
        input.normalizedAction === "reveal_next_move",
      revealCandidateTriggered: input.normalizedAction === "show_move",
      candidateAnalysisTriggered: input.normalizedAction === "analyze_idea",
      stateChanged,
      frameId: Number(trainerFrameId),
      revealTargetUci:
        input.extra?.revealTargetUci ??
        (input.normalizedAction.includes("reveal") ||
        input.normalizedAction === "answer"
          ? (instructionTarget?.uci ?? null)
          : null),
      revealTargetSource:
        input.extra?.revealTargetSource ??
        (instructionTarget ? "instruction_target" : "none"),
      revealIdempotentNoop: Boolean(input.extra?.revealIdempotentNoop),
      revealBlockedBecauseCoachHidden: Boolean(
        input.extra?.revealBlockedBecauseCoachHidden,
      ),
      ...input.extra,
    };
    setLastActionDebug(details);
    setDebugEventLog((events) =>
      appendDebugEvent(events, {
        type: "coach_action_clicked",
        action: input.action,
        normalizedAction: input.normalizedAction,
        before: input.before,
        after: input.after,
    ...
    setActionTimeline((prev) => {
      const renderedActionIds = effectiveRenderedButtons.map((value: any) =>
        String(value),
      );
      const actionEntryKey = [
        ...
        String(
          (input.after as any)?.coachInteraction ?? coachInteraction ?? "none",
        ),
        Boolean((input.after as any)?.answerShown ?? showAnswer) ? "1" : "0",
        Boolean((input.after as any)?.hintShown ?? coachHintRequestCount > 0)
          ? "1"
          : "0",
      ...
      const next = {
        ...
        resultingCoachInteraction:
          (input.after as any)?.coachInteraction ?? coachInteraction,
        resultingAnswerShown: Boolean(
          (input.after as any)?.answerShown ?? showAnswer,
        ),
        resultingHintShown: Boolean(
          (input.after as any)?.hintShown ?? coachHintRequestCount > 0,
        ),
  ...
  function handleCoachAction(button: CoachButton | string) {
    ...
    if (button === "continue_from_here") {
      const after = {
        ...before,
        coachInteraction: "continue_from_here",
        showAnswer: false,
      };
      ...
      const continuationUserTurn =
        !terminalDetected && current.turn() === userColor;
      ...
      recordDebugAction({
        ...
        reason: terminalDetected
          ? "user_continue_from_here_terminal"
          : "user_continue_from_here",
        extra: {
          continueFromHereClicked: true,
          terminalDetected,
          continuationTerminalReason: terminalDetected
            ? current.isCheckmate?.()
              ? "checkmate"
              : "no_legal_moves"
            : null,
          candidateAnalysisTriggered: continuationUserTurn,
          continuationRuntimeStatus: terminalDetected
            ? "terminal"
            : continuationUserTurn
              ? "analyzing"
              : "opponent_replying",
        },
      });
    ...
    if (button === "finish") {
      const after = { ...before, coachInteraction: "none", showAnswer: false };
      setActiveTab("home");
    ...
    if (!internalWhitelist.includes(String(button)) && !allowedBySurface) {
      recordDebugAction({
        action: button,
        normalizedAction: String(button),
        before,
        after: before,
        result: "blocked",
        reason: "action_not_in_current_visible_surface",
      });
      return;
    }
    if (button === "replay") {
      visualRecipePlayback.replay();
      recordDebugAction({
        action: button,
        ...
        after: before,
        result: "handled",
        reason: "playback_replay_requested",
      });
      return;
    }
    if (button === "restart_line") {
      const after = {
        ...before,
        coachInteraction: "replay",
        showAnswer: false,
        hintShown: false,
      };
      recordDebugAction({
        action: button,
        normalizedAction: "restart_line",
        before,
        after,
        result: "handled",
        reason: "user_restart_line",
      });
      resetBoard();
      return;
    }
    if (button === "hide") {
      const after = { ...before, coachInteraction: "hide" };
      setCoachHiddenFrameId(String(trainerFrameId));
      setCoachInteraction("hide");
      recordDebugAction({
        action: button,
    ...
    if (button === "hint") {
      const after = { ...before, hintShown: true, coachInteraction: "hint" };
      setCoachHintRequestCount((count) => count + 1);
      setCoachInteraction("hint");
      recordDebugAction({
        action: button,
        normalizedAction: "hint",
        before,
        after,
        result: "handled",
      });
    ...
    if ((button as any) === "hide_more") {
      const after = {
        ...before,
        coachInteraction: "hide",
        showMoreShown: false,
      };
    ...
    if (button === "show_more" || (button as any) === "show_more") {
      const after = {
        ...before,
        showMoreShown: true,
        coachInteraction: "show_plan",
      };
      setShowMoreShown(true);
      setCoachInteraction("show_plan");
      recordDebugAction({
        action: button,
        normalizedAction: "show_more",
        before,
        after,
        result: "handled",
        reason: "plain_show_more_escalation_to_full_content",
      });
      return;
    }
    if ((button as any) === "reveal_target") {
      const after = {
        ...before,
        answerShown: true,
        showAnswer: true,
      ...
      recordDebugAction({
        action: String(button),
        normalizedAction: "reveal_target",
        before,
        after,
        result: "handled",
      });
    ...
    if (button === "why") {
      const after = { ...before, coachInteraction: "why" };
      setCoachInteraction("why");
      recordDebugAction({
        action: button,
        normalizedAction: "why",
        before,
        after,
        result: "handled",
      });
    ...
    if (button === "show_plan") {
      const after = { ...before, coachInteraction: "show_plan" };
      setCoachInteraction("show_plan");
      recordDebugAction({
        action: button,
        normalizedAction: "show_plan",
        before,
        after,
        result: "handled",
      });
    ...
    if (button === "analyze_idea") {
      const after = { ...before, coachInteraction: "analyze_idea" };
      setCoachInteraction("analyze_idea");
      recordDebugAction({
        action: button,
        normalizedAction: "analyze_idea",
        before,
        after,
        result: "handled",
        reason:
          (bookComplete || trainingMode === "continuation") && isUserTurn
            ? "analysis_requested"
            : "state_only",
      });
      if ((bookComplete || trainingMode === "continuation") && isUserTurn)
        void runBrain("coach_analyze", { skipGpt: true });
      return;
    }
    if (button === "show_move") {
      if (!coachDecision?.exactMoveAllowed) {
        recordDebugAction({
          action: button,
          normalizedAction: "show_move",
          before,
          after: before,
          result: "blocked",
          reason: "exact_move_not_allowed",
        });
        return;
      }
      const after = {
  ...
  const intendedCoachMoveUci =
    (displayedCoachDecision?.debug as any)?.coachMoveUci ??
    (rawCoachDecision?.debug as any)?.coachMoveUci ??
    instructionTarget?.uci ??
    null;
  const intendedVisualMoveUci =
    presentationFrame?.visual?.shouldRender &&
    Array.isArray(presentationFrame.visual.lines) &&
    presentationFrame.visual.lines[0]
      ? `${(presentationFrame.visual.lines[0] as any).from}${(presentationFrame.visual.lines[0] as any).to}`
      : null;
  const intendedShowMoreTargetUci = showMoreShown
    ? (instructionTarget?.uci ?? null)
    : null;
  const intendedCoachPieceType =
    (displayedCoachDecision?.debug as any)?.coachPieceType ??
    instructionTarget?.pieceType ??
    null;
  ...
  const legacyVisibleTeachingSurface = buildVisibleTeachingSurface({
    ...
    legacyCoachDecision: isActiveTeachingFrame
      ? null
      : (coachDecision ??
        rawCoachDecision ??
        (liveCoachState as any)?.coach ??
        null),
    ...
    selectedTeachingConcept:
      (brainAnalysisForSurface as any)?.pedagogicalFocus?.focus ?? null,
  ...
  const v28VisibleSurface =
    v28VisibleSurfaceEnabled && currentInstructionFrame
      ...
      : null;
  const v28SurfaceActive = Boolean(
    v28VisibleSurfaceEnabled && v28VisibleSurface,
  );

  const v28CoachUiModel = v28VisibleSurface
    ? adaptVisibleSurfaceToCoachUi(v28VisibleSurface)
    : null;
  const v28BoardVisualUiModel = v28VisibleSurface
    ? adaptVisibleSurfaceToBoardVisuals(v28VisibleSurface)
    : null;
  const v28ActionKinds =
    v28CoachUiModel?.actions.map((action) => action.kind) ?? [];
  const plainBeforeShowMore =
    v28VisibleSurface?.mode === "plain_before_show_more";
  const plainHintShown = coachHintRequestCount > 0;
  const v28VisualLines: ActiveLine[] = (
    v28BoardVisualUiModel?.visualRecipes ?? []
  )
  ...
  const visibleTeachingSurface = v28VisibleSurface
    ? ({
        ...
        coach: {
          ...
          body: v28CoachUiModel?.body ?? null,
          suppressedReason:
            v28VisibleSurface.mode === "blocked" ? "blocked_surface" : null,
        },
        hint: {
          text:
            plainBeforeShowMore && plainHintShown
              ? (v28CoachUiModel?.body ?? null)
              : null,
        ...
        visual: {
          ...
          blockedReason:
            v28VisualLines.length > 0 ? null : "no_surface_visuals",
        },
        actions: v28ActionKinds,
        safety: {
          blocked: Boolean(
            v28VisibleSurface.safety.blocked ??
              !v28VisibleSurface.safety.allowed,
          ),
          reason:
            v28VisibleSurface.safety.blockedReason ??
            v28VisibleSurface.safety.criticalIssues[0] ??
            null,
          blockedSeverity: v28VisibleSurface.safety.blockedSeverity ?? null,
          blockedPolicy: v28VisibleSurface.safety.blockedPolicy ?? null,
          targetMismatch:
            Boolean(v28VisibleSurface.safety.targetMismatch) ||
            v28VisibleSurface.safety.criticalIssues.includes("target_mismatch"),
          pieceMismatch:
            v28VisibleSurface.safety.criticalIssues.includes("piece_mismatch"),
          visualMismatch: Boolean(
            (v28VisibleSurface.safety as any).visualMismatch,
          ),
          revealMismatch: Boolean(
            (v28VisibleSurface.safety as any).revealMismatch,
          ),
          unsupportedStrongClaim: Boolean(
            (v28VisibleSurface.safety as any).unsupportedStrongClaim,
          ),
          legacyBypassDetected: false,
          plainLeakDetected:
            Boolean(v28VisibleSurface.safety.plainLeakDetected) ||
            v28VisibleSurface.safety.criticalIssues.includes("plain_leak"),
          recoveredBySafeTeachingCopy: Boolean(
            v28VisibleSurface.safety.recoveredBySafeTeachingCopy,
          ),
        },
        debug: {
  ...
  if (
    isActiveTeachingFrame &&
    validatedContinuationCandidate?.isEmergencyLegalFallback
  ) {
  ...
  const pipelineCoachBody = displayedCoachDecision?.shouldShowCoachCard
    ? String(displayedCoachDecision.body ?? "").trim()
    : "";
  const pipelineCoachQuality =
    (displayedCoachDecision?.debug as any)?.coachQuality ?? {};
  const pipelineSafetyPassed =
    !Array.isArray((displayedCoachDecision?.debug as any)?.unverifiedClaims) ||
    ((displayedCoachDecision?.debug as any)?.unverifiedClaims as unknown[])
      .length === 0;
  const stage2CoachRenderState = resolveStage2CoachRenderState({
    openingId:
      runtimeBookFrameQuery.openingId ?? runtimeOpeningIdForFrame ?? undefined,
    playKeyBefore:
      runtimeBookFrameQuery.playKeyBefore ??
      runtimePlayKeyBeforeForFrame ??
      undefined,
    ...
    runtimeBookTopCandidateTotalGames:
      runtimeBookFrameQuery.candidates[0]?.totalGames,
    runtimeBookBookExhausted: runtimeBookFrameQuery.bookExhausted,
    plainRevealState:
      v28VisibleSurface?.mode === "plain_before_show_more"
        ? plainHintShown
          ? "hint"
          : "hidden"
        : v28VisibleSurface?.mode === "plain_after_show_more"
          ? "show_more"
          : "revealed",
    trainerPhase,
    isUserTurn,
    ...
    surfaceCopy: {
      title:
        v28CoachUiModel?.title ??
        convergedVisibleSurface.coach.title ??
        "Training move",
      body:
        v28CoachUiModel?.body ??
        convergedVisibleSurface.coach.body ??
        convergedVisibleSurface.hint.text ??
        "",
      bullets: v28CoachUiModel?.bullets ?? [],
    },
    pipelineCopy: {
    ...
    pipelineContainsDebugLeak: Boolean(
      pipelineCoachQuality?.containsDebugLeak ||
        isDebugLeakText(pipelineCoachBody),
    ),
    pipelinePassedSafety: pipelineSafetyPassed,
  });
  const stage2CoachContext = stage2CoachRenderState.stage2CoachContext;
  const stage2CoachingPacketResolution =
    stage2CoachRenderState.stage2CoachingPacketResolution;
  const pipelineCopyAuthorityDecision =
    stage2CoachRenderState.pipelineCopyAuthorityDecision;
  const stage2CoachCopyEnrichment =
    stage2CoachRenderState.stage2CoachCopyEnrichment;
  const coachCardTitleFromSurface =
    plainBeforeShowMore && !plainHintShown
      ? "Find the next move"
  ...
  const coachCardButtonsFromSurface = (
    visibleSurfaceActionKinds.length > 0
      ? visibleSurfaceActionKinds
      : convergedVisibleSurface.actions
  ) as any;
  ...
  const renderedCoachQualityForDebug = useMemo(() => {
    const baseQuality =
      (displayedCoachDecision?.debug as any)?.coachQuality ?? {};
    const renderedTitle = surfaceCoachCardDecision?.shouldShowCoachCard
      ? String(surfaceCoachCardDecision.title ?? "").trim() || null
      : null;
    const renderedBody = surfaceCoachCardDecision?.shouldShowCoachCard
      ? String(surfaceCoachCardDecision.body ?? "").trim() || null
      : null;
    const pipelineTitle = displayedCoachDecision?.shouldShowCoachCard
      ? String(displayedCoachDecision.title ?? "").trim() || null
      : null;
    const pipelineBody = displayedCoachDecision?.shouldShowCoachCard
      ? String(displayedCoachDecision.body ?? "").trim() || null
      : null;
    const recentRenderedBodies = coachCardRenderTimeline
      ...
      .slice(-8)
      .map((entry) =>
        String(entry.actualCoachCardBody ?? entry.visibleBody ?? ""),
      );
  ...
  const selectedLegalMoves =
    selectedSquare &&
    boardSettings.showMoveDots &&
    isUserTurn &&
    !isReviewingHistory &&
    !game.isGameOver()
      ? (game.moves({ square: selectedSquare as any, verbose: true }) as any[])
      : [];
  ...
  const projectiveTacticsVisualPriorityActive =
    projectiveTacticDisplay.shouldRender && !projectiveTacticsAllowNextVisuals;
  const gptDebugText = JSON.stringify(
    {
      pipeline: brainResponse?.pipeline ?? null,
      engine: enginePreview ?? null,
      moveQuality,
      moveQualityPending,
      shouldValidateTrainingMove,
      debug: brainResponse?.debug ?? null,
    },
    null,
    2,
  );
  const visualDebugText = JSON.stringify(visualDebugSnapshot, null, 2);
  const telemetryDebugText = JSON.stringify(
    telemetryEvents.slice(-30),
    null,
    2,
  );
  const visualModelRequestKey = useMemo(
    () =>
      JSON.stringify({
    ...
    [
      ...
      isUserTurn,
      userColor,
      expectedUserOptions.map((m) => m.uci).join("|"),
      engineLines[0]?.uci,
      engineLines[0]?.cp,
      repertoire.name,
      rating.label,
  ...
  function createTrackedLearningEvent(
    ...
    return recordLearningEvent({
      ...
      moveOrderKey:
        trainingMode === "restricted"
          ? (runtimePlayKeyBeforeForFrame ?? undefined)
          : undefined,
      trainerView,
      trainingMode,
  ...
  function trackLearningEvent(
    ...
    if (shouldPersistRemoteLearningEvent(event)) {
      void persistLearningEventRemotely(event).catch(() => {
        setFeedback(
          "Your move was not credited because secure progress storage is unavailable. Check your connection and try again.",
        );
        pushRuntimeCriticalIssue("learning_event_persistence_unavailable");
      });
    }
  }
  async function persistTrainerOutcome(
    key: string,
    input: Partial<LearningEvent> & Pick<LearningEvent, "type" | "source">,
  ) {
    ...
    const result = await trainerLearningPersistenceGateRef.current.persist(
      ...
      async (event) => {
        if (trainingMode !== "restricted")
          return persistLearningEventRemotely(event);
        ...
        if (!session || session.terminal)
          throw new Error("trainer_session_not_ready");
        const committed = await commitAuthoritativeTrainerAction({
          session,
          event,
        });
    ...
    if (result.status === "in_flight") {
      setFeedback("Saving this training result. Please wait.");
      return false;
    }
  ...
  function pushRuntimeCriticalIssue(issue: string) {
    setRuntimeCriticalIssues((prev) =>
      prev.includes(issue) ? prev : [...prev.slice(-19), issue],
    );
  }
  ...
  function clearPendingOpponentReplyRequest(options?: {
    ...
    if (options?.clearStaleIssue && (hadPendingRef || hadPendingState))
      clearRuntimeCriticalIssue("stale_opponent_reply_commit");
    return true;
  }
  function clearProjectiveTacticTimers() {
    if (projectiveTacticFadeTimeoutRef.current !== null) {
      window.clearTimeout(projectiveTacticFadeTimeoutRef.current);
      projectiveTacticFadeTimeoutRef.current = null;
    }
    if (projectiveTacticClearTimeoutRef.current !== null) {
  ...
  function clearProjectiveTacticOverlay(reason: string) {
    if (
      !shouldClearProjectiveTacticsForBoardReset(reason) &&
      reason !== "replace"
    )
      return;
    clearProjectiveTacticTimers();
    projectiveTacticTokenRef.current = nextProjectiveTacticToken(
      projectiveTacticTokenRef.current,
    );
  ...
  function showProjectiveTacticOverlay(visuals: ProjectiveTacticVisual[]) {
    ...
    if (!filtered.length || !display.shouldRender) {
      clearProjectiveTacticOverlay(
        PROJECTIVE_TACTICS_ENABLED ? "replace" : "feature_disabled",
      );
      return;
    }
    clearProjectiveTacticTimers();
    const token = nextProjectiveTacticToken(projectiveTacticTokenRef.current);
    ...
    projectiveTacticNextVisualsTimeoutRef.current = window.setTimeout(() => {
      if (isStaleProjectiveTacticToken(projectiveTacticTokenRef.current, token))
        return;
      setProjectiveTacticsAllowNextVisuals(true);
      projectiveTacticNextVisualsTimeoutRef.current = null;
    }, PROJECTIVE_TACTIC_NEXT_VISUALS_ALLOWED_AFTER_MS);
    projectiveTacticFadeTimeoutRef.current = window.setTimeout(
      () => {
        if (
          isStaleProjectiveTacticToken(projectiveTacticTokenRef.current, token)
        )
          return;
    ...
    projectiveTacticClearTimeoutRef.current = window.setTimeout(() => {
      if (isStaleProjectiveTacticToken(projectiveTacticTokenRef.current, token))
        return;
      setProjectiveTacticVisuals([]);
      setProjectiveTacticsFading(false);
  ...
  function maybeShowProjectiveTacticsAfterMove(input: {
    nextFen: string;
    lastMoveUci: string;
    movedColor: ChessColor;
  }) {
    if (!PROJECTIVE_TACTICS_ENABLED || trainerView !== "assisted") {
      clearProjectiveTacticOverlay(
        PROJECTIVE_TACTICS_ENABLED ? "plain_view" : "feature_disabled",
      );
      return;
    }
    try {
  ...
  function commitRuntimeFrame(input: {
    ...
    if (input.clearPendingOpponentRequest)
  ...
  function scheduleOpponentReply(input: {
    ...
    const restrictedRuntimeLineSnapshot =
      input.restrictedRuntimeLineSnapshot ??
    ...
    if (input.mode === "restricted" && !restrictedRuntimeLineSnapshot) {
      clearPendingOpponentReplyRequest({ clearStaleIssue: true });
      pushRuntimeCriticalIssue(
        "restricted_opponent_reply_missing_runtime_authority",
      );
      return null;
    }
    ...
    if (
      currentPending &&
      currentPending.baseFen === baseFenNormalized &&
      currentPending.mode === input.mode
    ) {
      clearRuntimeCriticalIssue("stale_opponent_reply_commit");
    ...
    opponentReplyTimeoutRef.current = window.setTimeout(
      () => {
        if (pendingOpponentRequestRef.current?.requestId !== request.requestId)
          return;
  ...
  useEffect(() => {
    ...
    if (savedCustom)
      try {
        setCustomRepertoires(JSON.parse(savedCustom));
      } catch {}
    if (savedSettings)
      try {
    ...
    if (savedTelemetry)
      try {
        const parsed = JSON.parse(
          savedTelemetry,
        ) as Partial<LocalTelemetryStore>;
        const nextEvents = Array.isArray(parsed.events)
          ? parsed.events.slice(-MAX_LOCAL_TELEMETRY_EVENTS)
          : [];
        setTelemetryEnabled(Boolean(parsed.enabled));
        setTelemetryEvents(nextEvents);
        telemetryEventsRef.current = nextEvents;
        telemetrySeq.current = nextEvents.reduce(
  ...
  useEffect(() => {
    try {
      const savedRuntimeLineMemory = localStorage.getItem(
        STAGE2_RUNTIME_TRAINING_LINE_MEMORY_KEY,
      );
      if (!savedRuntimeLineMemory) return;
      const parsed = normalizeRuntimeTrainingLineKeys(
        JSON.parse(savedRuntimeLineMemory),
      );
  ...
  useEffect(
    () => localStorage.setItem("blundr-v22-progress", JSON.stringify(progress)),
    [progress],
  );
  useEffect(
    () =>
      localStorage.setItem(
        "blundr-v22-custom",
        JSON.stringify(customRepertoires),
      ),
    [customRepertoires],
  );
  useEffect(() => {
    ...
    const nextPreferences: BlundrBoardPreferences = {
      ...currentPreferences,
      boardThemeId:
        normalizeHomeBoardTheme(boardSettings.boardTheme) === "blue"
          ? "blue"
          : normalizeHomeBoardTheme(boardSettings.boardTheme) === "walnut"
            ? "walnut"
            : "default",
  ...
  useEffect(
    () =>
      localStorage.setItem(
        STAGE2_RUNTIME_TRAINING_LINE_MEMORY_KEY,
        JSON.stringify(recentRuntimeTrainingLineKeys.slice(0, 2)),
      ),
    [recentRuntimeTrainingLineKeys],
  );
  useEffect(() => {
  ...
  useEffect(() => {
    telemetryEventsRef.current = telemetryEvents;
  }, [telemetryEvents]);
  useEffect(() => {
    branchCompleteLatchRef.current = branchCompleteLatch;
  }, [branchCompleteLatch]);
  useEffect(() => {
    continuationCandidateLockRef.current = continuationCandidateLock;
  }, [continuationCandidateLock]);
  useEffect(() => {
    lastCoachRecordsRef.current = lastCoachRecords;
  }, [lastCoachRecords]);
  useEffect(() => {
    coachTimelineRef.current = coachTimeline;
  }, [coachTimeline]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (
      existing &&
      existing.continuationCandidateLockFen4 === fen4 &&
      existing.continuationCandidateLockUci ===
        validatedContinuationCandidate.uci
    ) {
      return;
    }
    ...
    const lock = {
      ...
      continuationCandidateLockSan:
        ...
        validatedContinuationCandidate.uci,
      continuationCandidateLockSource: String(
        validatedContinuationCandidate.source ?? "continuation_policy",
      ),
      continuationCandidateLockReason: String(
        validatedContinuationCandidate.reason ?? "candidate_locked",
      ),
    };
    ...
    validatedContinuationCandidate?.san,
    validatedContinuationCandidate?.source,
    validatedContinuationCandidate?.reason,
    fen,
  ]);
  useEffect(() => {
    ...
    if (
      existing.active &&
      existing.lineId === selectedRuntimeLineId &&
      existing.fen4 === fen4
    )
      return;
  ...
  useEffect(() => {
    ...
    branchCompleteShouldCancelPending,
    pendingOpponentRequest?.requestId,
    trainerPhase,
  ]);
  useEffect(() => {
    if (!pendingOpponentRequest) return;
    const pendingMatchesBoard =
      normalizeFen(fen) === pendingOpponentRequest.baseFen;
    if (
      trainerPhase === "opponent_replying" &&
      !isUserTurn &&
      pendingMatchesBoard
  ...
  useEffect(() => {
    const store: LocalTelemetryStore = {
      enabled: telemetryEnabled,
      events: telemetryEvents.slice(-MAX_LOCAL_TELEMETRY_EVENTS),
      updatedAt: Date.now(),
    };
    localStorage.setItem(LOCAL_TELEMETRY_KEY, JSON.stringify(store));
  }, [telemetryEnabled, telemetryEvents]);
  useEffect(() => {
    const api = {
    ...
    return () => {
      if ((window as any).__blundrLocalTelemetry === api)
        delete (window as any).__blundrLocalTelemetry;
    };
  }, [visualDebugSnapshot]);
  useEffect(() => {
    const t = window.setInterval(() => {
      if (opponentCue && Date.now() > opponentCue.expiresAt)
        setOpponentCue(null);
    }, 250);
  ...
  useEffect(() => {
    if (
      ...
      setOverlayClearedOnPhaseChange(false);
  ...
  useEffect(
    () =>
      setBrain((p) => ({
        ...
        ratingPool: rating.target,
      })),
    [rating.label, rating.target],
  );
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fen4 = normalizeFen(fen);
    if (
      continuationCandidateLockRef.current &&
      continuationCandidateLockRef.current.continuationCandidateLockFen4 !==
        fen4
    ) {
  ...
  useEffect(() => {
    ...
    if (
      enginePreview &&
      normalizeFen(enginePreview.fen) === normalized &&
      enginePreview.pvs.length > 0
    ) {
      setContinuationAnalysisStatus("ready");
      return;
    ...
    continuationAnalysisDebounceRef.current = window.setTimeout(() => {
      ...
      void runBrowserStockfish(fen, rating.skill, 900, 10, controller.signal)
        .then((result) => {
          if (controller.signal.aborted) return;
  ...
  useEffect(() => {
    if (activeTab === "train") positionStartedAtRef.current = Date.now();
  }, [fen, activeTab]);
  useEffect(() => {
    setCoachInteraction("none");
  ...
  }, [fen, trainerFrameId, trainerView, trainerPhase]);
  useEffect(() => {
    if (!enabledViews.includes(activeBoardView) && enabledViews.length)
      setActiveBoardView(enabledViews[0]);
  }, [activeBoardView, enabledViews.join("|")]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (activeTab !== "train" || !shouldValidateTrainingMove || !moveQuality)
      return;
  ...
  useEffect(() => {
    ...
    const metaBoolean = (key: string) =>
      typeof meta[key] === "boolean" ? Boolean(meta[key]) : false;
  ...
  useEffect(() => {
    ...
    const continuationEnginePending =
      trainingMode === "continuation" &&
      requestGame.turn() === userColor &&
      !engineLines[0];
    if (requestGame.isGameOver()) {
      setVisualModelPending(false);
      setVisualModelError(null);
      return;
    }
    if (continuationEnginePending) {
      setVisualModelPending(false);
      setVisualModelError(null);
      return;
    }
    ...
    const payload = {
      ...
      trainingPhase,
      trainingMode,
      expectedMove: expectedUserOptions[0]
        ? { san: expectedUserOptions[0].san, uci: expectedUserOptions[0].uci }
        : undefined,
      expectedMoves: expectedUserOptions.map((m) => ({
        san: m.san,
        uci: m.uci,
      })),
      bookStatus: bookComplete
        ...
        : expectedUserOptions.length
          ...
          : "pending",
      openingName: repertoire.name,
      stockfishSummary: engineLines[0]
        ? {
            bestMove: { san: engineLines[0].san, uci: engineLines[0].uci },
            pvs: engineLines,
          }
        : undefined,
      coachingMemory: {
        conceptSeenCount: progress.trainedPositions[normalizeFen(requestFen)]
          ? 1
          : 0,
        missedCount: progress.mistakes[normalizeFen(requestFen)]?.count ?? 0,
        successCount: progress.trainedPositions[normalizeFen(requestFen)]
          ? 1
          : 0,
      },
    };
    setVisualDebugSnapshot({
      requestKey: visualModelRequestKey,
    ...
    recordLocalTelemetry("visual_request", {
      requestKey: visualModelRequestKey,
      fen: normalizeFen(requestFen),
      trainingPhase,
      trainingMode,
      bookStatus: payload.bookStatus,
      hasExpectedMove: Boolean(expectedUserOptions[0]),
      hasEngine: Boolean(engineLines[0]),
    });
    if (controller.signal.aborted) return;
    if (
      requestSeq !== visualRequestSeq.current ||
      normalizeFen(fenRef.current) !== normalizeFen(requestFen)
    ) {
      setStaleOverlayIgnored(true);
      return;
    }
    void fetch("/api/blundr-visual-model", {
      method: "POST",
      ...
      .then(async (res) => {
        if (!res.ok) throw new Error(`visual model ${res.status}`);
        return (await res.json()) as VisualModelOutput;
      })
      .then((data) => {
        if (
          requestSeq !== visualRequestSeq.current ||
        ...
        const suppress = Array.isArray(data.suppress) ? data.suppress : [];
        const responseSummary = {
          source: typeof data.source === "string" ? data.source : null,
          fallback: Boolean(data.fallback),
          suppress,
          arrowCount: Array.isArray(data.arrows) ? data.arrows.length : 0,
          squareCount: Array.isArray(data.squares) ? data.squares.length : 0,
          animation: data.animationPackage?.name ?? data.animation ?? null,
          contextHeadline: data.context?.headline ?? null,
        };
        setVisualDebugSnapshot((prev) => ({
          ...prev,
          responseSummary,
        ...
        setVisualModelOutput(data);
        setVisualModelPending(false);
        recordLocalTelemetry("visual_response", {
          requestKey: visualModelRequestKey,
          fen: normalizeFen(requestFen),
          durationMs,
          source: responseSummary.source,
          fallback: responseSummary.fallback,
          suppressed: suppress.includes("recommendation_pending"),
          arrowCount: responseSummary.arrowCount,
          squareCount: responseSummary.squareCount,
        });
        if (suppress.includes("recommendation_pending"))
          recordLocalTelemetry("visual_suppressed", {
            requestKey: visualModelRequestKey,
            fen: normalizeFen(requestFen),
      ...
      .catch((error) => {
        ...
        setVisualModelError(message);
        setVisualModelPending(false);
        recordLocalTelemetry("visual_error", {
          requestKey: visualModelRequestKey,
          fen: normalizeFen(requestFen),
          message,
        });
  ...
  useEffect(() => {
    if (activeTab !== "train") return;
    const fast = deriveFastAnnotation({
      fen,
      openingName: repertoire.name,
      userColor,
      trainingMode,
    ...
    setAnnotation(fast);
    setVisualReady(true);
    setThinkingStep("ready");
    setPipelineNote(
      trainingMode === "continuation" && isUserTurn
        ? continuationAnalysisStatus === "analyzing"
  ...
  useEffect(() => {
    ...
    if (moveQualityPending) {
      setPipelineNote("Blundr Brain is checking the position.");
      return;
    }
    if (
      moveQuality?.status === "verified_top1" ||
    ...
    if (moveQuality?.status === "rejected") {
      setPipelineNote(
        "Saved line needs review. Blundr will not invent a teaching cue.",
      );
      return;
    }
    if (moveQuality?.status === "unavailable") {
      setPipelineNote(
        "Move not verified. Blundr will stay quiet instead of guessing.",
      );
    }
  }, [
    activeTab,
  ...
  useEffect(() => {
    if (
      ...
      setFeedback(
        "Guided line complete. Continue from here against the bot, or restart the opening.",
      );
      setBrain((p) => ({
        ...p,
        book: "complete",
        source: guidedCoveragePolicy.guidedCoverageState,
        gpt: p.gpt,
      }));
    }
  }, [
    ...
    expectedMoveResolution.shouldTransitionToContinuation,
    guidedCoveragePolicy.guidedCompleteAllowed,
    guidedCoveragePolicy.guidedCoverageState,
    bookComplete,
    fen,
  ]);
  useEffect(() => {
    const restrictedLineCompleted = isTempoCompletionEligible({
      trainingMode,
      bookComplete,
      branchCompleteEligible: branchCompleteEligibleNow,
      terminalProof: stage2TerminalProof.proven,
    });
    const terminalCompletionId =
      authoritativeTrainerSession?.terminalCompletionId ?? null;
    if (
      activeTab !== "train" ||
      !restrictedLineCompleted ||
      !authoritativeTrainerSession?.terminal ||
    ...
    selectedRepertoireId,
    authoritativeTrainerSession?.terminal,
    authoritativeTrainerSession?.terminalCompletionId,
    fen,
    repertoireProgress,
    onboardingProfile,
  ]);
  ...
  useEffect(() => {
    ...
    if (expectedMoveResolution.source === "guided_branch_needs_continuation") {
      setFeedback(
        "This branch is beyond the guided line. Continue from here, or restart the opening.",
      );
      setBrain((p) => ({
        ...p,
        book: "ready",
        source: "guided branch needs continuation",
        note: expectedMoveResolution.reason,
        gpt: p.gpt,
      }));
      return;
    }
    if (
      ...
      setFeedback(
        "This branch is not mapped yet. Continue from here, or restart the opening.",
      );
      setBrain((p) => ({
        ...p,
        book: "ready",
        source: "resolver unresolved",
        note:
          guidedCoveragePolicy.bookCompleteBlockedReason ??
          expectedMoveResolution.reason,
        gpt: p.gpt,
      }));
    }
    ...
    expectedMoveResolution.reason,
    guidedCoveragePolicy.bookCompleteBlockedReason,
    guidedCoveragePolicy.guidedCompleteBlockedReason,
    bookComplete,
    fen,
    game.isGameOver(),
  ]);
  useEffect(() => {
    ...
    const handoff = resolveBlackOpeningInitialOpponentHandoff({
      ...
      selectedOpeningId: canonicalSelectedRepertoireId,
      selectedRuntimeLineOpeningId:
        selectedRuntimeTrainingLineSelection?.openingId ?? null,
      selectedRuntimeLineKey,
      ...
      pendingOpponentRequestExists: Boolean(
        pendingOpponentRequest || pendingOpponentRequestRef.current,
      ),
  ...
  useEffect(() => {
    ...
    if (game.isGameOver()) {
      ...
      clearPendingOpponentReplyRequest({ clearStaleIssue: true });
      setFeedback(
        (endingInfo?.title ?? "Game over") +
          ". Restart the opening to train again.",
      );
      return;
    }
  ...
  async function loadExplorer(positionFen: string) {
    ...
    if (explorerCache.current[cacheKey]) {
      ...
      setBrain((p) => ({
        ...p,
        lichess: "cached",
        note: "Local runtime package cache",
      }));
      return parsed;
    }
    ...
    const localRows = currentOpeningNodes
      .flatMap((node) => node.continuations)
      .flatMap((move) => {
        if (seenUcis.has(move.uci)) return [];
        seenUcis.add(move.uci);
        const total = currentOpeningNodes.filter((openingNode) =>
          openingNode.continuations.some(
            (continuation) => continuation.uci === move.uci,
          ),
        ).length;
        return [
          {
    ...
    const source = localRows.length
      ? "local_crawled_package"
      : "local_legal_fallback";
    const payload = {
      source,
      fallback: source === "local_legal_fallback",
      reason:
        source === "local_crawled_package"
          ? `${localRows.length} local runtime moves`
          : "Local legal-move fallback",
      moves: localRows.length
        ? localRows
        : (new Chess(positionFen).moves({ verbose: true }) as any[]).map(
            (move) => ({
              uci: moveToUci(move),
              san: move.san,
              white: 1,
    ...
    setBrain((p) => ({
      ...p,
      lichess: source === "local_crawled_package" ? "active" : "fallback",
      latency: 0,
      note: payload.reason,
    }));
    return parsed;
  ...
  async function runBrain(eventType: string, extra: Record<string, any> = {}) {
    ...
    setBrain((p) => ({
      ...p,
      engine: "loading",
      gpt: extra.skipGpt ? "fallback" : "loading",
      source: "Manual analysis",
    }));
    setThinkingStep("engine");
    setPipelineNote("Manual analysis started.");
    const browserEngine = extra.skipClientEngine
      ? null
      : await runBrowserStockfish(
          requestFen,
          rating.skill,
          eventType === "reveal" ? 1000 : 700,
        );
    if (
      requestSeq !== brainSeq.current ||
      normalizeFen(fenRef.current) !== normalizeFen(requestFen)
    ) {
      setStaleOverlayIgnored(true);
      return null;
    }
    const clientEngine = browserEngine
      ? {
          source: browserEngine.source,
          pvs: browserEngine.pvs,
          depth: browserEngine.depth,
          timeMs: browserEngine.timeMs,
        }
      : undefined;
    if (browserEngine?.pvs?.length) {
      ...
      if (trainingMode === "continuation" && requestGame.turn() === userColor) {
        setAnnotation(
          engineAnnotationFromLine({
            fen: requestFen,
            line: browserEngine.pvs[0],
            openingName: repertoire.name,
          }),
        );
        setVisualReady(true);
    ...
    setBrain((p) => ({
      ...p,
      engine: browserEngine ? "active" : "fallback",
      source: browserEngine ? "Manual analysis" : "Engine fallback",
      note: browserEngine
        ? `depth ${browserEngine.depth ?? "?"} • ${browserEngine.timeMs} ms`
    ...
    const payload = {
      ...
      opponentBookMoves: opponentBookOptions.map((m) => ({
        san: m.san,
        uci: m.uci,
      })),
      ratingPool: rating.target,
    ...
    setPipelineNote(
      extra.skipGpt
        ? "Manual analysis request sent."
        : "Sending local facts to Brain because the user requested reveal/debug.",
    );
    ...
    try {
      ...
      setPipelineNote(
        data.annotation?.fallback
          ? "Manual Brain response received (fallback)."
          : "Manual Brain response received.",
      );
      setBrainResponse(data);
      if (data.engine?.pvs?.length)
        setEnginePreview({
          fen: requestFen,
          pvs: data.engine.pvs,
      ...
      window.setTimeout(() => {
        if (
          ...
          setThinkingStep("ready");
          setPipelineNote("Manual Brain check complete.");
        }
      }, 250);
      setBrain((p) => ({
        ...p,
        engine: data.engine?.fallback ? "fallback" : "active",
        gpt: data.annotation?.fallback ? "fallback" : "active",
        latency: Math.round(performance.now() - start),
        source: data.annotation?.fallback
          ? "manual brain fallback"
          : "manual brain",
        note:
          data.pipeline?.gpt ||
          data.annotation?.reason ||
          "Manual reveal/debug only",
      }));
      return data;
    } catch (e) {
      ...
      setBrain((p) => ({
        ...
        gpt: "error",
        source: "Brain error",
        note: e instanceof Error ? e.message : "Brain failed",
      }));
      return null;
    }
  }
  function resetHistory(startFen: string) {
  ...
  function selectRepertoire(
    ...
    const canonicalId = resolveStage2CanonicalOpeningId(id) ?? id;
    const isLocked = Boolean(
      repertoireProgress.lockedOpeningIds.includes(canonicalId),
    );
    if (isLocked && !options.allowLocked) {
      setFeedback("Keep training to unlock this opening.");
      setActiveTab("home");
      return;
    }
    const nextLineSessionId = createLearningSessionId();
    clearAuthoritativeTrainerSessionResumes();
    clearProjectiveTacticOverlay("opening_switch");
    setSelectedRepertoireId(canonicalId);
    setRuntimeTrainingSessionId(nextLineSessionId);
    refreshRuntimeTrainingLineSelection(
      canonicalId,
      recentRuntimeTrainingLineKeys,
    ...
    setFeedback("Opening loaded. Play the restricted training move.");
    setLastMove(null);
    setLastMoveSan("");
    setLastMoveColor(null);
    setReviewingFen(null);
    resetBranchAndContinuationState();
    setMoveHistory([]);
    setMoveHistoryUci([]);
  ...
  function resetBoard() {
    ...
    const nextLineSessionId = createLearningSessionId();
    clearAuthoritativeTrainerSessionResumes();
    clearProjectiveTacticOverlay("restart");
    setRuntimeTrainingSessionId(nextLineSessionId);
    refreshRuntimeTrainingLineSelection(
      selectedRepertoireId,
      recentRuntimeTrainingLineKeys,
    ...
    setPendingPromotion(null);
    setPromotionAuthorityDebug(null);
    setFeedback("Restarted. Find the first training move.");
    setLastMove(null);
    setLastMoveSan("");
    setLastMoveColor(null);
    setReviewingFen(null);
  ...
  function recordPosition(nextFen: string) {
    const nextIndex = historyIndex + 1;
    setPositionHistory((prev) => [...prev.slice(0, nextIndex), nextFen]);
    setHistoryIndex(nextIndex);
  }
  function jumpHistory(direction: -1 | 1) {
    const next = Math.max(
      0,
      Math.min(positionHistory.length - 1, historyIndex + direction),
    );
    if (next === historyIndex) return;
    clearProjectiveTacticOverlay("reset");
    setHistoryIndex(next);
    setFen(positionHistory[next]);
    setSelectedSquare(null);
    setPendingPromotion(null);
    setPromotionAuthorityDebug(null);
    setLastMove(null);
    ...
    setBookComplete(false);
    setFeedback(
      next === positionHistory.length - 1
        ? "Returned to the live position."
        : `Reviewing previous position ${next} of ${positionHistory.length - 1}. Use the arrows to return to live play.`,
    );
    bumpRuntimeFrame();
  }
  async function playOpponentMove(request: PendingOpponentRequest) {
    ...
    if (
      shouldFlagStaleOpponentReplyCommit({
        ...
        currentPendingRequest: currentRequest
          ? {
              requestId: currentRequest.requestId,
              baseFen: currentRequest.baseFen,
            }
          : null,
        liveFen: liveFenRaw,
      })
    ...
    const currentOpeningNodes =
      openingTree.nodesByFen4[normalizeFen(current.fen())] ?? [];
    const currentOpponentBookOptions = currentOpeningNodes
      .flatMap((node) => node.continuations)
      .filter(
        (move, index, all) =>
          move.color === opponentColor &&
    ...
    setBrain((p) => ({
      ...p,
      source: "opponent thinking",
      book:
        mode === "restricted"
          ? currentOpponentBookOptions.length
            ? "active"
            : "complete"
          : "complete",
      lichess: "loading",
    ...
    let source = "";
    if (mode === "restricted") {
      const initialRestrictedOpponentMoveUci =
        request.initialRestrictedOpponentMoveUci?.trim().toLowerCase() ?? null;
      const liveRestrictedLine = restrictedRuntimeLineLiveRef.current;
      const currentSelectedLineAuthority =
        resolveRestrictedRuntimeLineAuthority({
          trainingMode: liveRestrictedLine.active
            ? "restricted"
            : "continuation",
      ...
      const currentRestrictedSnapshot =
        buildRestrictedRuntimeLineRequestSnapshot({
      ...
      const runtimeCandidateForMove = (uci: string | null | undefined) =>
        runtimeBookFrameQuery.candidates.find(
          (candidate) =>
            candidate.uci.trim().toLowerCase() ===
            String(uci ?? "")
              .trim()
              .toLowerCase(),
        ) ?? null;
      if (initialRestrictedOpponentMoveUci) {
        const applied = applyUci(
          current.fen(),
          initialRestrictedOpponentMoveUci,
        );
        if (!applied || applied.color !== opponentColor) {
          clearPendingOpponentReplyRequest({ clearStaleIssue: true });
          pushRuntimeCriticalIssue(
            "restricted_initial_black_opponent_handoff_illegal",
          );
          setTrainerPhase("error");
          setFeedback(
            "The selected runtime line's first opponent move could not be applied in the current position.",
          );
          setBrain((p) => ({
            ...p,
            book: "complete",
            source: "restricted_initial_black_handoff_apply_failed",
            lichess: "ready",
          }));
          return;
        }
        ...
        chosen = {
          ...
          uci: applied.uci,
          fen: applied.fen,
          playPct: runtimeCandidateForMove(applied.uci)?.playPct ?? null,
        };
        ...
        const applied = applyUci(
          current.fen(),
          currentSelectedLineAuthority.move.uci,
        );
        if (!applied || applied.color !== opponentColor) {
          clearPendingOpponentReplyRequest({ clearStaleIssue: true });
          pushRuntimeCriticalIssue(
          ...
          setTrainerPhase("error");
          setFeedback(
            "The selected runtime line reply could not be applied in the current position.",
          );
          setBrain((p) => ({
            ...p,
            book: "complete",
            source: "selected_runtime_line_reply_apply_failed",
            lichess: "ready",
          }));
          return;
        }
        ...
        if (currentSelectedLineAuthority.kind === "complete") {
          setTrainerPhase("ready_for_user");
          setFeedback(
            "Line complete. Continue from here, or train the line again.",
          );
          setBrain((p) => ({
            ...p,
            book: "complete",
            source: "selected_runtime_line_complete",
            lichess: "ready",
          }));
          return;
        }
        ...
        setTrainerPhase("error");
        setFeedback(
          "No selected runtime-line opponent reply is available for this frame. Select another line or reopen the opening.",
        );
        setBrain((p) => ({
          ...p,
          book: "complete",
          source: currentSelectedLineAuthority.reason,
          lichess: "ready",
        }));
        return;
      }
      ...
      const defaultMaiaStatus = classifyMaiaProviderStatus({
        isAvailable: maiaOpponentProvider.isAvailable(),
      });
      ...
      if (maiaGate.allowed) {
        ...
        setMaiaRuntimeMs(
          typeof maiaResult.providerMs === "number"
            ? maiaResult.providerMs
            : null,
        );
          ...
          maiaRequestId !== maiaOpponentRequestSeqRef.current ||
            ...
            shouldFlagStaleOpponentReplyCommit({
              request: {
                ...
                baseFen: request.baseFen,
              },
              currentPendingRequest: pendingOpponentRequestRef.current
                ? {
                    requestId: pendingOpponentRequestRef.current.requestId,
                    baseFen: pendingOpponentRequestRef.current.baseFen,
                  }
                : null,
        ...
        if (maiaResultIsStale) {
          ...
          const selectedMaiaCandidate = selectMaiaOpponentReply(
            maiaResult,
            legalMovesUci,
          );
          ...
          setMaiaOpponentHumanLikelihood(
            selectedMaiaCandidate?.humanLikelihood ?? null,
          );
          if (selectedMaiaCandidate) {
            ...
            if (
              !legality.legalOnRequestFen ||
              !legality.applied ||
              !legality.appliedMoveUci ||
              !legality.appliedMoveSan ||
              !legality.appliedFen
            ) {
              ...
              setMaiaOpponentSanityGuardBlockedReason(
                sanityGuard.blockedReason,
              );
              if (!sanityGuard.allowed) {
                setMaiaOpponentFallbackUsed(false);
                setMaiaOpponentFallbackReason(
                  sanityGuard.blockedReason ??
                    "maia_sanity_guard_rejected_candidate",
                );
                pushMaiaTimelineEvent({
                ...
                chosen = {
                  san: legality.appliedMoveSan,
                  uci: legality.appliedMoveUci,
                  fen: legality.appliedFen,
                };
                source = "Continuation reply";
                variationDebug.opponentVariationReason =
                  "maia_candidate_selected";
      ...
      if (!chosen) {
        ...
        setBrain((p) => ({
          ...p,
          book: "complete",
          source: "maia_continuation_unavailable",
          lichess: "ready",
        }));
        return;
      }
    ...
    if (!chosen) {
      ...
      setTrainerPhase("error");
      setFeedback(
        "The approved opponent-reply provider is unavailable. No substitute move was played.",
      );
      return;
    }
    ...
    if (
      shouldFlagStaleOpponentReplyCommit({
        request: { requestId: request.requestId, baseFen: request.baseFen },
        currentPendingRequest: requestAfterCompute
          ? {
              requestId: requestAfterCompute.requestId,
              baseFen: requestAfterCompute.baseFen,
            }
          : null,
        liveFen: fenRef.current,
      })
    ...
    if (mode === "continuation") {
      pushMaiaTimelineEvent({
        ...
        reason:
          source === "Continuation reply"
            ? "maia_candidate_selected"
            : "fallback_or_existing_policy",
        fallbackReason:
          source === "Continuation reply"
            ? null
            : (maiaOpponentFallbackReason ?? null),
    ...
    const nextPhase: OverlayPhase = next.isGameOver()
      ? "terminal"
      : next.turn() === userColor
        ? "ready_for_user"
        : "opponent_selecting";
    commitRuntimeFrame({
      ...
      nextPhase,
      recordHistory: true,
      clearPendingOpponentRequest: true,
    });
    clearRuntimeCriticalIssue("stale_opponent_reply_commit");
    clearRuntimeCriticalIssue(
      "restricted_opponent_reply_missing_runtime_authority",
    );
    clearRuntimeCriticalIssue(
      "restricted_initial_black_opponent_handoff_illegal",
    ...
    setLastMoveSan(chosen.san);
    setLastMoveColor(opponentColor);
    setMoveHistory((prev) => [...prev, chosen.san]);
    setMoveHistoryUci((prev) => [...prev, chosen.uci]);
    setSelectedSquare(null);
    setShowAnswer(false);
    setOpponentCue(
      boardSettings.showOpponentCue
        ? {
            expiresAt: Date.now() + 2500,
            title: `Opponent: ${chosen.san}`,
            message:
              "Brief opponent cue. Your selected user-side view stays visible after this fades.",
            lines: [
              {
            ...
            cues: [{ square: chosen.uci.slice(2, 4), kind: "opponent" }],
            committed: true,
            fen: normalizeFen(chosen.fen),
          }
        : null,
    );
    setFeedback(
      buildOpponentReplyFeedback({
        san: chosen.san,
        playPct: chosen.playPct,
        variationApplied: variationDebug.opponentVariationApplied,
        blockedThirdRepeatBranches: variationDebug.blockedThirdRepeatBranches,
      }),
    );
    maybeShowProjectiveTacticsAfterMove({
      nextFen: chosen.fen,
      lastMoveUci: chosen.uci,
      movedColor: opponentColor,
    });
    setBrain((p) => ({
      ...p,
      source,
      lichess: source.includes("Lichess") ? "active" : p.lichess,
      note: variationNote,
    }));
  }
  ...
  async function handleReveal() {
    ...
    if (!phaseActionGate.revealButtonVisible) {
      recordDebugAction({
        action: "reveal_next_move",
        normalizedAction: "reveal_next_move",
        before,
        after: before,
        result: "blocked",
        reason: coachHiddenForFrame
          ? "coach_hidden"
          : (phaseActionGate.blockedReason ?? "no_revealable_move"),
        extra: {
          revealBlockedBecauseCoachHidden: coachHiddenForFrame,
          revealTargetUci: instructionTarget?.uci ?? null,
          revealTargetSource: instructionTarget ? "instruction_target" : "none",
        },
      });
      return;
    }
    if (showAnswer) {
      recordDebugAction({
        ...
        after: before,
        result: "no_op",
        reason: "reveal_idempotent",
        extra: {
          revealIdempotentNoop: true,
          revealTargetUci: instructionTarget?.uci ?? null,
          revealTargetSource: instructionTarget ? "instruction_target" : "none",
        },
      });
    ...
    const after = {
      ...
      showAnswer: true,
      coachInteraction:
        coachInteraction === "none" ? "answer" : coachInteraction,
    };
    const persisted = await persistTrainerOutcome(
      `${trainerFrameId}:${normalizeFen(fen)}:cue_revealed:${instructionTarget?.uci ?? "none"}`,
      {
        type: "cue_revealed",
        source: "train",
  ...
  function continueVsBot() {
    ...
    if (terminal) {
      ...
      setBrain((p) => ({
        ...p,
        source: "continuation terminal",
        book: "complete",
        note: "No legal continuation exists.",
      }));
      trackLearningEvent({
        type: "teaching_cue_compiled",
        source: "train",
        fen,
        metadata: {
          eventType: "continuation_terminal",
          lastUserMoveSan: lastMoveSan,
          lastUserMoveUci: lastMove,
          terminalReason: current.isCheckmate?.()
            ? "checkmate"
            : "no_legal_moves",
        },
      });
    ...
    setTrainerPhase("ready_for_user");
    setFeedback(
      `Continuation mode active. Blundr is selecting a continuation at ${rating.target}.`,
    );
  ...
  function handleSquareTap(square: string) {
    ...
    if (endingInfo) {
      setFeedback("Game over. Restart the opening to continue.");
      return;
    }
    if (isReviewingHistory) {
      setFeedback(
        "You are reviewing an older position. Use the forward arrow to return to the live board before moving.",
      );
      return;
    }
    if (!isUserTurn) {
      setFeedback("Opponent is thinking. Wait for your turn.");
      return;
    }
    if (!selectedSquare) {
      if (isOwnPiece(game, square, userColor)) {
        setSelectedSquare(square);
        setFeedback(`Selected ${square}. Legal destinations are highlighted.`);
      } else setFeedback("Tap one of your pieces first.");
      return;
    }
  ...
  function logMistake(positionFen: string, expected: string, played: string) {
    const k = normalizeFen(positionFen);
    setProgress((prev) => {
      const old = prev.mistakes[k];
      return {
        ...prev,
  ...
  async function attemptMove(
    ...
    const promotionAttempt = promotionPiece
      ? null
      : getPendingPromotionFromAttempt({
          fen: beforeFen,
          from,
    ...
    if (!legal) {
      ...
      setFeedback(
        promotionPiece
          ? "Illegal promotion move. Try another piece."
          : "Illegal move. Try another move.",
      );
      return;
    }
    ...
    if (promotionPiece) {
      const promotionAuthorityResult = resolvePromotionAuthority({
        ...
        authorityPromotionUci:
          expectedUserOptions[0]?.uci ?? instructionTarget?.uci ?? null,
      });
      setPromotionAuthorityDebug({
        frameId: trainerFrameId,
        ...promotionAuthorityResult,
        acceptedPromotionUci: promotionAuthorityResult.promotionAuthorityMatched
          ? playedUci
          : null,
      });
    }
    if (trainingMode === "continuation") {
      ...
      if (ratingSource.providerStatus !== "ready") {
        const runtimeEval = await runBrowserStockfish(
          beforeFen,
          rating.skill,
          700,
          USER_MOVE_RATING_MULTIPV,
        );
        ratingSource = mapEngineLinesToStockfishTopMoves({
          fen: beforeFen,
          pvs: (runtimeEval?.pvs ?? []).map((line) => ({
            uci: line.uci,
            san: line.san,
            cp: line.cp,
          })),
          ...
          errorReason: runtimeEval?.pvs?.length
            ? undefined
            : "stockfish_provider_unavailable",
        });
      }
      const userMoveFoundInTopMoves = ratingSource.topMoves.some(
      ...
      if (!userMoveFoundInTopMoves && ratingSource.providerStatus === "ready") {
        try {
          const afterEval = await runBrowserStockfish(
            current.fen(),
            rating.skill,
            650,
            1,
          ...
          const cpFromSideToMove =
            typeof afterEval?.pvs?.[0]?.cp === "number"
              ? Number(afterEval.pvs[0].cp)
              : null;
          directAfterMoveEvaluation = {
            providerStatus: afterEval?.pvs?.length ? "ready" : "unavailable",
            centipawnsFromMoverPerspective:
              cpFromSideToMove === null ? null : -cpFromSideToMove,
      ...
      const strength = rateContinuationUserMove({
        ...
        geniusMotifs: {
          ...
          winsMajorMaterial: Boolean(
            legal.captured && ["q", "r"].includes(String(legal.captured)),
          ),
        },
      });
    ...
    if (trainingMode === "restricted") {
      ...
      if (!correct) {
        ...
        const persisted = await persistTrainerOutcome(
          `${trainerFrameId}:${normalizeFen(beforeFen)}:move_incorrect:${playedUci}`,
          {
            type: "move_incorrect",
            source: "train",
            fen: beforeFen,
        ...
        if (isMoveQualityVerified(moveQuality)) {
          setFeedback(
            `Not quite. ${legal.san} is legal, but this drill is looking for ${expected}. Blundr Brain validated this pattern.`,
          );
        } else if (
          moveQuality?.status === "rejected" ||
          moveQuality?.status === "unavailable"
        ) {
          setFeedback(
            `Not quite. ${legal.san} is legal, but this saved line needs review before Blundr teaches it as a pattern.`,
          );
        } else {
          setFeedback(
            `Not quite. ${legal.san} is legal, but this drill is looking for the saved line move.`,
          );
        }
        return;
      }
    ...
    const persisted = await persistTrainerOutcome(
      `${trainerFrameId}:${normalizeFen(beforeFen)}:move_correct:${playedUci}`,
      {
        type: "move_correct",
        source: "train",
        fen: beforeFen,
    ...
    const nextExpectedMoveResolution = resolveExpectedMoveForFrame({
      openingTree,
      fen: nextFen,
      trainerPhase: nextGame.isGameOver()
        ? "terminal"
        : nextIsUserTurn
          ? "ready_for_user"
          : "opponent_replying",
    ...
    const nextLineCompleteConfirmed =
      (nextExpectedMoveResolution.lineLength ?? 0) > 0 &&
      (nextExpectedMoveResolution.lineCursor ?? 0) >=
        (nextExpectedMoveResolution.lineLength ?? 0);
    const nextExactOpeningNodes =
      openingTree.nodesByFen4[normalizeFen(nextFen)] ?? [];
    const nextExactSelectedLineNodes = nextExactOpeningNodes.filter(
      (node) => node.lineId === selectedRuntimeLineId,
    );
    const nextExactNodeHasChildren = nextExactSelectedLineNodes.length
      ? nextExactSelectedLineNodes.some((node) => node.continuations.length > 0)
      : "unknown";
    const nextHasNextOpponentMove = nextExactSelectedLineNodes.length
      ? nextExactSelectedLineNodes.some((node) =>
          node.continuations.some((move) => move.color === opponentColor),
        )
      : "unknown";
    const nextHasNextUserMove = nextExactSelectedLineNodes.length
      ? nextExactSelectedLineNodes.some((node) =>
          node.continuations.some((move) => move.color === userColor),
        )
      : "unknown";
    const nextExplicitCuratedTerminalNode = nextExactSelectedLineNodes.some(
      (node) => node.terminal && node.sideToMove === nextGame.turn(),
    );
    const nextRuntimePlayKeyBefore =
      buildRuntimePlayKeyBeforeFromSanHistory([...moveHistory, legal.san]) ??
      null;
    const nextRuntimeLineCurrentPly = moveHistoryUci.length + 1;
    const nextRestrictedRuntimeLineAuthority =
      resolveRestrictedRuntimeLineAuthority({
        trainingMode: runtimeBackedRestrictedLineActive
          ? "restricted"
          : "continuation",
    ...
    const nextRestrictedRuntimeBookExhaustedOnOpponentTurnAfterUserMove =
      Boolean(
        trainingMode === "restricted" &&
          !userExplicitlyEnteredContinuation &&
          !nextIsUserTurn &&
    ...
    const nextRestrictedRuntimeBookExhaustedEligibleForBranchComplete = Boolean(
      nextRestrictedRuntimeBookExhaustedOnOpponentTurnAfterUserMove &&
        (nextExplicitCuratedTerminalNode || nextSelectedLineConfirmedComplete),
    );
    const nextBranchCompleteContract = resolveBranchCompleteContract({
      ...
      lineExhaustedByCursor:
        nextSelectedLineConfirmedComplete ||
        nextRestrictedRuntimeBookExhaustedEligibleForBranchComplete,
    ...
    const nextTerminalProof = resolveStage2TerminalProof({
      ...
      validBranchCompleteLatch: false,
      bookCompleteAllowed: Boolean(
        nextSelectedLineConfirmedComplete || nextExplicitCuratedTerminalNode,
      ),
      guidedCompleteAllowed: Boolean(
        nextSelectedLineConfirmedComplete || nextExplicitCuratedTerminalNode,
      ),
    ...
    const nextBranchCompleteEligible = Boolean(
      nextBranchCompleteContract.branchCompleteEligible &&
        nextTerminalProof.proven,
    );
    const needsOpponentReply = shouldScheduleContinuationOpponentReply({
      trainingMode,
      gameOver: nextGame.isGameOver(),
      turn: nextGame.turn() as ChessColor,
      userColor,
      nextBranchCompleteEligible,
    });
    commitRuntimeFrame({
      nextFen,
      nextPhase: nextGame.isGameOver()
        ? "terminal"
        : needsOpponentReply
          ? "opponent_selecting"
          : "ready_for_user",
    ...
    if (nextBranchCompleteEligible) {
      setBranchCompleteLatch({
        active: true,
        reason:
          nextTerminalProof.reason ??
          nextBranchCompleteContract.reason ??
          "line_complete",
        fen4: normalizeFen(nextFen),
        lineId: selectedRuntimeLineId,
    ...
    setFeedback(
      trainingMode === "restricted"
        ? `Correct: ${legal.san}.`
        : `Played ${legal.san}. Move will be evaluated.`,
    );
    maybeShowProjectiveTacticsAfterMove({
      nextFen,
      lastMoveUci: playedUci,
      movedColor: userColor,
    });
    ...
    setProgress((prev) => {
      ...
      if (reviewingFen && next[reviewingFen]) {
        if (next[reviewingFen].count <= 1) delete next[reviewingFen];
          ...
          next[reviewingFen] = {
            ...next[reviewingFen],
            count: next[reviewingFen].count - 1,
          };
      }
      return {
        ...prev,
        ...
        trainedPositions: { ...prev.trainedPositions, [currentKey]: true },
        mistakes: next,
      };
    });
    setReviewingFen(null);
  ...
  function practiceMistake(m: Mistake) {
    const rep = repertoires.find((r) => r.id === m.repertoireId);
    clearProjectiveTacticOverlay("reset");
    if (rep)
      setSelectedRepertoireId(
        resolveStage2CanonicalOpeningId(rep.id) ?? rep.id,
      );
    setFen(m.fen);
    resetHistory(m.fen);
    setReviewingFen(normalizeFen(m.fen));
    setSelectedSquare(null);
    setPendingPromotion(null);
    setPromotionAuthorityDebug(null);
    setFeedback("Review this opening position. Play the expected move.");
    setLastMove(null);
    setLastMoveSan("");
    setLastMoveColor(null);
    resetBranchAndContinuationState();
  ...
  function createCustomRepertoire() {
    ...
    for (const move of moves) {
      try {
        if (!test.move(move)) {
          setFeedback(`Could not parse move: ${move}`);
          return;
        }
      } catch {
        setFeedback(`Could not parse move: ${move}`);
    ...
    const rep: Repertoire = {
      ...
      color: newRepColor,
      description: "Custom line saved on this device.",
      lines: [moves],
      custom: true,
    };
    clearProjectiveTacticOverlay("opening_switch");
    ...
    setBookComplete(false);
    clearPendingOpponentReplyRequest({ clearStaleIssue: true });
    setFeedback("Custom repertoire saved. Restricted training is active.");
    setActiveTab("train");
    bumpRuntimeFrame();
  }
  const squareStyles: Record<string, CSSProperties> = {};
  if (lastMove && lastMove.length >= 4) {
    squareStyles[lastMove.slice(0, 2)] = {
      boxShadow:
        "inset 0 0 0 999px rgba(255,255,255,.12), inset 0 0 22px rgba(255,255,255,.5)",
    };
    squareStyles[lastMove.slice(2, 4)] = {
      boxShadow:
        "inset 0 0 0 999px rgba(255,255,255,.16), inset 0 0 24px rgba(255,255,255,.62)",
    };
  }
  if (activeBoard) {
    const suppressPlainPreVisuals =
      v28VisibleSurface?.mode === "plain_before_show_more";
    const visualSquares = suppressPlainPreVisuals
      ? []
      : !presentationFrame.visual.shouldRender
        ? []
        : presentationFrame.visual.source === "visual_recipe"
          ? visualRecipePlayback.squares
              .filter((sq) => isValidSquare(sq.square))
              .slice(0, 4)
          : visualModelOutput
            ? activeVisualModelOutput &&
              trainerPhase === "ready_for_user" &&
    ...
    for (const cue of visualSquares.slice(0, 4)) {
      ...
      const shadow =
        role === "destination"
          ? "inset 0 0 0 3px rgba(94,126,255,.82), inset 0 0 26px rgba(94,126,255,.48)"
          : role === "king_safety"
            ? "inset 0 0 0 3px rgba(22,163,74,.62), inset 0 0 24px rgba(22,163,74,.42)"
            : role === "weakness" || role === "danger"
              ? "inset 0 0 0 3px rgba(239,68,68,.58), inset 0 0 24px rgba(239,68,68,.34)"
              : "inset 0 0 22px rgba(255,210,70,.58)";
      squareStyles[cue.square] = {
        ...squareStyles[cue.square],
        background: `radial-gradient(circle, ${bg} 0%, ${bg} 38%, transparent 72%)`,
        boxShadow: shadow,
      };
    }
    if (
      ...
      boardSettings.showOpponentCue &&
      shouldRenderOpponentLastMoveHighlight({
        committed: opponentCue.committed,
        cueFen: opponentCue.fen,
        boardFen: normalizeFen(fen),
      })
    )
      for (const cue of opponentCue.cues) {
        squareStyles[cue.square] = {
          ...squareStyles[cue.square],
          background:
            "radial-gradient(circle, rgba(184,132,255,.28) 0%, rgba(184,132,255,.18) 38%, transparent 72%)",
        };
      }
  }
  if (boardSettings.showMoveDots && selectedLegalMoves.length) {
    for (const m of selectedLegalMoves) {
      ...
      squareStyles[m.to] = {
        ...squareStyles[m.to],
        background: `radial-gradient(circle, ${dot} 0%, ${dot} 18%, transparent 23%)`,
        boxShadow: isCapture
  ...
  if (presentationFrame.visual.source === "continuation_candidate") {
    for (const highlight of continuationCandidateVisual.highlights) {
      squareStyles[highlight.square] = {
        ...squareStyles[highlight.square],
        background:
          "radial-gradient(circle, rgba(22,163,74,.24) 0%, rgba(22,163,74,.16) 38%, transparent 72%)",
        boxShadow: "inset 0 0 0 3px rgba(22,163,74,.45)",
      };
    }
  ...
  if (selectedSquare)
    squareStyles[selectedSquare] = {
      ...squareStyles[selectedSquare],
      boxShadow:
        "inset 0 0 0 3px rgba(22,101,52,.85), inset 0 0 24px rgba(22,101,52,.5)",
    };
  const visualSourceForRender = String(
    presentationFrame.visual.source ?? "none",
  );
  const staleCanSuppressVisual =
    visualFrameStale && visualSourceForRender === "visual_recipe";
  ...
  const surfaceVisualLines = v28SurfaceActive
    ...
    : visibleTeachingSurface?.visual?.shouldRender
      ? (visibleTeachingSurface.visual.lines as ActiveLine[])
      : null;
  const rawBoardLines: ActiveLine[] =
    surfaceVisualLines ??
    (presentationFrame.visual.shouldRender && !staleCanSuppressVisual
      ? (presentationFrame.visual.lines as ActiveLine[])
      : []);
  const boardLinesToRender: ActiveLine[] = useMemo(() => {
    ...
    const expectedFrom = instructionTarget.from;
    const expectedTo = instructionTarget.to;
    const matching = rawBoardLines.find(
      (line) => line.from === expectedFrom && line.to === expectedTo,
    );
    const primary = matching ?? {
      from: expectedFrom,
  ...
  const suppressPlainPreTargetHighlights =
    v28VisibleSurface?.mode === "plain_before_show_more";
  if (
    !instructionTarget?.uci &&
    !selectedSquare &&
    ...
    const allowedSquares = buildTrainingBoardVisibilitySquares({
      instructionTargetFrom: suppressPlainPreTargetHighlights
        ? null
        : (instructionTarget?.from ?? null),
      instructionTargetTo: suppressPlainPreTargetHighlights
        ? null
        : (instructionTarget?.to ?? null),
      selectedSquare,
  ...
  const transientLinesToRender: ActiveLine[] =
    activeBoard &&
    opponentCue &&
    boardSettings.showOpponentCue &&
    shouldRenderOpponentLastMoveHighlight({
      ...
      : [];
  const legalVerboseMoves = game.moves({ verbose: true }) as any[];
  const expectedMoveLegal = expectedUserOptions[0]
    ? legalVerboseMoves.some(
        (move) => moveToUci(move) === expectedUserOptions[0].uci,
      )
    : null;
  useEffect(() => {
    const actualTitle = surfaceCoachCardDecision?.shouldShowCoachCard
      ? String(surfaceCoachCardDecision.title ?? "").trim()
      : null;
    const actualBody = surfaceCoachCardDecision?.shouldShowCoachCard
      ? String(surfaceCoachCardDecision.body ?? "").trim()
      : null;
    const actualButtons = surfaceCoachCardDecision?.shouldShowCoachCard
      ? ((surfaceCoachCardDecision.buttons ?? []) as any[]).map(String)
      : [];
    const pipelineTitle = displayedCoachDecision?.shouldShowCoachCard
      ? String(displayedCoachDecision.title ?? "").trim()
      : null;
    ...
    const preAuthoritySurfaceTitle = visibleTeachingSurface?.coach?.shouldRender
      ? String(visibleTeachingSurface.coach.title ?? "").trim()
      : null;
    const preAuthoritySurfaceBody = visibleTeachingSurface?.coach?.shouldRender
      ? String(visibleTeachingSurface.coach.body ?? "").trim()
      : null;
    const debugVisibleTitle = actualTitle;
    const debugVisibleBody = actualBody;
    const debugVisibleButtons = Array.isArray(visibleTeachingSurface?.actions)
      ? visibleTeachingSurface.actions.map(String)
      : [];
    const visualCount = Array.isArray(boardLinesToRender)
      ? boardLinesToRender.length
      : 0;
    const revealTargetUci = instructionTarget?.uci ?? null;
    const visualAligned = instructionTarget?.uci
      ? Boolean(
          boardLinesToRender[0] &&
            `${boardLinesToRender[0].from}${boardLinesToRender[0].to}` ===
              instructionTarget.uci,
        )
      : "not_applicable";
    const revealAligned = instructionTarget?.uci
      ? showMoreShown
        ? true
        : "not_applicable"
      : "not_applicable";
    ...
    if (
      (actualTitle ?? null) !== (debugVisibleTitle ?? null) ||
      (actualBody ?? null) !== (debugVisibleBody ?? null) ||
      JSON.stringify(actualButtons) !== JSON.stringify(debugVisibleButtons)
    ) {
    ...
    if (
      visualCount !==
      Number((visibleTeachingSurface?.visual?.lines ?? []).length)
    ) {
    ...
    const entry: CoachCardRenderTimelineEntry = {
      ...
      visibleCoachOwner:
        visibleTeachingSurface?.debug?.visibleCoachOwner ?? null,
      coachIntent: String(
        displayedCoachDecision?.debug?.coachIntent ??
          visibleTeachingSurface?.mode ??
          "",
      ),
      coachDecisionSource: String(
        displayedCoachDecision?.debug?.coachDecisionSource ?? "",
      ),
      runtimeSafeFallbackUsed: Boolean(
        displayedCoachDecision?.debug?.verifiedFallbackUsed,
      ),
      runtimeSafeFallbackReason:
        String(displayedCoachDecision?.debug?.fallbackReason ?? "").trim() ||
        null,
      surfaceSafetyBlocked: Boolean(visibleTeachingSurface?.safety?.blocked),
      surfaceSafetyBlockedReason:
        visibleTeachingSurface?.safety?.reason ??
        visibleTeachingSurface?.safety?.blockedReason ??
        null,
      surfaceSafetyBlockedSeverity:
        visibleTeachingSurface?.safety?.blockedSeverity ?? null,
      surfaceSafetyRecoveredBySafeTeachingCopy: Boolean(
        visibleTeachingSurface?.safety?.recoveredBySafeTeachingCopy,
      ),
      plainLeakDetected: Boolean(
        visibleTeachingSurface?.safety?.plainLeakDetected,
      ),
      targetAligned: instructionTarget?.uci
        ? Boolean(displayedCoachDecision?.debug?.coachQuality?.targetAligned)
        : "not_applicable",
      pieceAligned: instructionTarget?.pieceType
        ? Boolean(displayedCoachDecision?.debug?.coachQuality?.pieceAligned)
        : "not_applicable",
      ...
      pipelineQualityScore: Number.isFinite(
        Number(
        ...
        : null,
      renderedQualityScore: Number.isFinite(
      ...
      qualityScoreSource:
        String(
          (renderedCoachQualityForDebug as any)?.qualityScoreSource ?? "",
        ) || null,
      qualityScoreReasonCodes: Array.isArray(
        (renderedCoachQualityForDebug as any)?.qualityScoreReasonCodes,
      )
        ? (renderedCoachQualityForDebug as any).qualityScoreReasonCodes.map(
            String,
          )
        : [],
      pipelineCopyRejected: pipelineCopyAuthorityDecision.pipelineCopyRejected,
      pipelineCopyRejectedReason:
        pipelineCopyAuthorityDecision.pipelineCopyRejectedReason,
      renderedCopyAuthority:
        pipelineCopyAuthorityDecision.renderedCopyAuthority,
      pipelineCopyAuthority:
        pipelineCopyAuthorityDecision.pipelineCopyAuthority,
      ...
      preAuthoritySurfaceReason:
        preAuthoritySurfaceTitle !== actualTitle ||
        preAuthoritySurfaceBody !== actualBody
    ...
    if (coachCardRenderEntryKey === lastCoachCardRenderEntryKeyRef.current)
      return;
    ...
    visibleTeachingSurface?.mode,
    visibleTeachingSurface?.owner,
    visibleTeachingSurface?.coach?.title,
    visibleTeachingSurface?.coach?.body,
    (visibleTeachingSurface?.actions ?? []).map(String).join("|"),
    visibleTeachingSurface?.safety?.blocked,
    visibleTeachingSurface?.safety?.reason,
    visibleTeachingSurface?.safety?.blockedReason,
    visibleTeachingSurface?.safety?.blockedSeverity,
    visibleTeachingSurface?.safety?.recoveredBySafeTeachingCopy,
    visibleTeachingSurface?.safety?.plainLeakDetected,
    displayedCoachDecision?.debug?.coachIntent,
    displayedCoachDecision?.debug?.coachDecisionSource,
    displayedCoachDecision?.debug?.verifiedFallbackUsed,
    displayedCoachDecision?.debug?.fallbackReason,
    displayedCoachDecision?.debug?.coachQuality?.targetAligned,
    displayedCoachDecision?.debug?.coachQuality?.pieceAligned,
    displayedCoachDecision?.title,
    displayedCoachDecision?.body,
    renderedCoachQualityForDebug?.qualityScore,
    JSON.stringify(
      (renderedCoachQualityForDebug as any)?.qualityScoreReasonCodes ?? [],
    ),
    pipelineCopyAuthorityDecision.pipelineCopyRejected,
    pipelineCopyAuthorityDecision.pipelineCopyRejectedReason,
    pipelineCopyAuthorityDecision.renderedCopyAuthority,
    pipelineCopyAuthorityDecision.pipelineCopyAuthority,
  ...
  useEffect(() => {
    const renderedActionIds =
      ...
      [];
    const primitiveIds = (v28BoardVisualUiModel?.visualRecipes ?? []).map(
      (visual) => String(visual.id),
    );
    const primitiveIdsKey = primitiveIds.join("|");
    const boardVisualIds = boardLinesToRender.map(
      (line) => `${line.from}${line.to}`,
    );
    const boardVisualIdsKey = boardVisualIds.join("|");
    const renderedActionIdsKey = renderedActionIds.join("|");
    const moveArrowCount = (v28BoardVisualUiModel?.visualRecipes ?? []).filter(
      (visual) => visual.type === "move_arrow",
    ).length;
    const srcDstCount = (v28BoardVisualUiModel?.visualRecipes ?? []).filter(
      (visual) =>
        visual.type === "source_highlight" ||
        visual.type === "destination_highlight",
    ).length;
    const entry = {
      ...
      trainerView,
      visibleSurfaceMode:
        v28VisibleSurface?.mode ?? visibleTeachingSurface?.mode ?? null,
      surfaceVisualPrimitiveIds: primitiveIds,
      boardVisualPrimitiveIds: boardVisualIds,
      moveArrowCount,
      sourceDestinationHighlightCount: srcDstCount,
      visualTargetUci:
        v28VisibleSurface?.targetUci ?? instructionTarget?.uci ?? null,
      plainBeforeShowMoreSuppressedVisuals:
        v28VisibleSurface?.mode === "plain_before_show_more"
          ? primitiveIds.length === 0 && boardLinesToRender.length === 0
          : "not_applicable",
      showMoreRestoredAssistedEquivalentVisuals:
        v28VisibleSurface?.mode === "plain_after_show_more"
          ? boardLinesToRender.length > 0
          : "not_applicable",
      renderedActionIds,
    };
    ...
    trainerFrameId,
    trainerView,
    v28VisibleSurface?.mode,
    v28VisibleSurface?.targetUci,
    visibleTeachingSurface?.mode,
    instructionTarget?.uci,
    (v28BoardVisualUiModel?.visualRecipes ?? [])
      .map((visual) => String(visual.id))
      .join("|"),
    (v28BoardVisualUiModel?.visualRecipes ?? []).filter(
      (visual) => visual.type === "move_arrow",
    ).length,
    (v28BoardVisualUiModel?.visualRecipes ?? []).filter(
      (visual) =>
  ...
  useEffect(() => {
    ...
    const renderedActionIds = (
      (surfaceCoachCardDecision?.buttons as any[] | undefined) ?? []
    ).map(String);
    const renderedActionIdsKey = renderedActionIds.join("|");
    ...
    const sourceDestinationHighlightRendered = Boolean(
      (from && squareStyles[from]) || (to && squareStyles[to]),
    );
    ...
    const leakedPiece = Boolean(
      piece && piece.length > 1 && text.includes(piece),
    );
    const leakKinds = [
      leakedSan ? "san" : "",
      leakedUci ? "uci" : "",
    ...
    (surfaceCoachCardDecision?.buttons ?? []).map(String).join("|"),
    instructionTarget?.from
      ? Number(Boolean(squareStyles[instructionTarget.from]))
      : 0,
    instructionTarget?.to
      ? Number(Boolean(squareStyles[instructionTarget.to]))
      : 0,
  ]);
  useEffect(() => {
    const nextState = {
      frameId: Number(trainerFrameId),
      mode: String(
        v28VisibleSurface?.mode ?? visibleTeachingSurface?.mode ?? "unknown",
      ),
      hintShown: coachHintRequestCount > 0,
      showMoreShown,
    ...
    if (
      previous &&
      (previous.frameId !== nextState.frameId ||
        previous.mode !== nextState.mode ||
        previous.hintShown !== nextState.hintShown ||
        previous.showMoreShown !== nextState.showMoreShown ||
        previous.targetUci !== nextState.targetUci)
    ) {
      const trigger =
        previous.frameId !== nextState.frameId
          ? "frame_changed"
      ...
      if (transitionEntryKey !== lastSurfaceTransitionEntryKeyRef.current) {
        ...
        setSurfaceModeTransitionTimeline((prev) => [
          ...prev.slice(-74),
          {
            ...
            visualsReset:
              previous.visualCount > 0 && nextState.visualCount === 0,
          },
        ]);
      }
    }
  ...
  const visualMoveUciForDebug = boardLinesToRender[0]
    ? `${boardLinesToRender[0].from}${boardLinesToRender[0].to}`
    : null;
  const visualTargetMatchesInstructionTarget = instructionTarget?.uci
    ? visualMoveUciForDebug === instructionTarget.uci
    : "unknown";
  const promotionDebugActive =
    promotionAuthorityDebug?.frameId === trainerFrameId
      ? promotionAuthorityDebug
      : null;
  const acceptedTargetUci = promotionDebugActive
    ? (promotionDebugActive.acceptedPromotionUci ??
      instructionTarget?.uci ??
      null)
    : (instructionTarget?.uci ?? null);
  const promotionAuthorityTargetUci =
    expectedUserOptions[0]?.uci ?? instructionTarget?.uci ?? null;
  const selectedContinuationCandidate =
    trainingMode === "continuation" && currentSelectedCandidateUci
      ? {
          uci: currentSelectedCandidateUci,
          san: currentSelectedCandidateSan ?? currentSelectedCandidateUci,
      ...
      : null;
  const lastMoveAttribution = attributeLastMove({
    lastMoveSan,
    lastMoveUci: lastMove,
    lastMoveColor,
    userColor,
  ...
  const diagnosticsSnapshot: TrainerDebugSnapshot =
    blundrDebugEnabled && debugSnapshotCollector
      ? debugSnapshotCollector({
          debugEnabled: blundrDebugEnabled,
          trainerFrameId,
          ...
          selectedConceptId:
            visualRecipe?.conceptId ?? teachingOrchestration?.cue.conceptId,
          activeLineName: repertoire.name,
          ...
          openingSelectionEligibleOpeningIds:
            runtimeOpeningSelection.eligibleOpeningIds,
          ...
          openingSelectionVisibilityGated:
            runtimeOpeningSelection.visibilityGated,
          openingSelectionWeightsSummary:
            runtimeOpeningSelection.weightsSummary,
          openingSelectionStickyReason:
            runtimeOpeningSelection.openingSelectionStickyReason,
          openingSelectionSeed: runtimeOpeningSelection.openingSelectionSeed,
          openingSelectionWasPersisted:
            runtimeOpeningSelection.openingSelectionWasPersisted,
          ...
          lineSelectionSource:
            ...
            (selectedOpeningAvailability?.runtimeAvailable
              ? "local_runtime_package"
              : "curated_repertoire"),
          lineSelectionWeighted: Boolean(
            selectedRuntimeTrainingLineSelection?.weighted ??
              selectedOpeningAvailability?.runtimeAvailable,
          ),
          lineSelectionContentGated: false,
          lineSelectionRuntimeBacked: Boolean(
            selectedRuntimeTrainingLineSelection?.source ===
              "local_runtime_package" ||
              selectedOpeningAvailability?.runtimeAvailable,
          ),
          lineSelectionEligibleCount:
            selectedRuntimeTrainingLineSelection?.eligibleCount ?? 0,
          lineSelectionEligibleLineIds:
            selectedRuntimeTrainingLineSelection?.eligibleLineIds ?? [],
          lineSelectionEligibleLineKeys:
            selectedRuntimeTrainingLineSelection?.eligibleLineKeys ?? [],
          lineSelectionRecentLineKeys: recentRuntimeTrainingLineKeys,
          lineSelectionBlockedRecentLineKeys:
            selectedRuntimeTrainingLineSelection?.blockedRecentLineKeys ?? [],
          lineSelectionBlockedThirdRepeatLineKeys:
            selectedRuntimeTrainingLineSelection?.blockedThirdRepeatLineKeys ??
            selectedRuntimeTrainingLineSelection?.blockedRecentLineKeys ??
            [],
          ...
          lineSelectionVariationReason:
            selectedRuntimeTrainingLineSelection?.variationReason ?? null,
          lineSelectionRepeatUnavoidable:
            selectedRuntimeTrainingLineSelection?.repeatUnavoidable ?? false,
          lineSelectionSeed:
            selectedRuntimeTrainingLineSelection?.selectionSeed ?? null,
          selectedRuntimeLineId:
            selectedRuntimeTrainingLineSelection?.selectedLineId ?? null,
          selectedRuntimeLineKey,
          selectedRuntimeLineIndex:
            selectedRuntimeTrainingLineSelection?.selectedLineIndex ?? null,
          selectedRuntimeLinePlayKey:
            selectedRuntimeTrainingLineSelection?.selectedPlayKey ?? null,
          selectedRuntimeLinePlaySequenceUci:
            selectedRuntimeTrainingLineSelection?.selectedPlaySequenceUci ?? [],
          ...
          selectedLineCompleteConfirmed,
          terminalProofLineAuthority: stage2OpeningDepthReached
            ? "actual_runtime_branch_or_depth"
            : selectedRuntimeTrainingLineSelection?.selectedLineKey
              ? "selected_runtime_line_play_sequence_uci"
              : "expected_move_resolution",
          terminalProofBlockedReason:
            stage2TerminalProof.blockedReasons[0] ?? null,
          currentInstructionFrame,
          playKeyBefore: runtimePlayKeyBeforeForFrame ?? null,
          playKey:
            runtimePlayKeyBeforeForFrame && instructionTarget?.uci
              ? `${runtimePlayKeyBeforeForFrame},${instructionTarget.uci}`
              : (instructionTarget?.uci ?? null),
          currentInstructionFrameKind: currentInstructionFrame?.kind ?? null,
          instructionTargetSource:
            currentInstructionFrame?.targetSource ?? null,
          moveHistory,
          fen,
          feedback,
          overlayFen,
          ...
          sanUciResolutionStatus: expectedUserOptions[0]
            ? "resolved"
            : expectedMoveResolution.source,
          ...
          visualRecipePrimitiveIds:
            visualRecipe?.beats.flatMap((beat) =>
              beat.primitives.map((primitive) => primitive.id),
            ) ?? [],
          ...
          selectedPromotionPiece:
            promotionDebugActive?.selectedPromotionPiece ?? null,
          attemptedPromotionUci:
            promotionDebugActive?.attemptedPromotionUci ?? null,
          acceptedPromotionUci:
            promotionDebugActive?.acceptedPromotionUci ?? null,
          acceptedTargetUci,
          promotionAuthorityMatched:
            promotionDebugActive?.promotionAuthorityMatched ?? null,
          promotionAuthorityMismatchReason:
            promotionDebugActive?.promotionAuthorityMismatchReason ?? null,
          ...
          showMoreShown,
          selectedCandidateSafetySource: (coachDecision?.debug as any)
            ?.coachEngineStatus,
          enginePreview,
          continuationCandidateLines: continuationCandidateVisual.lines,
          shouldRenderContinuationLines:
            trainingMode === "continuation" &&
            continuationCandidateVisual.shouldRender &&
            boardLinesToRender.length > 0,
          continuationVisualBlockedReason:
            trainingMode === "continuation" &&
            selectedContinuationCandidate &&
            !boardLinesToRender.length
              ? (continuationCandidateVisual.blockedReason ??
                "candidate_exists_but_no_board_lines")
              : "none",
          continuationAnalysisStatus,
          continuationRuntimeStatus: continuationRuntimeState.status,
          ...
          continuationFlowCriticalIssue:
            continuationFlowContract.criticalIssueIfInvalid,
          continuationCandidateLockId:
            continuationCandidateLock?.continuationCandidateLockId ?? null,
          continuationCandidateLockFen4:
            continuationCandidateLock?.continuationCandidateLockFen4 ?? null,
          continuationCandidateLockRequestId:
            continuationCandidateLock?.continuationCandidateLockRequestId ??
            null,
          continuationCandidateLockUci:
            continuationCandidateLock?.continuationCandidateLockUci ?? null,
          continuationCandidateLockSan:
            continuationCandidateLock?.continuationCandidateLockSan ?? null,
          continuationCandidateLockSource:
            continuationCandidateLock?.continuationCandidateLockSource ?? null,
          continuationCandidateLockReason:
            continuationCandidateLock?.continuationCandidateLockReason ?? null,
          continuationPauseRequired: forceContinuationPause,
          continuationPauseClicked,
          continuationPauseReason: continuationPauseDecision.pauseReason,
          continuationHardStopMoveNumber:
            continuationPauseDecision.hardStopMoveNumber,
          continuationHardStopPlyLimit:
            continuationPauseDecision.hardStopPlyLimit,
          continuationCurrentPlyCount:
            continuationPauseDecision.currentPlyCount,
          continueFromHereClickHandled,
          continueFromHereClickBlockedReason,
          continuationSessionId,
          continuationCandidateEvaluationBlockedUntilClick: Boolean(
            forceContinuationPause && !continuationPauseClicked,
          ),
          continuationInstructionTargetBeforeClick: !continuationPauseClicked
            ? (instructionTarget?.uci ?? null)
            : null,
          continuationVisualTargetBeforeClick: !continuationPauseClicked
            ? visualMoveUciForDebug
            : null,
          continuationCoachTargetBeforeClick: !continuationPauseClicked
            ? ((displayedCoachDecision?.debug as any)?.coachMoveUci ??
              instructionTarget?.uci ??
              null)
            : null,
          continuationPolicyDebug: continuationPolicyCandidate?.debug ?? null,
          continuationSelectionSource:
            validatedContinuationCandidate?.source ?? null,
          continuationSelectionReason:
            validatedContinuationCandidate?.reason ?? null,
          runtimeBookQueried: Boolean(runtimeBookFrameShouldQuery),
          runtimeBookOpeningId:
            runtimeBookFrameQuery.openingId ?? runtimeOpeningIdForFrame ?? null,
          ...
          runtimeBookTopCandidateUci:
            runtimeBookFrameQuery.candidates[0]?.uci ?? null,
          runtimeBookTopCandidateSan:
            runtimeBookFrameQuery.candidates[0]?.san ?? null,
          runtimeBookTopCandidateRank:
            runtimeBookFrameQuery.candidates[0]?.rank ?? null,
          runtimeBookTopCandidateGames:
            runtimeBookFrameQuery.candidates[0]?.totalGames ?? null,
          runtimeBookTopCandidatePlayPct:
            runtimeBookFrameQuery.candidates[0]?.playPct ?? null,
          opponentReplyAuthoritySource:
            restrictedOpponentReplyAuthorityPreview.opponentReplyAuthoritySource,
          opponentReplyAuthorityCandidateUci:
            restrictedOpponentReplyAuthorityPreview.opponentReplyAuthorityCandidateUci,
          opponentReplyAuthorityCandidateSan:
            restrictedOpponentReplyAuthorityPreview.opponentReplyAuthorityCandidateSan,
          opponentReplyAuthorityCandidateGames:
            restrictedOpponentReplyAuthorityPreview.opponentReplyAuthorityCandidateGames,
          opponentReplyAuthorityCandidatePlayPct:
            restrictedOpponentReplyAuthorityPreview.opponentReplyAuthorityCandidatePlayPct,
          opponentReplyAuthorityRejectedReason:
            restrictedOpponentReplyAuthorityPreview.opponentReplyAuthorityRejectedReason,
          runtimeBookBookExhausted: runtimeBookFrameQuery.bookExhausted,
          ...
          visibleOpeningCount: STAGE2_OPENING_AVAILABILITY_MATRIX.filter(
            (opening) => opening.userVisible,
          ).length,
          selectedOpeningRuntimeAvailable:
            selectedOpeningAvailability?.runtimeAvailable ?? null,
          selectedOpeningContentStatus:
            selectedOpeningAvailability?.contentStatus ?? null,
          candidateSource: selectedOpeningAvailability?.runtimeAvailable
            ? "local_runtime_package"
            : "curated_repertoire",
          liveLichessCalled: false,
          openingAvailabilityStatus:
            selectedOpeningAvailability?.runtimeAvailable
              ? "runtime_available"
              : (selectedOpeningAvailability?.qaStatus ?? "smoke_pass"),
          ...
          stage2CoachingSafetyStatus:
            stage2CoachingPacketResolution.kind === "none"
              ? null
              : stage2CoachingPacketResolution.packet.safetyStatus,
          stage2CoachingSurface: stage2CoachContext.surface,
          stage2CoachingSourceFile:
            stage2CoachingPacketResolution.kind === "none"
              ? null
              : stage2CoachingPacketResolution.packet.sourceFile,
          stage2CoachingRuntimeMatched:
            stage2CoachingPacketResolution.kind === "none"
              ? null
              : stage2CoachingPacketResolution.packet.runtimeReconciliation
                  .status === "matched",
          continuationTargetResolverStatus,
          continuationResolvedTargetUci,
          continuationResolvedTargetSource,
          continuationResolvedTargetLabel,
          effectiveContinuationCandidateUci:
            continuationCandidateResolution.guard
              .effectiveContinuationCandidateUci,
          effectiveContinuationCandidateSan:
            continuationCandidateResolution.guard
              .effectiveContinuationCandidateSan,
          effectiveContinuationCandidateSource:
            continuationCandidateResolution.guard
              .effectiveContinuationCandidateSource,
          effectiveContinuationCandidateFen4:
            effectiveContinuationCandidate?.fen4 ?? null,
          effectiveContinuationCandidateBlockedReason:
            continuationCandidateResolution.guard
              .effectiveContinuationCandidateBlockedReason,
          stockfishPromotionGuardTrainingMode:
            continuationCandidateResolution.guard
              .stockfishPromotionGuardTrainingMode,
          stockfishPromotionGuardIsUserTurn:
            continuationCandidateResolution.guard
              .stockfishPromotionGuardIsUserTurn,
          stockfishPromotionGuardTrainerPhase:
            continuationCandidateResolution.guard
              .stockfishPromotionGuardTrainerPhase,
          stockfishPromotionGuardFenMatches:
            continuationCandidateResolution.guard
              .stockfishPromotionGuardFenMatches,
          stockfishPromotionGuardLegal:
            continuationCandidateResolution.guard.stockfishPromotionGuardLegal,
          stockfishPromotionGuardSourceAllowed:
            continuationCandidateResolution.guard
              .stockfishPromotionGuardSourceAllowed,
          continuationEngineFallbackUsed: Boolean(
            validatedContinuationCandidate?.engineFallbackUsed ||
              validatedContinuationCandidate?.isEngineBestFallback,
          ),
          continuationEngineFallbackReason:
            validatedContinuationCandidate?.engineFallbackReason ?? null,
          continuationDatabaseCandidatesRejected: Boolean(
            validatedContinuationCandidate?.databaseCandidatesRejected,
          ),
          continuationRejectionReasons:
            validatedContinuationCandidate?.rejectionReasons ?? [],
          continuationSelectedCandidateSource:
            validatedContinuationCandidate?.isEngineBestFallback
              ? "engine_best"
              : (validatedContinuationCandidate?.source ?? null),
          continuationAnalysisStatusLabel:
            validatedContinuationCandidate?.isEngineBestFallback
              ? "complete"
              : continuationAnalysisStatus,
          stockfishProviderStatus:
            stockfishTopMovesForContinuation.providerStatus,
          stockfishDepth: stockfishTopMovesForContinuation.depth,
          ...
          stockfishTopMoveUcis: stockfishTopMovesForContinuation.topMoves.map(
            (move) => move.uci,
          ),
          stockfishTopMoveSans: stockfishTopMovesForContinuation.topMoves.map(
            (move) => move.san,
          ),
          stockfishEvaluationFen: stockfishTopMovesForContinuation.fen,
          stockfishEvaluationMatchesBoardFen:
            normalizeFen(stockfishTopMovesForContinuation.fen) ===
            normalizeFen(fen),
          suggestedMoveUci: validatedContinuationCandidate?.uci ?? null,
          suggestedMoveSan: validatedContinuationCandidate?.san ?? null,
          stockfishSuggestedRank:
            continuationSuggestionValidation?.rank ?? null,
          stockfishSuggestedTop10: Boolean(
            continuationSuggestionValidation?.isTop10,
          ),
          suggestionAccepted: Boolean(
            continuationSuggestionValidation?.accepted,
          ),
          candidateReplacedByStockfish: Boolean(
            continuationSuggestionValidation &&
              !continuationSuggestionValidation.accepted &&
              Boolean(continuationSuggestionValidation.replacementUci),
          ),
          replacementUci:
            continuationSuggestionValidation?.replacementUci ?? null,
          replacementReason:
            continuationSuggestionValidation?.rejectionReason ?? null,
          lastContinuationUserMoveRating,
          userMoveFoundInMultipv32:
            lastContinuationUserMoveRating?.userMoveFoundInTopMoves ?? null,
          userMoveOutsideMultipv32: lastContinuationUserMoveRating
            ? lastContinuationUserMoveRating.userMoveFoundInTopMoves === false
            : null,
          directEvalFallbackUsed:
            lastContinuationUserMoveRating?.ratingMethod ===
          ...
          badgeSuppressedReason: continuationRatingBadgeVisible
            ? "none"
            : trainingMode !== "continuation"
              ? "not_continuation_mode"
              : trainerView === "plain"
          ...
          continuationAnalysisFen4: enginePreview?.fen
            ? normalizeFen(enginePreview.fen)
            : null,
          ...
          maiaContinuationEnabled: Boolean(
            trainingMode === "continuation" &&
              userExplicitlyEnteredContinuation,
          ),
          ...
          maiaOpponentProviderUsed: Boolean(
            trainingMode === "continuation" &&
              userExplicitlyEnteredContinuation,
          ),
          maiaOpponentRequestPending: Boolean(pendingOpponentRequest),
          maiaOpponentLastMoveUci:
            lastMoveColor === opponentColor ? lastMove : null,
          ...
          coachPieceType:
            ...
            null,
          revealTargetUci:
            (lastActionDebug as any)?.frameId === trainerFrameId
              ? ((lastActionDebug as any)?.revealTargetUci ??
                instructionTarget?.uci ??
                null)
              : (instructionTarget?.uci ?? null),
          revealTargetSource:
            (lastActionDebug as any)?.frameId === trainerFrameId
              ? ((lastActionDebug as any)?.revealTargetSource ??
                (instructionTarget ? "instruction_target" : "none"))
              : instructionTarget
                ? "instruction_target"
                : "none",
          ...
          coachMemoryClearedLegacyCount:
            coachMemoryMigration.clearedLegacyCount,
          liveCoachState,
          presentationFrame,
          coachSurfacePolicy,
          branchTransitionSurfaceRendered: Boolean(
            branchTransitionSurface?.render ||
              v28VisibleSurface?.mode === "branch_complete",
          ),
          branchTransitionReason: branchTransitionSurface?.reason ?? null,
          continueFromHereAvailable: Boolean(
            branchTransitionSurface?.render ||
              (v28CoachUiModel?.actions ?? []).some(
                (action) =>
                  action.kind === "continue_from_here" &&
                  action.visible !== false,
              ),
          ),
          branchCompleteEligible: branchCompleteEligibleNow,
          branchCompleteReason: branchCompleteReasonNow,
          branchCompleteBlockedReason:
            branchCompleteContract.branchCompleteBlockedReason,
          branchCompleteLineExhaustedEvidence:
            branchCompleteContract.lineExhaustedEvidence,
          branchCompleteAfterFinalUserMove:
            branchCompleteContract.afterFinalUserMove,
          ...
          selectedLineExhaustionReason:
            branchCompleteContract.selectedLineExhaustionReason,
          selectedLineExhaustionBlockedReason:
            branchCompleteContract.selectedLineExhaustionBlockedReason,
          ...
          finalGuidedUserMoveCompletedLine:
            branchCompleteContract.finalGuidedUserMoveCompletedLine,
          explicitCuratedTerminalNode: explicitCuratedTerminalNode,
          pendingOpponentRequestConflict:
            branchCompleteContract.pendingOpponentRequestConflict,
          pendingOpponentRequestCancelledForBranchComplete: Boolean(
            branchCompleteShouldCancelPending && !pendingOpponentRequest,
          ),
          branchCompleteBlockedOpponentRequestId:
            branchCompleteBlockedOpponentRequestIdRef.current,
          continueFromHereClicked,
          opportunityCount:
            (displayedCoachDecision?.debug as any)?.opportunityCount ??
            liveCoachState?.opportunities?.length,
          renderableOpportunityCount:
            (displayedCoachDecision?.debug as any)
              ?.renderableOpportunityCount ??
            liveCoachState?.opportunities?.length,
          coachQuality:
            renderedCoachQualityForDebug ??
            (displayedCoachDecision?.debug as any)?.coachQuality ??
            null,
          pipelineCoachCardTitle: displayedCoachDecision?.shouldShowCoachCard
            ? String(displayedCoachDecision?.title ?? "").trim() || null
            : null,
          pipelineCoachCardBody: displayedCoachDecision?.shouldShowCoachCard
            ? String(displayedCoachDecision?.body ?? "").trim() || null
            : null,
          pipelineCoachCardSource:
            String(
              (displayedCoachDecision?.debug as any)?.coachDecisionSource ?? "",
            ).trim() || null,
          moveFactPacket:
            (displayedCoachDecision?.debug as any)?.moveFactPacket ?? null,
          positionDeltaPacket:
            (displayedCoachDecision?.debug as any)?.positionDeltaPacket ?? null,
          featurePacket:
            (displayedCoachDecision?.debug as any)?.featurePacket ?? null,
          planPacket:
            (displayedCoachDecision?.debug as any)?.planPacket ?? null,
          opportunityPacket:
            (displayedCoachDecision?.debug as any)?.opportunityPacket ?? null,
          safetyResult:
            (displayedCoachDecision?.debug as any)?.safetyResult ?? null,
          pendingOpponentRequest,
          runtimeCriticalIssues,
          lastActionDebug,
          eventLog: debugEventLog,
          lastCoachRecords,
          lastCoachBodies: getRecentInstructionalCoachRecords(
            lastCoachRecordsRef.current,
            5,
          ...
          actualCoachCardButtons: (surfaceCoachCardDecision?.buttons ?? []).map(
            String,
          ),
          actualCoachCardSource: surfaceCoachCardDecision
            ? "surfaceCoachCardDecision"
            : null,
          actualActionSource: v28SurfaceActive
            ? "visible_surface_v28"
            : "legacy_or_presentation",
          actualVisualSource: v28SurfaceActive
            ? "visible_surface_v28"
            : String(presentationFrame?.visual?.source ?? "none"),
          renderedActionIds: (surfaceCoachCardDecision?.buttons ?? []).map(
            String,
          ),
          surfaceActionIds: (v28CoachUiModel?.actions ?? [])
            .filter((action) => action.visible)
          ...
          currentSelectedCandidateUci,
          previousSelectedCandidateUci:
            candidateSyncDebugRef.current.previousSelectedCandidateUci,
          staleSelectedCandidateDetected:
            candidateSyncDebugRef.current.staleSelectedCandidateDetected,
          staleSelectedCandidateCleared:
            candidateSyncDebugRef.current.staleSelectedCandidateCleared,
          autoContinuationReason: userExplicitlyEnteredContinuation
            ? "user_explicit"
            : "none",
          userExplicitlyEnteredContinuation,
          prematureContinuationBlocked:
            trainingMode !== "continuation" &&
            !guidedCoveragePolicy.guidedCompleteAllowed,
          transitionToContinuationAllowed: userExplicitlyEnteredContinuation,
          transitionToContinuationReason: userExplicitlyEnteredContinuation
            ? "user_explicit"
            : "not_allowed_until_continue_line",
          continuationPauseAlreadyConsumed,
          ...
          visibleSurfaceOwner: visibleTeachingSurface?.owner ?? null,
          visibleCoachOwner:
            visibleTeachingSurface?.debug?.visibleCoachOwner ??
            presentationFrame?.coach?.owner ??
            "none",
          visibleVisualOwner:
            visibleTeachingSurface?.debug?.visibleVisualOwner ??
            presentationFrame?.visual?.source ??
            "none",
          visibleActionOwner:
            visibleTeachingSurface?.debug?.visibleActionOwner ??
            "visibleActionPolicy",
          showMoreTargetUci: intendedShowMoreTargetUci,
          plainLeakDetected:
            visibleTeachingSurface?.safety?.plainLeakDetected ?? false,
          legacyBypassDetectedFromSurface:
            visibleTeachingSurface?.safety?.legacyBypassDetected ?? false,
          surfaceSafetyBlocked:
            visibleTeachingSurface?.safety?.blocked ?? false,
          surfaceFourTargetMismatch:
            visibleTeachingSurface?.debug?.fourTargetMismatch ?? false,
          surfaceTwoPieceMismatch:
            visibleTeachingSurface?.debug?.twoPieceTypeMismatch ?? false,
        })
  ...
  if (isActiveTab(activeTab, "repertoire")) {
    return (
      <RepertoireProgressPanel
        embedded
        onTrainOpening={(openingId) =>
          selectRepertoire(openingId, { allowLocked: false })
        }
      />
    );
  }
    ...
    <main className="w-full text-stone-950">
      <div className="mx-auto flex w-full max-w-[1340px] flex-col pb-24">
        {isActiveTab(activeTab, "home") && (
          <section className="space-y-5">
            <header className="rounded-[2rem] border border-stone-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-4 grid gap-2">
                <button
                  ...
                  className="inline-flex w-full items-center justify-between rounded-2xl bg-green-700 px-4 py-3 text-sm font-black text-white shadow-sm"
                >
                ...
                <div className="grid grid-cols-3 gap-2">
                  <Link
                    href="/daily"
                    className="rounded-2xl bg-stone-100 px-3 py-3 text-center text-xs font-black text-stone-700"
                  >
                    Daily
                  </Link>
                  <Link
            ...
            {shouldShowOnboarding(onboardingProfile) && (
              <Link
                href="/onboarding"
                className="flex items-center justify-between rounded-3xl border border-green-200 bg-green-50 p-4 text-left shadow-sm"
              >
                <div>
                  <div className="text-sm font-black uppercase tracking-[0.18em] text-green-700">
                    Finish setup
                  </div>
                  <p className="mt-2 text-sm leading-6 text-stone-700">
                    Choose a starter pack, daily goals, and your first training
                    destination.
                  </p>
                </div>
            ...
            <TempoDailyBlundrCard />
            <DailyRingsCard
              repertoireProgress={repertoireProgress}
              refreshKey={repertoireProgress.updatedAt}
              completionResult={latestCompletionResult}
              onStartTraining={() => router.push("/train")}
            />
            <section className="rounded-3xl border border-stone-200 bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-lg font-black tracking-tight text-stone-950">
                    Build a repertoire you actually remember.
                  </h2>
                  <p className="mt-2 text-sm leading-6 text-stone-600">
                    Unlock openings with points earned from consistent training.
                  </p>
                </div>
                <Link
                  href="/repertoire"
                  className="rounded-2xl bg-white px-4 py-3 text-sm font-black text-green-700 ring-1 ring-green-200"
                >
                  View
                </Link>
              </div>
              <div className="mt-4 grid gap-3">
                <RepertoirePointsSummary
                  ...
                  compact
                />
                <RepertoireUnlockProgress progress={repertoireProgress} />
              </div>
            </section>
            ...
            <section className="space-y-3">
              ...
              {repertoires.slice(0, 3).map((r) => {
                const positions = getRepertoirePositionCount(r);
                const isLocked = repertoireProgress.lockedOpeningIds.includes(
                  resolveStage2CanonicalOpeningId(r.id) ?? r.id,
                );
                return (
                  <button
                    key={r.id}
                    onClick={() => selectRepertoire(r.id)}
                    disabled={isLocked}
                    className={classNames(
                  ...
                  >
                    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-stone-100 text-2xl">
                      {r.color === "white" ? "♙" : "♟"}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="font-bold">{r.name}</div>
                      <div className="text-sm text-stone-500">
                        {r.lines.length} lines • {positions} positions
                      </div>
                      {isLocked ? (
                        <div className="mt-2 inline-flex rounded-full bg-stone-100 px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-stone-500">
                          Locked
                        </div>
                      ) : null}
                    </div>
        ...
        {activeTab === "train" && (
          <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(340px,400px)] lg:items-start">
            ...
            >
                ...
                <div className="mb-3 grid grid-cols-4 gap-2">
                  {RATING_PRESETS.map((p) => (
                    <button
                      ...
                      className={classNames(
                        "rounded-full px-2 py-2 text-[11px] font-black",
              ...
              {blundrDebugEnabled && activeBoard && enabledViews.length > 0 && (
                <div
                  ...
                  style={{
                    gridTemplateColumns: `repeat(${enabledViews.length}, minmax(0,1fr))`,
                  }}
                >
                  {enabledViews.map((v) => (
                    <button
                      key={v}
                      onClick={() => setActiveBoardView(v)}
                      className={classNames(
              ...
              <TapChessboard
                ...
                lines={
                  projectiveTacticsVisualPriorityActive
                    ? []
                    : boardLinesToRender
                }
                transientLines={transientLinesToRender}
                projectiveTacticVisuals={projectiveTacticDisplay.visuals}
                projectiveTacticsFading={projectiveTacticsFading}
            ...
            <aside data-train-aside className="flex min-h-0 flex-col gap-4">
              ...
              {showDetails && (
                ...
                <div>
                  coachUtteranceId: {coachDecision.utteranceId ?? "none"}
                </div>
                <div>
                  coachUtteranceFamily:{" "}
                  {coachDecision.utteranceFamily ?? "none"}
                </div>
                <div>
                  coachVariationReason:{" "}
                  {String(
                    (coachDecision.debug as any)?.coachVariationReason ?? "n/a",
                  )}
                </div>
                <div>
                  coachHintStrength:{" "}
                  {String(
                    (coachDecision.debug as any)?.coachHintStrength ?? "none",
                  )}
                </div>
                <div>coachRevealRisk: {coachDecision.revealRisk}</div>
                <div>
                  coachGivesAnswer:{" "}
                  {coachDecision.givesAnswer ? "true" : "false"}
                </div>
                <div>
                  coachButtons:{" "}
                  {displayedCoachDecision.buttons.join(", ") || "none"}
                </div>
                <div>
                  coachShouldMarkReviewWorthy:{" "}
                  {coachDecision.shouldMarkReviewWorthy ? "true" : "false"}
                </div>
                <div>
                  coachSuppressedReason:{" "}
                  {coachDecision.suppressedReason ?? "none"}
                </div>
                <div>
                  coachFrameMatchesBoard:{" "}
                  {coachContextResult.context?.recipeFrameMatchesBoard
                    ? "true"
                    : "false"}
                </div>
                <div>
                  coachFenMatchesBoard:{" "}
                  {coachContextResult.context?.recipeFenMatchesBoard
                    ? "true"
                    : "false"}
                </div>
                <div>
                  recentCoachUtteranceIds:{" "}
                  {coachUtteranceMemory
                    .slice(-5)
                    .map((entry: any) => entry.utteranceId)
                    .join(", ") || "none"}
                </div>
                <div>
                  coachSafetyWarnings:{" "}
                  {JSON.stringify(
                    (coachDecision.debug as any)?.coachSafetyWarnings ?? [],
                  )}
                </div>
                <div>
                  coachReviewMarked: {coachReviewMarked ? "true" : "false"}
                </div>
                <div>
                  selectedOpportunity:{" "}
                  {String(
                    (coachDecision.debug as any)?.selectedOpportunity ??
                      liveCoachState?.selected?.opportunity ??
                      "none",
                  )}
                </div>
                <div>
                  selectedIntent:{" "}
                  {String(
                    (coachDecision.debug as any)?.selectedIntent ??
                      liveCoachState?.selected?.intent ??
                      "none",
                  )}
                </div>
                <div>
                  exactMoveAllowed:{" "}
                  {coachContextResult.context?.exactMoveAllowed
                    ? "true"
                    : "false"}
                </div>
                <div>
                  claimTypes: {coachDecision.claimTypes.join(", ") || "none"}
                </div>
                <div>
                  blockedClaims:{" "}
                  {String(
                    (coachDecision.debug as any)?.blockedClaims ?? "none",
                  )}
                </div>
                <div>
                  silenceReason:{" "}
                  {String(
                    (coachDecision.debug as any)?.silenceReason ??
                      liveCoachState?.debug?.silenceReason ??
                      "none",
                  )}
                </div>
                <div>
                  branchTransitionSurfaceRendered:{" "}
                  {branchTransitionSurface?.render ? "true" : "false"}
                </div>
                <div>
                  branchTransitionReason:{" "}
                  {branchTransitionSurface?.reason ?? "none"}
                </div>
                <div>
                ...
                <div>
                  continueFromHereClicked:{" "}
                  {continueFromHereClicked ? "true" : "false"}
                </div>
                <div>coachSurfaceOwner: {coachSurfacePolicy.owner}</div>
                <div>
                  allowLegacyTrainingCard:{" "}
                  {coachSurfacePolicy.allowLegacyTrainingCard
                    ? "true"
                    : "false"}
                </div>
                <div>
                  allowMoveImpactCard:{" "}
                  {coachSurfacePolicy.allowMoveImpactCard ? "true" : "false"}
                </div>
                <div>
                  allowNextMoveText:{" "}
                  {coachSurfacePolicy.allowNextMoveText ? "true" : "false"}
                </div>
                <div>
                  legacyCueSuppressedReason: {coachSurfacePolicy.reason}
                </div>
                <div>
                  moveImpactPresenterReason: {moveImpactPresentation.reason}
                </div>
              </div>
            )}
            {!v28SurfaceActive && bookComplete && (
              <div className="rounded-3xl border border-green-200 bg-green-50 p-4 shadow-sm">
                ...
                <p className="mt-2 text-sm leading-6 text-green-800">
                  You finished this training line. Continue from this position,
                  train the line again, or finish and return home.
                </p>
                <div className="mt-4 grid grid-cols-3 gap-3">
                  <button
                    onClick={continueFromHere}
                    className="rounded-2xl bg-green-700 px-4 py-3 font-black text-white shadow-sm"
                  >
                    Continue
                  </button>
                  <button
                    onClick={resetBoard}
                    className="rounded-2xl bg-white px-4 py-3 font-black text-green-800 shadow-sm"
                  >
                    Repeat
                  </button>
                  <button
                    onClick={() => handleCoachAction("finish")}
                    className="rounded-2xl bg-stone-900 px-4 py-3 font-black text-white shadow-sm"
                  >
                    Finish
                  </button>
                </div>
         ...
         handleReveal still exists for internal/debug paths only. */}
            ...
            {false &&
              ...
              !displayedCoachDecision?.shouldShowCoachCard &&
              !branchTransitionSurface &&
              coachSurfacePolicy.allowLegacyAnswerCard &&
              !visibleTeachingSurface?.coach?.shouldRender &&
              !isActiveTeachingFrame && (
                <div className="rounded-3xl bg-stone-900 p-4 text-white">
                  <div className="text-sm text-stone-300">Study-line move</div>
                  <div className="mt-2 text-2xl font-black">
                    {expectedUserOptions.length
                      ? expectedUserOptions.map((m) => m.san).join(" / ")
                      : (engineLines[0]?.san ?? "Analysis pending")}
                  </div>
                  <p className="mt-2 text-xs leading-5 text-stone-400">
                    Source:{" "}
                    {trainingMode === "restricted"
                      ? "Saved repertoire line"
            ...
            {false &&
              ...
              !visibleTeachingSurface?.coach?.shouldRender &&
              !isActiveTeachingFrame && (
                <div className="rounded-3xl border border-stone-200 bg-white/95 p-4 shadow-sm">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <div>
                      <div className="text-xs font-black uppercase tracking-wide text-green-700">
                        {patternCueBadgeLabel.replace("Cue ready", "Plan mode")}
                      </div>
                      <h2 className="text-lg font-black">{patternCue.title}</h2>
                    </div>
                    <button
                  ...
                  {opponentCue &&
                    ...
                    shouldRenderOpponentLastMoveHighlight({
                      ...
                      <p className="mt-2 rounded-2xl bg-purple-50 p-3 text-sm leading-6 text-purple-800">
                        <span className="font-black">Last opponent move: </span>
                        {opponentCue.message}
                      </p>
                    )}
                  {coachSurfacePolicy.allowNextMoveText &&
                    patternCue.next &&
                    (trainerView === "assisted" || showAnswer) && (
                      <p className="mt-2 rounded-2xl bg-stone-50 p-3 text-sm leading-6 text-stone-600">
                  ...
                  {visualModelError && (
                    <p className="mt-2 rounded-2xl bg-amber-50 p-2 text-[11px] font-bold leading-5 text-amber-700">
                      Visual cue unavailable: {visualModelError}
                    </p>
                  )}
                  {coachSurfacePolicy.allowMoveImpactCard &&
                    moveImpactPresentation.show && (
                      <MoveImpact
                        impact={{
                          label: moveImpactPresentation.label,
                          pct: moveImpact.pct,
                          tone: moveImpact.tone,
                          note: moveImpactPresentation.note,
                        }}
                      />
                    )}
                  {showDetails && (
                    <div className="mt-3 space-y-2">
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Headline: {patternCue.title}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Visual:{" "}
                        {activeVisualModelOutput?.animationPackage?.name ??
                          annotation.visualExplanation}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Move Quality Gate
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Version: {MOVE_QUALITY_GATE_VERSION}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Required: {shouldValidateTrainingMove ? "yes" : "no"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Status:{" "}
                        {moveQualityPending
                          ? "pending"
                          : (moveQuality?.status ?? "idle")}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Expected UCI:{" "}
                        {moveQuality?.expectedMovesUci?.join(", ") ||
                          expectedUserUcis.join(", ") ||
                          "none"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Expected SAN: {expectedUserSans.join(", ") || "none"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Stockfish top two:{" "}
                        {moveQuality?.topMoves
                          ?.map((line) => line.uci)
                          .join(", ") || "none"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Reason: {moveQuality?.reason ?? "No validation result."}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Checked:{" "}
                        {moveQuality?.checkedAt
                          ? new Date(moveQuality.checkedAt).toLocaleTimeString()
                          : "n/a"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Hints hidden:{" "}
                        {hideUnverifiedTrainingHints ? "yes" : "no"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Teaching Cue Compiler
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Compiler version: {TEACHING_CUE_COMPILER_VERSION}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Compiler concept:{" "}
                        {teachingOrchestration?.cue.conceptId ?? "none"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Compiler confidence:{" "}
                        {teachingOrchestration
                          ? Number(
                              (
                                teachingOrchestration.cue.debug.confidence ?? 0
                              ).toFixed(3),
                            )
                          : "n/a"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Compiler reason:{" "}
                        {teachingOrchestration?.cue.debug.selectedReason ??
                          "n/a"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Compiler delta:{" "}
                        {teachingOrchestration?.cue.debug.deltaSummary?.join(
                          " | ",
                        ) || "n/a"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Compiler scores:{" "}
                        {teachingOrchestration?.cue.debug.detectorScores
                          ?.map(
                            (s) => `${s.conceptId}:${s.finalScore.toFixed(2)}`,
                          )
                          .slice(0, 6)
                          .join(", ") || "n/a"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Orchestrator tier:{" "}
                        {teachingOrchestration?.classification.tier ?? "n/a"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Selected story:{" "}
                        {teachingOrchestration?.selectedStory?.kind ?? "n/a"} (
                        {teachingOrchestration?.selectedStory?.score.total?.toFixed?.(
                          2,
                        ) ?? "n/a"}
                        )
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Rejected stories:{" "}
                        {teachingOrchestration?.debug.rejectedStories
                          ?.map((r) => `${r.kind}:${r.total.toFixed(2)}`)
                          .join(", ") || "n/a"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Visual budget:{" "}
                        {teachingOrchestration
                          ? JSON.stringify(
                              teachingOrchestration.debug.visualBudget,
                            )
                          : "n/a"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Suppressed visuals:{" "}
                        {teachingOrchestration?.debug.suppressionReasons?.join(
                          ", ",
                        ) || "none"}
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-500">
                        Learning events are being stored locally for future
                        progress and Review features.
                      </div>
                      {annotation.reason && (
                        <div className="rounded-2xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700">
                          Fallback reason: {annotation.reason}
                        </div>
                      )}
                    </div>
                  )}
            ...
            {showDetails && teachingOrchestration && (
              <div className="rounded-3xl border border-stone-200 bg-white/95 p-4 text-xs font-semibold text-stone-500 shadow-sm">
                ...
                <div className="mt-2">Mode: {teachingOrchestration.mode}</div>
                <div>
                  Move trust / context trust: {teachingOrchestration.moveTrust}{" "}
                  / {teachingOrchestration.contextTrust}
                </div>
                <div>
                  Saved move not validated:{" "}
                  {teachingOrchestration.debug.savedMoveNotValidated
                    ? "yes"
                    : "no"}
                </div>
                <div>
                  Next suppressed:{" "}
                  {teachingOrchestration.debug.nextPlaySuppressionReason ??
                    "no"}
                </div>
                <div>
                  Move semantic effects:{" "}
                  {teachingOrchestration.debug.moveSemanticSummary.join(
                    " | ",
                  ) || "n/a"}
                </div>
                <div>
                  Top move comparisons:{" "}
                  {teachingOrchestration.debug.topMoveComparisons
                    .map((c) => `${c.relationship}:${c.alternativeTheme}`)
                    .join(", ") || "n/a"}
                </div>
                <div>
                  Selected grounding:{" "}
                  {teachingOrchestration.debug.selectedStoryGrounding
                    ? JSON.stringify(
                        teachingOrchestration.debug.selectedStoryGrounding,
                      )
                    : "n/a"}
                ...
                <div>
                  Visual alignment:{" "}
                  {teachingOrchestration.debug.visualConceptAlignment}
                </div>
              </div>
            )}
         ...
         They remain in memo deps for legacy input to surface (bypass detection only). */}
            ...
            v28VisibleSurface?.mode === "branch_complete" ? (
              <div className="rounded-3xl border border-green-200 bg-gradient-to-b from-green-50 to-white p-4 shadow-sm">
                ...
                <h3 className="mt-1 text-base font-black text-stone-900">
                  {surfaceCoachCardDecision.title ?? "Line complete"}
                </h3>
                <p className="mt-2 text-sm leading-6 text-stone-700">
                  {surfaceCoachCardDecision.body ?? ""}
                </p>
                <div className="mt-3 grid grid-cols-3 gap-3">
                  {(v28CoachUiModel?.actions ?? [])
                    .filter((action) => action.visible)
                    .map((action) => (
                      <button
                        key={action.kind}
            ...
            {showDetails && visualRecipe && (
              <div className="rounded-3xl border border-stone-200 bg-white/95 p-4 text-xs font-semibold text-stone-500 shadow-sm">
                <div className="font-black text-stone-800">Visual Recipe</div>
                <div className="mt-2">
                  visualRecipeId: {visualRecipe.visualRecipeId}
                </div>
                <div>
                  recipeSchemaVersion: {visualRecipe.recipeSchemaVersion}
                </div>
                <div>patternId: {visualRecipe.patternId}</div>
                <div>recipeMode: {visualRecipe.mode}</div>
                <div>recipeConceptId: {visualRecipe.conceptId}</div>
                <div>recipeFrameId: {visualRecipe.frameId ?? "n/a"}</div>
                <div>recipeFen: {visualRecipe.fen}</div>
                <div>recipeBeatCount: {visualRecipe.beats.length}</div>
                <div>
                  recipePrimitiveCount:{" "}
                  {visualRecipe.beats.reduce(
                    (sum, beat) => sum + beat.primitives.length,
                    0,
                  )}
                </div>
                <div>
                  ...
                  {visualRecipe.beats
                    .flatMap((beat) =>
                      beat.primitives.map(
                        (primitive) => `${primitive.type}:${primitive.id}`,
                      ),
                    )
                    .join(", ") || "none"}
                </div>
                <div>
                  recipePermissions: {JSON.stringify(visualRecipe.permissions)}
                </div>
                <div>
                  recipeLearningAnchor:{" "}
                  {JSON.stringify(visualRecipe.learningAnchor)}
                </div>
                <div>
                  recipeSuppressedReason:{" "}
                  {visualRecipe.debug?.recipeSuppressedReason ?? "none"}
                </div>
                <div>
                  recipeLanes:{" "}
                  {visualRecipe.debug?.recipeLanes?.join(", ") || "none"}
                </div>
                <div>
                  recipeEffectFamilies:{" "}
                  {visualRecipe.debug?.recipeEffectFamilies?.join(", ") ||
                    "none"}
                </div>
                <div>
                  recipePrioritySummary:{" "}
                  {visualRecipe.debug?.recipePrioritySummary ?? "none"}
                </div>
                <div>
                  recipeTimingProfile:{" "}
                  {visualRecipe.debug?.recipeTimingProfile
                    ? JSON.stringify(visualRecipe.debug.recipeTimingProfile)
                    : "n/a"}
                </div>
                <div>
                  recipeOpacityPolicy:{" "}
                  {visualRecipe.debug?.recipeOpacityPolicy
                    ? JSON.stringify(visualRecipe.debug.recipeOpacityPolicy)
                    : "n/a"}
                </div>
                <div>
                  suppressedByPriority:{" "}
                  {visualRecipe.debug?.suppressedByPriority?.join(", ") ||
                    "none"}
                </div>
                <div>
                  suppressedByBudget:{" "}
                  {visualRecipe.debug?.suppressedByBudget?.join(", ") || "none"}
                </div>
                <div>
                  tacticalPrimitivesPresent:{" "}
                  {visualRecipe.debug?.tacticalPrimitivesPresent
                    ? "true"
                    : "false"}
                </div>
                <div>
                  tacticalPrimitivesRendered:{" "}
                  {visualRecipeOverlay.tacticalPrimitivesRendered
                    ? "true"
                    : "false"}
                </div>
                <div>
                  schemaSerializable:{" "}
                  {visualRecipe.debug?.schemaSerializable ? "true" : "false"}
                </div>
                <div>
                  adapterAllowed:{" "}
                  {visualRecipeOverlay.adapterAllowed ? "true" : "false"}
                </div>
                <div>
                  adapterSuppressedReason:{" "}
                  {visualRecipeOverlay.adapterSuppressedReason ?? "none"}
                </div>
                <div>
                  recipeFenRaw: {visualRecipeOverlay.recipeFenRaw ?? "n/a"}
                </div>
                <div>boardFenRaw: {visualRecipeOverlay.boardFenRaw}</div>
                <div>
                  recipeFenNormalized:{" "}
                  {visualRecipeOverlay.recipeFenNormalized ?? "n/a"}
                </div>
                <div>
                  boardFenNormalized:{" "}
                  {visualRecipeOverlay.boardFenNormalized ?? "n/a"}
                </div>
                <div>
                  recipeFrameIdRaw:{" "}
                  {String(visualRecipeOverlay.recipeFrameIdRaw ?? "n/a")}
                </div>
                <div>
                  boardFrameIdRaw: {String(visualRecipeOverlay.boardFrameIdRaw)}
                </div>
                <div>
                  recipeFrameMatchesBoard:{" "}
                  {visualRecipeOverlay.recipeFrameMatchesBoard
                    ? "true"
                    : "false"}
                </div>
                <div>
                  recipeFenMatchesBoard:{" "}
                  {visualRecipeOverlay.recipeFenMatchesBoard ? "true" : "false"}
                </div>
              </div>
            )}
            {showDetails && (
              <div className="rounded-3xl border border-stone-200 bg-white/95 p-4 text-xs font-semibold text-stone-500 shadow-sm">
                ...
                <div className="mt-2">trainerFrameId: {trainerFrameId}</div>
                <div>overlayFrameId: {overlayFrameId}</div>
                <div>
                  overlayFen: {overlayFen ?? visualRecipe?.fen ?? "n/a"}
                </div>
                <div>boardFen: {boardFen}</div>
                <div>
                  overlaySuppressedReason: {overlaySuppressedReason ?? "none"}
                </div>
                <div>overlaySource: {overlaySource}</div>
                <div>
                  opponentCandidateRenderedInMainUi:{" "}
                  {visualRecipeOverlay.opponentCandidateRenderedInMainUi
                    ? "true"
                    : "false"}
                </div>
                <div>
                  staleOverlayIgnored: {staleOverlayFlag ? "true" : "false"}
                </div>
                <div>
                  overlayClearedOnPhaseChange:{" "}
                  {overlayClearedOnPhaseChange ? "true" : "false"}
                </div>
              </div>
            )}
            {showDetails && visualRecipe && (
              <div className="rounded-3xl border border-stone-200 bg-white/95 p-4 text-xs font-semibold text-stone-500 shadow-sm">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={visualRecipePlayback.replay}
                      disabled={
                        !visualRecipePlayback.replayAvailable ||
                        trainerView === "plain"
                      }
                    ...
                    <button
                      onClick={visualRecipePlayback.skipToEnd}
                      disabled={
                        visualRecipePlayback.animationState !== "playing"
                      }
                      className={classNames(
                ...
                <div className="mt-2">
                  animationState: {visualRecipePlayback.animationState}
                </div>
                <div>
                  activeVisualRecipeId:{" "}
                  {visualRecipePlayback.activeVisualRecipeId ?? "none"}
                </div>
                <div>
                  activePatternId:{" "}
                  {visualRecipePlayback.activePatternId ?? "none"}
                </div>
                <div>
                  activeBeatIndex:{" "}
                  {visualRecipePlayback.activeBeatIndex ?? "n/a"}
                </div>
                <div>
                  activeBeatId: {visualRecipePlayback.activeBeatId ?? "n/a"}
                </div>
                <div>
                  activePrimitiveIds:{" "}
                  {visualRecipePlayback.activePrimitiveIds.join(", ") || "none"}
                </div>
                <div>
                  animationReducedMotion:{" "}
                  {visualRecipePlayback.animationReducedMotion
                    ? "true"
                    : "false"}
                </div>
                <div>
                  animationSkippedToEnd:{" "}
                  {visualRecipePlayback.animationSkippedToEnd
                    ? "true"
                    : "false"}
                </div>
                <div>
                  animationClearedReason:{" "}
                  {visualRecipePlayback.animationClearedReason ?? "none"}
                </div>
                <div>
                  animationSuppressedReason:{" "}
                  {visualRecipePlayback.animationSuppressedReason ?? "none"}
                </div>
                <div>
                  recipeFrameMatchesBoard:{" "}
                  {visualRecipePlayback.recipeFrameMatchesBoard
                    ? "true"
                    : "false"}
                </div>
                <div>
                  recipeFenMatchesBoard:{" "}
                  {visualRecipePlayback.recipeFenMatchesBoard
                    ? "true"
                    : "false"}
                </div>
                <div>
                  replayAvailable:{" "}
                  {visualRecipePlayback.replayAvailable ? "true" : "false"}
                </div>
                <div>
                  tacticalPrimitivesRendered:{" "}
                  {visualRecipePlayback.tacticalPrimitivesRendered
                    ? "true"
                    : "false"}
                </div>
              </div>
            ...
            {showDetails && opponentVariationDebug && (
              <div className="rounded-3xl border border-stone-200 bg-white/95 p-4 text-xs font-semibold text-stone-500 shadow-sm">
                <div className="font-black text-stone-800">
                  Opponent Variation Guard
                </div>
                <div className="mt-2">
                  Variation applied:{" "}
                  {opponentVariationDebug.opponentVariationApplied
                    ? "yes"
                    : "no"}
                </div>
                <div>
                  Reason:{" "}
                  {opponentVariationDebug.opponentVariationReason || "n/a"}
                </div>
                <div>
                  Recent branch keys:{" "}
                  {opponentVariationDebug.recentOpponentBranchKeys.join(", ") ||
                    "n/a"}
                </div>
                <div>
                  Selected branch key:{" "}
                  {opponentVariationDebug.selectedOpponentBranchKey ?? "n/a"}
                </div>
                <div>
                  Candidates:{" "}
                  {opponentVariationDebug.candidateOpponentBranches
                    .map(
                      (c) =>
                        `${c.san ?? c.uci}:${c.baseWeight.toFixed(2)}→${c.adjustedWeight.toFixed(2)}:${c.safetyStatus ?? "unknown"}:${c.selectionScore?.toFixed?.(2) ?? "n/a"}${c.blockedReason ? `(${c.blockedReason})` : ""}`,
                    )
                    .join(", ") || "n/a"}
                </div>
                <div>
                  Blocked third-repeat branches:{" "}
                  {opponentVariationDebug.blockedThirdRepeatBranches.join(
                    ", ",
                  ) || "none"}
                </div>
                <div>
                  Fallback used:{" "}
                  {opponentVariationDebug.fallbackUsed ? "yes" : "no"}
                </div>
                <div>
                  continuedPlaySelectedMoveInCandidateList:{" "}
                  {opponentVariationDebug.continuedPlaySelectedMoveInCandidateList
                    ? "true"
                    : "false"}
                </div>
                <div>
                  continuedPlaySelectionConsistency:{" "}
                  {opponentVariationDebug.continuedPlaySelectionConsistency ??
                    "n/a"}
                </div>
                <div>
                  continuationMoveSafetySource:{" "}
                  {opponentVariationDebug.continuationMoveSafetySource ?? "n/a"}
                </div>
              </div>
            )}
            <div className="rounded-3xl border border-stone-200 bg-white p-4 shadow-sm">
              <div className="flex items-start gap-3">
                {feedback.toLowerCase().includes("correct") ? (
                  ...
                  feedback.toLowerCase().includes("illegal") ? (
        ...
        {isActiveTab(activeTab, "review") && (
          <section className="space-y-5">
            <ReviewHub embedded homeHref="/" settingsHref="/settings" />
          </section>
        )}
        {activeTab === "progress" && (
          <section className="space-y-5">
            <ProgressDashboard embedded homeHref="/" settingsHref="/settings" />
          </section>
        )}
      </div>
      {showAddLine && (
        <div className="fixed inset-0 z-[60] flex items-end bg-black/35 p-4">
          <div className="mx-auto w-full max-w-md rounded-3xl bg-white p-5 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-xl font-black">Add Custom Line</h2>
              <button
      ...
      {blundrDebugEnabled && (
        <BlundrDiagnosticsPanel
          ...
          enabled={blundrDebugEnabled}
          onEnabledChange={setBlundrDebugEnabled}
          onClearEvents={() => setDebugEventLog([])}
        />
      )}
    </main>
  );
...
function TapChessboard({
  ...
  lines: ActiveLine[];
  transientLines: ActiveLine[];
  projectiveTacticVisuals: ProjectiveTacticVisual[];
  projectiveTacticsFading: boolean;
  ...
  const centerFor = (sq: string) => {
    const fileIndex = FILE_TO_INDEX[sq[0]],
      rank = Number(sq[1]);
      ...
      <div data-train-board-bleed className="train-board-mobile-bleed">
        ...
        <div className="flex items-stretch gap-0 sm:gap-1.5">
          ...
          >
            ...
            <BoardLines
              lines={transientLines}
              centerFor={centerFor}
              transient
            />
            <ProjectiveTacticalOverlay
              visuals={projectiveTacticVisuals}
              orientation={orientation}
              fading={projectiveTacticsFading}
              showLines={projectiveTacticShowLines}
              showLabels={projectiveTacticShowLabels}
            />
                  ...
                  return (
                    <button
                      ...
                      className={classNames(
                        ...
                        boardThemeClasses(settings.boardTheme, isDark),
                        selectedSquare === square
                          ? "ring-4 ring-inset ring-green-800"
                          : "",
                      )}
                    ...
                    >
                      {showRank ? (
                        <span
                          className={classNames(
                            "pointer-events-none absolute left-1 top-0.5 text-[10px] font-black",
                            coordTone(settings.boardTheme, isDark),
                          )}
                        >
                          {rank}
                        </span>
                      ...
                      {showFile ? (
                        <span
                          className={classNames(
                            "pointer-events-none absolute bottom-0.5 right-1 text-[10px] font-black",
                            coordTone(settings.boardTheme, isDark),
                          )}
                        >
                          {file}
                      ...
                      <span
                        className={classNames(
                          "pointer-events-none flex h-full w-full items-center justify-center leading-none antialiased",
                          settings.pieceStyle === "letters"
            ...
            {pendingPromotion && (
              ...
              >
                <div
                  className="w-full max-w-[250px] rounded-[24px] border border-stone-200 bg-white p-3 shadow-2xl"
                ...
                >
                  <div className="mb-2 flex items-center justify-between gap-3">
                    ...
                    <button
                      type="button"
                      onClick={onPromotionCancel}
                      className="rounded-full bg-stone-100 px-2 py-1 text-[10px] font-black text-stone-600"
                    >
                      Cancel
                    </button>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {(["q", "r", "b", "n"] as PromotionPiece[]).map((piece) => {
                      const enabled = pendingPromotion.legalPromotionUcis.some(
                        (uci) => uci.endsWith(piece),
                      );
                      return (
                        <button
                          key={piece}
...
function CapturedStrip({
  ...
  return (
    <div className="my-2 flex min-h-8 items-center justify-between rounded-2xl bg-white/90 px-3 py-2 text-xs font-black text-stone-600 shadow-sm ring-1 ring-stone-200">
      <div className="flex min-w-0 items-center gap-2">
        ...
        <span className="truncate text-base leading-none">
          {captured.length ? (
            captured.map((t, i) => (
              <span key={`${t}-${i}`}>
                {pieceGlyph(color, t, settings.pieceStyle)}
              </span>
            ))
          ) : (
            <span className="text-xs text-stone-300">no captures</span>
          )}
        </span>
      </div>
      {advantage > 0 ? (
        ...
        <span className="rounded-full bg-stone-100 px-2 py-1 text-stone-400">
          even
        </span>
      )}
    </div>
...
function temporalGateColor(line: ActiveLine, transient: boolean) {
  if (transient || line.kind === "opponent")
    return {
      primary: "#b884ff",
      secondary: "#d2b0ff",
      danger: "#f0e5ff",
      soft: "rgba(184,132,255,.14)",
    };
...
function BoardLines({
  ...
  lines: ActiveLine[];
  centerFor: (s: string) => { x: number; y: number };
  transient: boolean;
}) {
  const visible = lines
    .filter((l) => isValidSquare(l.from) && isValidSquare(l.to))
    .slice(0, transient ? 1 : 2);
  ...
  return (
    ...
    >
      <defs>
        {visible.map((l, i) => {
          const c = temporalGateColor(l, transient);
          return (
            <linearGradient
              key={i}
              id={`tg-${transient ? "t" : "p"}-${i}`}
              x1="0%"
              y1="0%"
              x2="100%"
              y2="100%"
            >
              ...
              <stop offset="54%" stopColor={c.secondary} stopOpacity=".92" />
              <stop offset="100%" stopColor={c.primary} stopOpacity=".76" />
            </linearGradient>
          );
        })}
      </defs>
      {visible.map((l, i) => {
        const f = centerFor(l.from),
          t = centerFor(l.to),
          c = temporalGateColor(l, transient);
        const knight = isKnightGeometry(l.from, l.to);
        const corner = { x: t.x, y: f.y };
        const d = knight
          ? `M ${f.x} ${f.y} L ${corner.x} ${corner.y} L ${t.x} ${t.y}`
          : `M ${f.x} ${f.y} Q ${(f.x + t.x) / 2} ${(f.y + t.y) / 2 - 3.2} ${t.x} ${t.y}`;
        const points = `${f.x},${f.y} ${corner.x},${corner.y} ${t.x},${t.y}`;
        return (
          <g
            key={`${l.from}-${l.to}-${i}`}
            className={
              transient ? "blundr-opponent-line" : "blundr-intent-line"
            }
          >
            <circle cx={f.x} cy={f.y} r="4.0" fill={c.soft} opacity=".95" />
            <circle
              cx={f.x}
              cy={f.y}
              r="3.15"
              ...
              opacity=".9"
            />
            <circle cx={t.x} cy={t.y} r="6.1" fill={c.soft} opacity=".95" />
            <circle
              cx={t.x}
              cy={t.y}
              r="4.7"
              ...
              opacity=".96"
            />
            <circle cx={t.x} cy={t.y} r="2.15" fill={c.primary} opacity=".22" />
            {knight ? (
              <>
                <polyline
                  points={points}
                  fill="none"
                  stroke={c.soft}
                  strokeWidth={transient ? "2.4" : "2.15"}
                  strokeLinecap="round"
                ...
                <polyline
                  points={points}
                  fill="none"
                  stroke={`url(#tg-${transient ? "t" : "p"}-${i})`}
                  strokeWidth={transient ? "1.2" : "1.02"}
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </>
...
function HistoryControls({
  ...
  index: number;
  total: number;
  lessonProgress?: RestrictedRuntimeLineProgress | null;
  onBack: () => void;
  onForward: () => void;
}) {
  ...
  const label = reviewing
    ...
    : lessonProgress && lessonProgress.totalLearnerMoves > 0
      ...
      : "Start position";
  return (
    <div className="mt-3 flex items-center justify-between rounded-2xl bg-stone-50 px-3 py-2 text-xs font-black text-stone-500">
      <button
        disabled={index <= 0}
        onClick={onBack}
        className="rounded-full bg-white px-3 py-2 text-stone-700 shadow-sm disabled:opacity-30"
      >
        ← Back
      </button>
      <span>{label}</span>
...
function GameEndCard({
  ...
  return (
    <div className="rounded-3xl border border-amber-200 bg-amber-50 p-5 text-center shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-amber-950">{title}</h2>
      <p className="mt-2 text-sm leading-6 text-amber-800">{message}</p>
      <button
        onClick={onRestart}
        className="mt-4 w-full rounded-2xl bg-amber-600 px-4 py-3 font-black text-white shadow-sm"
      >
        Restart
      </button>
    </div>
...
function SettingsPanel({
  ...
  const OptionButton = ({
    ...
    <button
      ...
      className={classNames(
        ...
        active ? "bg-green-700 text-white" : "bg-stone-100 text-stone-600",
      )}
    >
      {label}
    </button>
  ...
  const Toggle = ({
    ...
    >
      <span>{label}</span>
      <span>{settings[id] ? "ON" : "OFF"}</span>
    </button>
  );
  return (
    <div className="fixed inset-0 z-[70] flex items-end bg-black/35 p-4">
      <div className="mx-auto max-h-[86vh] w-full max-w-md overflow-auto rounded-3xl bg-white p-5 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-black">Board Settings</h2>
            <p className="text-xs font-semibold text-stone-500">
              Customize board, pieces, and active displays.
            </p>
          </div>
        ...
        <div className="space-y-5">
          ...
          <div>
            <div className="mb-2 text-sm font-black">Board</div>
            <div className="grid grid-cols-3 gap-2">
              <OptionButton
                active={isHomeDefaultBoardTheme(settings.boardTheme)}
                label="Default"
                onClick={() => update("boardTheme", "default")}
              />
              <OptionButton
                active={settings.boardTheme === "blue"}
                label="Blue"
                onClick={() => update("boardTheme", "blue")}
              />
              <OptionButton
                active={settings.boardTheme === "walnut"}
                label="Walnut"
                onClick={() => update("boardTheme", "walnut")}
              />
            </div>
          </div>
          <div>
            <div className="mb-2 text-sm font-black">Pieces</div>
            <div className="grid grid-cols-3 gap-2">
              <OptionButton
                active={settings.pieceStyle === "unicode"}
                label="Classic"
                onClick={() => update("pieceStyle", "unicode")}
              />
              <OptionButton
                active={settings.pieceStyle === "neo"}
                label="Neo"
                onClick={() => update("pieceStyle", "neo")}
              />
              <OptionButton
                active={settings.pieceStyle === "letters"}
                label="Letters"
                onClick={() => update("pieceStyle", "letters")}
              />
            </div>
          </div>
          <div>
            <div className="mb-2 text-sm font-black">Active displays</div>
            <div className="grid grid-cols-2 gap-2">
              <Toggle id="showMoveDots" label="Legal move dots" />
              <Toggle id="showEvalBar" label="Advantage bar" />
          ...
          <button
            onClick={onClose}
            className="w-full rounded-2xl bg-stone-950 px-4 py-4 font-black text-white"
          >
            Done
          </button>
        </div>
...
function PipelineStatus({ step, note }: { step: ThinkingStep; note: string }) {
  ...
  const tone =
    step === "error"
      ...
      : step === "ready" || step === "idle"
        ? "bg-green-50 text-green-700 ring-green-100"
        : "bg-blue-50 text-blue-700 ring-blue-100";
  return (
    <div
      className={classNames(
        "max-w-[190px] rounded-2xl px-3 py-2 text-right text-[11px] font-black leading-4 ring-1",
        tone,
      )}
      title={note}
    >
...
function MoveImpact({
  ...
  impact: { label: string; pct: number; tone: string; note: string };
}) {
  return (
    <div className="mt-3 rounded-2xl bg-stone-50 p-3">
      <div className="mb-2 flex items-center justify-between text-xs font-black">
        <span>Move Impact</span>
        <span className="text-green-700">{impact.label}</span>
      </div>
      <div className="h-2 rounded-full bg-stone-200">
        <div
          className={classNames("h-2 rounded-full", impact.tone)}
          style={{ width: `${impact.pct}%` }}
        />
      </div>
      <p className="mt-2 text-xs leading-5 text-stone-500">{impact.note}</p>
    </div>
  );
}
function GptDebugPanel({
  open,
  setOpen,
  text,
}: {
  ...
  return (
    <div className="rounded-3xl border border-stone-200 bg-white p-3 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs font-semibold text-stone-500">
            Live prompt/input/output from /api/brain
          </div>
        </div>
        <button
      ...
      {open ? (
        <pre className="mt-3 max-h-72 overflow-auto rounded-2xl bg-stone-950 p-3 text-[10px] leading-4 text-green-200 whitespace-pre-wrap">
          {text}
        </pre>
      ) : null}
    </div>
    ...
    <div className="rounded-3xl border border-stone-200 bg-white p-3 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs font-semibold text-stone-500">
            Rule visual payload/response plus local-only telemetry events.
          </div>
        </div>
        <button
          onClick={() => setOpen(!open)}
        ...
        <div className="mt-3 space-y-3">
          <div className="flex items-center justify-between rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-600">
            <span>
              ...
              {telemetryCount} event{telemetryCount === 1 ? "" : "s"}
            </span>
            <div className="flex items-center gap-2">
              <button
              ...
              <button
                ...
                className="rounded-full bg-stone-200 px-3 py-1 font-black text-stone-700"
              >
                Clear
              </button>
            </div>
          ...
          <div>
            <div className="mb-1 text-[11px] font-black uppercase tracking-wide text-stone-500">
              Visual Snapshot
            </div>
            <pre className="max-h-52 overflow-auto rounded-2xl bg-stone-950 p-3 text-[10px] leading-4 text-green-200 whitespace-pre-wrap">
              {visualText}
            </pre>
          </div>
          <div>
...
function LiveBrain({ brain }: { brain: LiveBrain }) {
  return (
    <div className="rounded-3xl border border-stone-200 bg-white p-3 shadow-sm">
      <div className="mb-2 flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm font-black">
          <Zap size={16} className="text-green-700" /> Live Brain
        </div>
        <span className="text-xs font-black text-stone-400">
          {brain.ratingLabel} • {brain.ratingPool}
        </span>
      </div>
      <div className="grid grid-cols-4 gap-1.5">
        <Status label="Book" state={brain.book} />
        <Status label="Lichess" state={brain.lichess} />
        <Status label="Engine" state={brain.engine} />
        <Status label="Brain" state={brain.gpt} />
      </div>
      <div className="mt-2 rounded-2xl bg-stone-50 px-3 py-2 text-xs font-semibold leading-5 text-stone-500">
        Source:{" "}
        <span className="font-black text-stone-800">{brain.source}</span>
        {brain.latency ? ` • ${brain.latency} ms` : ""}
        {brain.note ? ` • ${brain.note}` : ""}
      </div>
    </div>
  );
}
function Status({ label, state }: { label: string; state: SystemState }) {
  const tone =
    state === "active" || state === "ready" || state === "cached"
  ...
  return (
    <div
      className={classNames(
        "rounded-full px-2 py-1 text-center text-[10px] font-black",
        tone,
      )}
    >
      {label}: {state}
  ...
  return (
    ...
    >
      <div
        className={classNames(
          "mb-2",
          warning ? "text-orange-600" : "text-green-700",
        )}
      >
        {icon}
      </div>
...
function BottomNav({
  ...
  return (
    <BlundrBottomNav
      activeTab={
        activeTab as "home" | "train" | "review" | "progress" | "repertoire"
      }
    />
  );
}
