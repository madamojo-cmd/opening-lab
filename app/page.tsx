...
function deriveFastAnnotation({fen,openingName,userColor,trainingMode,expectedUserOptions,opponentBookOptions}: {fen:string;openingName:string;userColor:ChessColor;trainingMode:TrainingMode;expectedUserOptions:Continuation[];opponentBookOptions:Continuation[]}):BrainAnnotation{
  ...
  const legal=(local.moves({verbose:true}) as any[]).filter(m=>m.color===local.turn()).slice(0,2);
  const lines=legal.map(m=>lineFromVerboseMove(m,"plan"));
  const cues:SquareCue[]=lines.flatMap(l=>[{square:l.from,kind:"origin" as const},{square:l.to,kind:"target" as const}]);
  const moveText=legal.map(m=>m.san).join(" / ")||"any legal move";
  return{...base,source:"fast local continuation",fallback:true,selectedView:"plan",headline:trainingMode==="continuation"?"Continuation mode":"Waiting for opponent",mainExplanation:trainingMode==="continuation"?"Legal moves are accepted from this position. Brain will refine the best teaching cue after analysis returns.":"The opponent is selecting a continuation.",visualExplanation:"The board is showing an immediate legal-move cue so the UI never waits on GPT.",planExplanation:"Use this as a temporary local cue. Engine/GPT refinement may update the coaching text if the FEN still matches.",nextPlan:trainingMode==="continuation"?`Candidate legal move: ${moveText}.`:"Wait for the opponent move.",keySquares:lines.map(l=>l.to),planArrows:lines,attack:{title:"Candidate activity",message:"Fast continuation cue. Tactical labels are withheld unless validated.",lines:lines.map(l=>({...l,kind:"attack" as LineKind})),cues},defense:{title:"Candidate safety",message:"No defensive tactic is claimed locally. Brain may refine this after validation.",lines:[],cues:lines.map(l=>({square:l.from,kind:"support" as const}))},plan:{title:"Fast local continuation",message:`Candidate legal continuation: ${moveText}.`,lines,cues},confidence:"local"};
}
...
export default function App(){
  ...
  useEffect(()=>{if(activeTab!=="train")return;const fast=deriveFastAnnotation({fen,openingName:repertoire.name,userColor,trainingMode,expectedUserOptions,opponentBookOptions});setAnnotation(fast);setVisualReady(true);setThinkingStep("ready");setPipelineNote("Fast local visual ready. Brain is refining in the background.");void runBrain("position_update")},[fen,activeTab,selectedRepertoireId,trainingMode,ratingFilter,activeBoardView]);
  ...
  async function runBrain(eventType:string,extra:Record<string,any>={}){
    ...
    setPipelineNote("Fast local visual is active. Checking chess.js facts for Brain refinement.");
    setBrain(p=>({...p,engine:"loading",gpt:extra.skipGpt?"fallback":"loading",source:"Browser Stockfish"}));
    setThinkingStep("engine");
    setPipelineNote("Running lightweight browser Stockfish without blanking the board.");
    ...
    setPipelineNote(extra.skipGpt?"Asking Brain for engine-only continuation.":"Sending local facts to Brain. Current board stays usable.");
    ...
    try{
      ...
      setPipelineNote(data.annotation?.fallback?"Received fallback refinement for current FEN":"Received GPT refinement for current FEN");
      setBrainResponse(data);
      setAnnotation(data.annotation);
      setVisualReady(true);
      setThinkingStep("visual-update");
      setPipelineNote("Applied current-FEN Brain refinement.");
      window.setTimeout(()=>{if(requestSeq===brainSeq.current&&normalizeFen(fenRef.current)===normalizeFen(requestFen)){setThinkingStep("ready");setPipelineNote(data.annotation?.fallback?"Ready with local/fallback annotation":"Ready with GPT-refined annotation")}},250);
      setBrain(p=>({...p,engine:data.engine?.fallback?"fallback":"active",gpt:data.annotation?.fallback?"fallback":"active",latency:Math.round(performance.now()-start),source:data.annotation?.fallback?"Brain fallback":"GPT visual Brain",note:data.pipeline?.gpt||data.annotation?.reason||""}));
      return data;
    }catch(e){
