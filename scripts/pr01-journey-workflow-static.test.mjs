...
assert.match(workflow, /Journey A preflight and fresh zero-to-49 rebuild/);
assert.match(workflow, /Journey B preflight and exact 25-to-49 upgrade/);
assert.match(
  workflow,
  /BLUNDR_EXPECTED_MIGRATION_COUNT=49 BLUNDR_EXPECTED_MIGRATION_HEAD=20260914120000/,
);
assert.match(workflow, /tail -n 24 >"\$expected_pr03_migration"/);
...
assertInOrder(
  "Journey A preflight and fresh zero-to-49 rebuild",
...
assertInOrder(
  "Journey B preflight and exact 25-to-49 upgrade",
